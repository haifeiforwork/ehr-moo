...
/**
 * 사업장코드 관리용 Controller
 * 
 * @author 최재영
 * @version $Id: WorkPlaceController.java 16243 2011-08-18 04:10:43Z giljae $
 */
@Controller
@RequestMapping(value = "/portal/admin/code/workplace")
@SessionAttributes("workplace")
public class WorkPlaceController extends BaseController {
	...
	public ModelAndView getList(
		...
		ModelAndView mav = new ModelAndView("portal/admin/code/workplace/workPlaceList");
		...
		try {
			...
			String workPlaceCode = request.getParameter("tempCode");

			if (workPlaceCode != null && !"".equalsIgnoreCase(workPlaceCode)) {

				workPlace = workPlaceService.read(workPlaceCode);
				workPlace.setCodeCheck("modify");

				mav.addObject("oldSortOrder", workPlace.getSortOrder());
			} else {
				workPlace = new WorkPlace();
				workPlace.setSortOrder(workPlaceService.getMaxSortOrder());
			}

			searchCondition.setUserLocaleCode(sessionUser.getLocaleCode());
			searchCondition.setPortalId(portal.getPortalId());

			SearchResult<WorkPlace> searchResult = workPlaceService.list(searchCondition);
	...
	public String getForm(
		...
		if (code != null && !"".equalsIgnoreCase(code)) {
			workPlace = workPlaceService.read(code);
			workPlace.setCodeCheck("modify");

			model.addAttribute("oldSortOrder", workPlace.getSortOrder());
		} else {
			workPlace = new WorkPlace();
			workPlace.setCodeCheck("new");
			workPlace.setSortOrder(workPlaceService.getMaxSortOrder());
		}
		...
		return "portal/admin/code/workplace/form";
	}
	...
	String checkCode(
		...
		boolean result = workPlaceService.exists(code);
	 ...
	 * 
	 * @param workPlace 신규/수정 등록하고자 하는 직위(WorkPlace) 객체
	 * @param accessResult 사용자 권한체크 결과
	 ...
	 */
	@RequestMapping(value = "/createWorkPlace.do", method = RequestMethod.POST)
	public @ResponseBody
	String onSubmit(
			@IsAccess(@Attribute(type = AccessType.SYSTEM, className = "Portal", operationName = { "MANAGE" }, resourceName = "Portal")) @ValidEx WorkPlace workPlace,
		...
		String code = workPlace.getWorkPlaceCode();
		Portal portal = (Portal) getRequestAttribute("ikep.portal");
		boolean isCodeExist = workPlaceService.exists(code);

		// workPlaceCode가 이미 존재하는 경우, 기존의 코드를 수정하는 프로세스
		if (isCodeExist) {

			workPlace.setPortalId(portal.getPortalId());
			workPlace.setUpdaterId("admin");
			workPlace.setUpdaterName("관리자");

			workPlaceService.update(workPlace);

		} else {

			workPlace.setSortOrder(request.getParameter("sortOrder"));
			workPlace.setPortalId(portal.getPortalId());
			workPlace.setRegisterId("admin");
			workPlace.setRegisterName("관리자");
			workPlace.setUpdaterId("admin");
			workPlace.setUpdaterName("관리자");
	 ...
	 */
	@RequestMapping(value = "/deleteWorkPlace.do")
	public String delete(
			@IsAccess(@Attribute(type = AccessType.SYSTEM, className = "Portal", operationName = { "MANAGE" }, resourceName = "Portal")) @RequestParam("workPlaceCode") String code,
		...
		workPlaceService.delete(code);

		return "redirect:/portal/admin/code/workplace/getList.do";
	}
	...
	String goUp(
		...
		map.put("workPlaceCode", code);
		map.put("sortOrder", sortOrder);

		workPlaceService.goUp(map);
