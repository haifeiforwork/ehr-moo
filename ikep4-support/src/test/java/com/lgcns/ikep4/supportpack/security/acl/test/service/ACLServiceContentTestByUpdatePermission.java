 ...
 * 
 * @author 주길재
 * @version $Id: ACLServiceContentTestByUpdatePermission.java 16259 2011-08-18 05:40:01Z giljae $
 */
@ContextConfiguration({"classpath:/configuration/spring/context-dao.xml", "classpath:/configuration/spring/context-service.xml"}) 
public class ACLServiceContentTestByUpdatePermission extends AbstractTransactionalJUnit4SpringContextTests {
	@Autowired
	...
	public void updatePermission() {
		// 현재 권한 정보
		ACLResourcePermission firstAclRp = aclService.getContentPermission("BBS-BD-Item", "TestResourceGIL", "READ");
		// 변경할 권한 정보
		...
		// 권한 정보 업데이트
		aclService.updateContentPermission(secondAclRp);
		
		// 수정한 데이터를 가져온다.
		ACLResourcePermission thirdAclRp = aclService.getContentPermission("BBS-BD-Item", "TestResourceGIL", "READ");
		
		// 업데이트 되었는지 확인
		assertFalse(firstAclRp.equals(thirdAclRp));
	}
}
