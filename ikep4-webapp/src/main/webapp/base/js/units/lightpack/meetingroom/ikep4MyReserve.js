...
//	}
	...
	function _deleteEvent(params, isRef) {
		...
		$.postJSON(url, params, function(res) {		
			...
			var url = home_url + "lightpack/meetingroom/reserve/myReserveList.do";
...
//		}
	...
	function createEvent(event, isRef) {
		...
		var startTimePick = $("#start-time-pick").val();
		var endTimePick = $("#end-time-pick").val();
		
		if(startTimePick >= endTimePick){
	...
	function updateEvent(newSchedule, isRef) {
		...
		if(schedule.sco.repeatType) { // 반복일정
				...
				close: function(e, ui) {
					if(dialogButton == 1) {
						...
						$.postJSON(url, params, function(res) {
							...
							var subContext = $("#listType").val();
							var mainUrl = home_url + "lightpack/meetingroom/" + subContext + "/meetingRoomMain.do";
			...
			$.postJSON(url, params, function(res) {
				...
				var subContext = $("#listType").val();
				var mainUrl = home_url + "lightpack/meetingroom/reserve/myReserveList.do";
				
				$jq("#searchForm").attr("action", mainUrl);
				$jq("#searchForm")[0].submit();
			});
		}
	}
	...
	function initValidator() {
		...
		validatorObj = new iKEP.Validator("#fileUploadForm", {
			...
			notice : {
				title:{
				direction : "bottom",
				title : Message.messageText.maxTitle
				}
			},
			submitHandler : save
		});
	...
	function getData() {
		...
		var recurrence,repeatType,rt, repeatPeriodOption = "",ropIdx,
			...
			sdstart = $.toDate( $("#start-date-picker").val() + $("#start-time-pick").val() ),
			sdend = $.toDate( $("#end-date-picker").val() + $("#end-time-pick").val() );
		...
		*/
		...
		var newSchedule = {
				...
				cartooletcId		: $("#tool").val(),
				cartooletcName		: $("#toolName").val()
		};
		/*
		 * 반복 일정 
	...
	function cancel() {
		...
		if(sco && sco.scheduleId.length > 0) {
			...
			url = home_url + "lightpack/meetingroom/reserve/myReserveView.do";
			...
			url = home_url + "lightpack/meetingroom/" + subContext + "/meetingRoomMain.do";
