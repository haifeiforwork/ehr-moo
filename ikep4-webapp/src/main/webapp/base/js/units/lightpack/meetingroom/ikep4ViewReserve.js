var viewUrl = iKEP.getContextRoot() + "/lightpack/meetingroom/reserve/reservePopUpView.do?docPopup=true&scheduleId=";
var fileDownloadUrl = iKEP.getContextRoot() + '/support/fileupload/downloadFile.do?thumbnailYn=N&fileId=';
...
(function($) {
	function populate(schedule) {
		...
		$("#meetingRoomId").val(schedule.meetingRoomId);
		...
		*/
		...
		s = schedule.schedulePublic == 0 ? iKEPLang.planner.labelText.open : iKEPLang.planner.labelText.noopen;
		...
		$(".btn_sendmail").click(function() {
			...
			content += "<br><p><a href='" + viewUrl + scheduleId + 
	...
	function doDeleteSchedule(data) {
		...
		var url = iKEP.getContextRoot() + "/lightpack/meetingroom/reserve/deleteReserve.do";
		$.postJSON(url, data, function(res) {
			
			var meetingRoomId = $("#meetingRoomId").val();
			
			$("#tabs").ajaxLoadComplete();
			
			var subContext = $("#listType").val();
			//var url = home_url + "lightpack/meetingroom/" + subContext + "/meetingRoomMain.do";
			var url = home_url + "lightpack/meetingroom/main.do";
	...
	function doDeleteView(viewType, data) {
		
		data.sendmail = (scheduleInfo.participantList.length > 0) ? confirm(iKEPLang.planner.messageText.doSendMail) : false;
		
		var tartDiv = "#" + viewType + "Div";
		
		$(tartDiv).ajaxLoadStart();
		
		var url = iKEP.getContextRoot() + "/lightpack/meetingroom/" + viewType + "/deleteReserve.do";
		
		$.postJSON(url, data, function(res) {
			
			$(tartDiv).ajaxLoadComplete();
