( function($) {
	function eventMgmt(options) {
		...
		var myWorkspaceUrl = iKEP.getContextRoot() + "/collpack/collaboration/workspace/myWorkspace.do?userId=";
...
//			});
		...
		function setDivOption(event) {
			if(event.sco.searchCode == "1"){
				//휴가
				$("#optCheck1").attr("checked", false);
				$("#opt_tr").hide();
				$("#optCheck2").val("1");
			}else if(event.sco.searchCode == "2"){
				//출장
				$("#optCheck1").attr("checked", false);
				$("#opt_tr").hide();
				$("#optCheck2").val("2");
			}else if(event.sco.searchCode == "3"){
				$("#optCheck1").attr("checked", true);
				$("#optCheck2").val("3");
			}else{
				$("#optCheck1").attr("checked", false);
				$("#optCheck2").val("0");
		...
		function searchUser(event, selId) {	// append
			...
			iKEP.searchPlannerUser(event, "Y", function( data ) {
		...
		function renderTeam(event) {
			...
			if(userId == "hyun9313" || userId == "softpower" || userId == "qo0319" || userId == "lak511"){
				$("<option value='100000849143'>국내영업1팀</option>").appendTo("#team-sel");
			}else{
				if($("#leftMenu-pane").size() > 0 && !mid) { // left menu가 있는 경우 and 위임자 일정이 아닌 경우
					$("#team-pane li").each( function(i, n) {
						var item = $(this).data("workspace");
						$("<option value=" +item.workspaceId+">" + item.workspaceName + "</option>").appendTo("#team-sel");
					} );			
					updateTeam(event);
				} else {
		...
		function getData() {
			...
			var newSchedule = {
					...
					searchCode			: $("#optCheck2").val()
							
			};
			if($("input[name=optCheck1]").is(":checked")){
				newSchedule.searchCode = "3";
			}else{
				if($("#optCheck2").val() != "1" && $("#optCheck2").val() != "2"){
					newSchedule.searchCode = "0";
				}
			}
		...
		function updateEvent(newSchedule, isRef, wh) {
			...
			if(schedule.sco.repeatType) { // 반복일정
				...
				$mgmtPanel.ajaxLoadStart();
				$.postJSON("updateSchedule.do", params, function(res) {
					destroy();
		...
		function _deleteEvent(params, isRef) {
			...
			$mgmtPanel.ajaxLoadStart();
			$.postJSON("deleteSchedule.do", params, function(res) {		
				destroy();
		...
		function strikeEvent(event) {
			...
			$.postJSON("strikeSchedule.do", params, function(res) {		
				destroy();
...
//						}			
		...
		function populate(schedule, event) {
			...
			d = "";
			if(schedule.searchCode == "1"){
				d = "휴가";
			}else if(schedule.searchCode == "2"){
				d = "출장";
			}else if(schedule.searchCode == "3"){
				d = "고객사 방문";
			}
			$("#opt td").append(d);
					...
					$("a.accept, a.reject", $participants).click(function(event) {
						...
						switch(true) {
							case $anchor.hasClass("accept") :	// 참석
								...
								$.postJSON("updateAcceptInfo.do", params, function(data) {
									$anchor.parent()
			...
			$(".btn_update").click(function() {
				...
				event.sco.searchCode = schedule.searchCode;
				editEvent(event);
			});
		...
		function viewEvent(event) {
			$.get("getScheduleAllData.do", {scheduleId: event.id}, function(data) {
