$jq(document).ready(function() {
	...
	function init() {
		...
		.bind("onDirectCreate", function(e, data) { // 2014.09.03 웹다이어리 등록 화면을 위한 기능 추가
			if(data["start"]) data.start = new Date(data.start);
			if(data["end"]) data.end = new Date(data.end);
			currentSource = {source: "myCalendar", workspaceId: sWorkspaceId};
			editEvent(data);
	...
	function init2() {
		...
		_action = urlParameters["action"] || "onDirectCreate";
		
		//if(_action != "onDirectUpdate") calElement.show();
		if((_action != "onDirectUpdate")||(_action != "onDirectCreate")) calElement.show();
	}
