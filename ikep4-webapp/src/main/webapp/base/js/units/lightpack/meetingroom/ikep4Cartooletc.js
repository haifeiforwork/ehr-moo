	...
	function init() {
		...
		*/
		...
		.delegate("#checkDuplicateRepeatReserve", "click", function(e) {
			...
			$.postJSON(home_url + "lightpack/meetingroom/reserve/checkCarDuplicate.do", event, function(res) {
				if(res.length > 0) {
...
//	}
	...
	function _deleteEvent(params, isRef) {
		params.sendmail = isRef && confirm(messageConfirmMail);
		...
		$.postJSON(url, params, function(res) {		
			...
			var cartooletcId = $("#cartooletcId").val();
			...
			$jq.ajax({     
				...
				data :  {
					...
					cartooletcId : cartooletcId,
					categoryId : $("building").val(),
					regionId : $("floor").val()
				},     
				type : "post",     
				loadingElement : {
...
//		}
	...
	function existParticipant() {
		if($("#participants")[0].options.length > 0) {
			return true;
		} else {
			return false;
		}
	...
	function createEvent(event, isRef) {
		...
		var url = home_url + "lightpack/meetingroom/reserve/createCarReserve.do";
		...
		$.postJSON(url, event, function(res) {
			if(res.success == 'success') {
				...
				var url = home_url + "lightpack/meetingroom/" + getTabName() + "/cartooletcMain.do";
				$("#tabs").load(url, {
					cartooletcId : event.cartooletcId,
	...
	function getData() {
		...
		var newSchedule = {
				...
				categoryId			: "1",
				categoryName		: "",
				schedulePublic		: 0,		// 일정 공개 여부 ( 0 : 공개일정, 1 : 비공개일정)
	...
	function save() {
		...
		if(newSchedule) {
			...
			if(newSchedule.scheduleId) {
				...
				updateEvent(newSchedule, false);
			} else {			
				
				createEvent(newSchedule, false);
			}	
	...
	function cancel() {
		...
		if(sco && sco.scheduleId.length > 0) {
			...
			url = home_url + "lightpack/meetingroom/" + getTabName() + "/cartooletcMain.do";
