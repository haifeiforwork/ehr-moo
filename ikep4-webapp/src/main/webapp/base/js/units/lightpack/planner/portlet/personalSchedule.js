$jq(document).ready(function() {
	...
	$jq.getJSON(home_url+"lightpack/planner/calendar/readUserDateSchedule.do",
			{userId: dailyOptions.userId, 
			 startDate: dailyOptions.personalScheduleDate,
			 endDate: dailyOptions.personalScheduleDate}, 
			function(data) {
				 var event = {};
				$jq.each(data, function(n, item) {
					...
					//alert(dailyOptions.loginUserId);
					if(dailyOptions.loginUserId != item.registerId && item.schedulePrivate == '0'){
						events.push(event);
					}else if(dailyOptions.loginUserId == item.registerId){
						events.push(event);
					}
				});			
...
//		var end = dayDiff(cloneDate(today, true), cloneDate(e.end, true)) == 0 ? formatDate(e.end, "HH:mm") : formatDate(e.end, "MM.dd HH:mm");
		...
		var loginUserId = dailyOptions.loginUserId;//로그인 사용자
