 ...
 $.fn.splitter = function(args){
	...
	return this.each(function() {
		...
		var opts = $.extend({
			...
			barNormalClass: "splitter-bar-vertical",			// splitbar normal
			barHoverClass:  "splitter-bar-vertical-hover",			// splitbar mouse hover
		...
		if ( opts.resizeTo ) {
			...
			$(window).bind("resize"+opts.eventNamespace, function(){
				...
				var _vheight = Math.ceil(Math.max(eh-top-splitter._hadjust, splitter._hmin))-5;
				splitter.height(_vheight);
		...
		splitter
			...
			.bind("resize"+opts.eventNamespace, function(e, size){
				...
				var dd = !isNaN(size)? size : (!(opts.sizeRight||opts.sizeBottom)? A[0][opts.pxSplit] : splitter._DA-B[0][opts.pxSplit]-bar._DA);
