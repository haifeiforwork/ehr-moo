...
/*			backImg.src="mopSlider/paperBack.gif";*/
		...
		for (i=1; i<(itemNum+1); i++){
			...
			var itemW=$(this).children().eq(num).outerWidth();
			nextMov.push(itemW);
		...
		//iKEP.debug(boxW + " = " + allW + " + " + itemMgnAll + " + " + itemMgn);
		/*put mopSlider*/
		...
		$(mopSliderName+" .sliderCaseLeft").css({height:"22px",width:"25px",position:"absolute",top:"0px",left:"0px",backgroundRepeat:"no-repeat"});
		$(mopSliderName+" .sliderCaseRight").css({height:"22px",width:"25px",position:"absolute",top:"0px",cursor:"",backgroundRepeat:"no-repeat"});
		$(mopSliderName+" .slider").css({height:"22px",position:"relative",top:"0px",left:"12px"});
		$(mopSliderName+" .sldLeft").css({left:"0px",position:"absolute",height:"22px",width:"20px",backgroundRepeat:"no-repeat"});
		$(mopSliderName+" .sldCenter").css({left:"20px",width:sldW-40+px,position:"absolute",height:"22px",backgroundRepeat:"repeat-x"});
		$(mopSliderName+" .sldRight").css({right:"0px",position:"absolute",height:"22px",width:"20px",backgroundRepeat:"no-repeat"});
		$(mopSliderName+" .sliderBtn").css({position:"absolute",height:"22px",left:"0px",cursor:"default"});
		$(mopSliderName+" .sldBtnLeft").css({left:"0px",position:"absolute",height:"22px",width:"20px",backgroundRepeat:"no-repeat"});
		$(mopSliderName+" .sldBtnCenter").css({left:"20px",width:btnW-40+px,position:"absolute",height:"22px",backgroundRepeat:"repeat-x"});
		$(mopSliderName+" .sldBtnRight").css({right:"0px",position:"absolute",height:"22px",width:"20px",backgroundRepeat:"no-repeat"});
		...
		$(mopSliderName).css({position:"relative",overflow:"hidden"});
		...
		if(mopSliderType!="tutorialzine")
		{
			...
			$(mopSliderName+" .leftTopIn").css({height:"10px",width:"10px",backgroundRepeat:"no-repeat"});
			$(mopSliderName+" .rightTop").css({height:"10px",width:"10px",position:"absolute",top:"0px",right:"0px"});
			$(mopSliderName+" .rightTopIn").css({height:"10px",width:"10px",backgroundRepeat:"no-repeat"});
			$(mopSliderName+" .leftBottom").css({height:"10px",width:"10px",position:"absolute",bottom:"0px",left:"0px"});
			$(mopSliderName+" .leftBottomIn").css({height:"10px",width:"10px",backgroundRepeat:"no-repeat",backgroundPosition:"bottom"});
			$(mopSliderName+" .rightBottom").css({height:"10px",width:"10px",position:"absolute",bottom:"0px",right:"0px"});
			$(mopSliderName+" .rightBottomIn").css({height:"10px",width:"10px",backgroundRepeat:"no-repeat",backgroundPosition:"bottom"});
		...
		$(mopSliderName+" .holder").children().children(":first").css({marginLeft:parseInt(itemMgn/2, 10)+"px",float:"left",position:"relative"});
		$(mopSliderName+" .holder").children().children(":not(:first)").css({marginLeft:itemMgn+"px",float:"left",position:"relative"});
		...
		var $itemWrap = $(this);
		$(mopSliderName).parent().resize(function(){
			var elem = $(this);
		    var mainWidth = elem.width();
		    var sBoxWidth = $itemWrap.children().width();
			var sBoxCnt = $itemWrap.children().length;
		    var sliderWidth = (sBoxWidth * sBoxCnt) + (itemMgn * (sBoxCnt + 1));
		    
		    if(sliderWidth > mainWidth){
				sliderWidth = mainWidth;
			}
		    var sliderBarWidth = sliderWidth-42;//*0.90;
		    
		    mopSliderW = sliderWidth;
			sldW = sliderBarWidth;
