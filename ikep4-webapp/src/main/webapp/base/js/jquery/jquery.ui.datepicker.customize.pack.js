/*
 * jQuery UI Datepicker v1.8.11 with customize
 * customizer : alan.yu
 * create event
 * added options
 * 	alarmDays : ["2011-04-07", ...]
 * 	hoverWeek : true/false
 * 	hoverRange : number, 0
 * 	hoverStart : number, 1
 */
(function($, undefined) {
	...
	function Datepicker() {
		...
		this._defaults = {
			...
			gotoCurrent : false,
			changeYear : true,//무림제지 변경  false->true
			changeMonth : true,//무림제지 변경  false->true
			yearRange : 'c-10:c+10',
						...
						parseDate : function(format, value, settings) {
							...
							var getName = function(match, shortNames, longNames) {
								var names = (lookAhead(match) ? longNames
										: shortNames);
								for ( var i = 0; i < names.length; i++) {
									if (value.substr(iValue, names[i].length)
											.toLowerCase() == names[i]
											.toLowerCase()) {
						...
						_generateMonthYearHeader : function(inst, drawMonth,
							...
							if (secondary || !changeYear)
								...
								if (!$.browser.mozilla) {
									...
									html += '<select class="ui-datepicker-year"><option value="'
											+ drawYear
											+ '" selected="selected">'
											+ drawYear + '</option></select>'
								}
							}
					...
					Datepicker.prototype,
					{
							...
							for ( var key in inst.settings) {
								...
								case "hoverWeek":
									if (inst.settings.hoverWeek === true) {
										if (isNaN(inst.settings["hoverRange"]))
											inst.settings["hoverRange"] = 0;
													...
													function() {
														...
														$days
																.hover(function(
																	...
																	case inst.settings.hoverRange < 0:
																		var range = Math
																				.abs(inst.settings.hoverRange) - 1;
																		(inst.settings.hoverRange > 0 ? $days
																				: $($
																						.makeArray(
																								$days)
																						.reverse()))
																		...
																		break;
																	default:
																		if (isNaN(inst.settings["hoverStart"])
																				|| (inst.settings.hoverStart < 0 || inst.settings.hoverStart > 6))
																			inst.settings.hoverStart = 1;
																		if (inst.settings.hoverStart == 0)
																			$(
																					this)
																					.parent()
																					.siblings()
																			...
																			var $currRow = $(
																					...
																					.parents(
																							"tr:first");
																			var $currRowItems = $currRow
																					.find("a");
																			var currIndex = $currRowItems
																					.index(this);
																			if (currIndex < inst.settings.hoverStart) {
										...
										if (inst.settings["onSelect"])
											inst.settings["onUseSelect"] = inst.settings["onSelect"];
										inst.settings["onSelect"] = function(
												date, ui) {
											...
											if (inst.settings.hoverRange == 0) {
												...
												currDate
														.setDate(currDate
																.getDate()
																- (currDate
																		.getDay() - inst.settings.hoverStart));
												startDate = datepicker
														._formatDate(inst,
																currDate);
												currDate.setDate(currDate
														.getDate() + 6);
												endDate = datepicker
														._formatDate(inst,
																currDate)
											} else {
												var range = inst.settings.hoverRange > 0 ? inst.settings.hoverRange - 1
														: inst.settings.hoverRange + 1;
												startDate = date;
												endDate = datepicker
														._formatDate(
																inst,
											...
											if (inst.settings.minDate
												...
												startDate = inst.settings.minDate;
											if (inst.settings.maxDate
													&& inst.settings.maxDate < endDate)
												endDate = inst.settings.maxDate;
											var result;
											...
											if (inst.settings["onUseSelect"])
												inst.settings["onUseSelect"](
														result, ui);
											else
												inst.input.val(result);
											return false
										}
									}
									break
						...
						_inlineDatepicker : function(target, inst) {
							...
							for ( var key in inst.settings) {
								switch (key) {
								case "create":
									inst.settings["create"].apply(inst.input);
									break;
								case "alarmDays":
						...
						_setEventChangeMonthYear : function(inst) {
							...
							if (inst.settings["onChangeMonthYear"]) {
								...
								inst.input
										.bind("datepicker:onChangeMonthYear",
												function(event, year, month,
														datepicker) {
													onChangeMonthYear(year,
															month, datepicker)
												})
							}
							inst.settings["onChangeMonthYear"] = function(year,
									month, argDatepicker) {
								setTimeout(function() {
									datepicker._setAlarmDays.apply(datepicker,
											[ inst ]);
									inst.input.trigger(
											"datepicker:onChangeMonthYear",
											[ year, month, argDatepicker ])
								}, 100)
							}
						},
											...
											function() {
												var alarmDate = $.datepicker
														.parseDate(
																...
																$.datepicker
																		._getFormatConfig(inst));
												if (currYear == alarmDate
														.getFullYear()
														&& currMonth == (alarmDate
																.getMonth() + 1)) {
													currDays.push(alarmDate
															.getDate())
												}
											});
							if (currDays.length > 0) {
								var dayItems = $(
										"table.ui-datepicker-calendar > tbody td > a:not(.ui-priority-secondary)",
										inst.input);
								$.each(currDays,
										function() {
											var currDay = parseInt(this
													.toString(), 10);
						...
						_addAlarmDaysDatepicker : function(target, days) {
							...
							switch (typeof (days)) {
							case "string":
								addAlarmDays.push(days);
								break;
							case "object":
								$.merge(addAlarmDays, days);
								break;
							default:
								return false
							}
							$.each(addAlarmDays,
									function() {
										if ($.inArray(this.toString(),
												orgAlarmDays) == -1)
											orgAlarmDays.push(this.toString())
									});
							if (orgLength < orgAlarmDays.length) {
								this._optionDatepicker(target, "alarmDays",
										orgAlarmDays);
								if (orgLength == 0)
									this._setEventChangeMonthYear(inst);
								this._setAlarmDays(inst)
						...
						_removeAlarmDaysDatepicker : function(target, days) {
							...
							switch (typeof (days)) {
							case "string":
								removeAlarmDays.push(days);
								break;
							case "object":
								$.merge(removeAlarmDays, days);
								break;
							default:
								return false
							}
							...
							$.each(orgAlarmDays,
									function() {
										if ($.inArray(this.toString(),
												removeAlarmDays) == -1)
											alarmDays.push(this.toString())
									});
							if (orgLength > alarmDays.length) {
								this._optionDatepicker(target, "alarmDays",
										alarmDays);
								if (alarmDays.length == 0)
									inst.settings.onChangeMonthYear = undefined;
								this._setAlarmDays(inst)
							}
						}
					});
			...
			.ready(
					function() {
						...
						$.datepicker.regional[langDatepicker.langCode] = {
							...
							monthNames : [ langDatepicker.lblMonthNames[0],
									langDatepicker.lblMonthNames[1],
									langDatepicker.lblMonthNames[2],
									langDatepicker.lblMonthNames[3],
									langDatepicker.lblMonthNames[4],
									langDatepicker.lblMonthNames[5],
									langDatepicker.lblMonthNames[6],
									langDatepicker.lblMonthNames[7],
									langDatepicker.lblMonthNames[8],
									langDatepicker.lblMonthNames[9],
									langDatepicker.lblMonthNames[10],
									langDatepicker.lblMonthNames[11] ],
							monthNamesShort : [
									langDatepicker.lblMonthNamesShort[0],
									langDatepicker.lblMonthNamesShort[1],
									langDatepicker.lblMonthNamesShort[2],
									langDatepicker.lblMonthNamesShort[3],
									langDatepicker.lblMonthNamesShort[4],
									langDatepicker.lblMonthNamesShort[5],
									langDatepicker.lblMonthNamesShort[6],
									langDatepicker.lblMonthNamesShort[7],
									langDatepicker.lblMonthNamesShort[8],
									langDatepicker.lblMonthNamesShort[9],
									langDatepicker.lblMonthNamesShort[10],
									langDatepicker.lblMonthNamesShort[11] ],
							dayNames : [ langDatepicker.lblDayNames[0],
									langDatepicker.lblDayNames[1],
									langDatepicker.lblDayNames[2],
									langDatepicker.lblDayNames[3],
									langDatepicker.lblDayNames[4],
									langDatepicker.lblDayNames[5],
									langDatepicker.lblDayNames[6] ],
							dayNamesShort : [
									langDatepicker.lblDayNamesShort[0],
									langDatepicker.lblDayNamesShort[1],
									langDatepicker.lblDayNamesShort[2],
									langDatepicker.lblDayNamesShort[3],
									langDatepicker.lblDayNamesShort[4],
									langDatepicker.lblDayNamesShort[5],
									langDatepicker.lblDayNamesShort[6] ],
							dayNamesMin : [ langDatepicker.lblDayNamesMin[0],
									langDatepicker.lblDayNamesMin[1],
									langDatepicker.lblDayNamesMin[2],
									langDatepicker.lblDayNamesMin[3],
									langDatepicker.lblDayNamesMin[4],
									langDatepicker.lblDayNamesMin[5],
									langDatepicker.lblDayNamesMin[6] ],
							weekHeader : 'Wk',
							dateFormat : 'yy.mm.dd',
							firstDay : 0,
							isRTL : false,
						...
						$.datepicker
								.setDefaults($.datepicker.regional[langDatepicker.langCode])
					})
})(jQuery);