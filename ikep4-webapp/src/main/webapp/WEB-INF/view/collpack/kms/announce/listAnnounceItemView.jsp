		...
		$("#adminMultiDeleteAnnounceItemButton").click(function() {  
			...
			if(countCheckBox>0)
			{
				if(confirm("<ikep4j:message pre="${preMessage}" key="deleteItem" />")) {
					$jq.ajax({
						url : "<c:url value='/collpack/kms/announce/adminMultiDeleteAnnounceItem.do'/>?"+$jq("#searchAnnounceItemForm").serialize(),
						type : "get",
...
<form id="searchAnnounceItemForm" method="get" action="<c:url value='/collpack/kms/announce/listAnnounceItemView.do' />">  
		...
		<div id="listDiv"> 
			...
			<c:choose>
				...
				<div class="blockListTable">
					<%@ include file="/WEB-INF/view/collpack/kms/announce/listTypeAnnounceItemView.jsp"%>
				</c:when>
				<c:when test="${searchCondition.viewMode eq '1'}">
				<div class="blockListTable summaryView"> 	
					<%@ include file="/WEB-INF/view/collpack/kms/announce/summaryTypeAnnounceItemView.jsp"%>
				</c:when>
			</c:choose>
			...
			<ul>
				<c:if test="${announcePermission>1}">
					<li>
						...
						<a id="createAnnounceItemButton" class="button" href="<c:url value='/collpack/kms/announce/createAnnounceItemView.do'/>">
