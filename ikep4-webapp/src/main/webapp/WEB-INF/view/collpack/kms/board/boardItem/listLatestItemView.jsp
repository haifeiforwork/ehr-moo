...
(function($){	
	$(document).ready(function() {    
		...
		sort = function(sortColumn, sortType) {
			$("#searchLatestItemForm input[name=sortColumn]").val(sortColumn); 
			$("#searchLatestItemForm input[name=sortType]").val(sortType == 'ASC' ? 'DESC' : 'ASC');	 
			$("#searchLatestItemButton").click();
		}; 
		//뷰모드 체인지 함수
		changeViewMode = function(changeViewMode) { 
			$("#searchLatestItemForm input[name=viewMode]").val(changeViewMode);
			$("#searchLatestItemButton").click(); 
			return false; 
		}; 
		
		$("#searchLatestItemButton").click(function() {   
			$("#searchLatestItemForm").attr({method:'GET'}).submit(); 
			return false; 
		});
		
		
		$("#checkboxAllLatestItem").click(function() { 
			$("#searchLatestItemForm input[name=boardItemIds]").attr("checked", $(this).is(":checked"));  
		});  

		
		$("#searchLatestItemForm select[name=pagePerRecord]").change(function() {
			//서브밋하는 코드 넣으시면 됩니다.
			//ex : $("#searchLatestItemButton").click(); 
			$("#searchLatestItemButton").click(); 
		}); 
...
<form id="searchLatestItemForm" method="get" action="<c:url value='/collpack/kms/board/boardItem/listLatestItemView.do' />">  
...
<!--pageTitle Start-->
	<div class="tableTop">
		...
		<h2><ikep4j:message pre="${preHeader}" key="latestTitle" /></h2>  
		<div class="listInfo">  
		...
		<div class="tableSearch"> 
			...
			<a id="searchLatestItemButton" name="searchLatestItemButton" href="#a"class="ic_search"><span><ikep4j:message pre='${preButton}' key='search'/></span></a>
		</div>			
		<div class="clear"></div>
	</div>
	<!--//tableTop End--> 
	...
	<div id="container">
		...
		<div class="blockListTable" id="listDiv"> 
			<%@ include file="/WEB-INF/view/collpack/kms/board/boardItem/listTypeLatestItemView.jsp"%>
			
			<!--Page Number Start-->
			<div class="pageNum">	
			<spring:bind path="searchCondition.pageIndex">
				<ikep4j:pagination searchButtonId="searchLatestItemButton" pageIndexInput="${status.expression}" searchCondition="${searchCondition}" /> 
				<input name="${status.expression}" type="hidden" value="${status.value}" title="<ikep4j:message pre='${preSearch}' key='${status.expression}'/>" /> 
			</spring:bind>
			<!--//Page Number End-->
			</div>
