...
<form id="searchUserPermForm" method="post" action="<c:url value='/collpack/kms/admin/listUserPermission.do' />">
                	...
                	<div class="corner_RoundBox03">
						...
						<tbody>
		                     <tr>
		                         ...
		                         <th scope="row" width="5%"><ikep4j:message pre="${preAdminSearch}" key="infoGrade" /></th>
		                         <td width="20%">
		                          	
		                         <spring:bind path="searchCondition.infoGrade">							
		                             <select title="${status.expression}" name="${status.expression}" aa="${status.value}">
		                             	 <option value="">ALL</option>
		                                 <option value="B" <c:if test="${'B' eq status.value}">selected="selected"</c:if>>보안</option>
		                                 <option value="C" <c:if test="${'C' eq status.value}">selected="selected"</c:if>>공유</option>
		                             </select>
		                         </td>
		                         </spring:bind>		                         
						...
						<tr>							
							...
							<th scope="col" width="10%">
								<a onclick="sort('INFO_GRADE', '<c:if test="${searchCondition.sortColumn eq 'INFO_GRADE'}">${searchCondition.sortType}</c:if>');"  href="#a">
									<ikep4j:message pre='${preList}' key='infoGrade' />
								</a>
								<c:choose>
							...
							<c:choose>
							    ...
							    <c:otherwise>
									...
									<tr class="<c:if test="${itemDisplay eq 1}"></c:if>" name="boardItemLine" onmouseover="mouseOver(this);" onmouseout="mouseOut(this);">												
										<td>										
											<a onclick="javascript:viewPopUp('${adminPermission.userId}');" href="#a">${adminPermission.rowNum}</a>
										</td>
										<td>
											<a onclick="javascript:viewPopUp('${adminPermission.userId}');" href="#a">${adminPermission.userName}</a>										
										</td>
										<td>
											<a onclick="javascript:viewPopUp('${adminPermission.userId}');" href="#a">${adminPermission.jobDutyName}</a>
										</td>
										<td>
											<a onclick="javascript:viewPopUp('${adminPermission.userId}');" href="#a">${adminPermission.empNo}</a>
										</td>
										<td>
											<a onclick="javascript:viewPopUp('${adminPermission.userId}');" href="#a">${adminPermission.userId}</a>
										</td>
										<td>
											<a onclick="javascript:viewPopUp('${adminPermission.userId}');" href="#a">${adminPermission.workPlaceName}</a>
										</td>
										<td>
											<a onclick="javascript:viewPopUp('${adminPermission.userId}');" href="#a">${adminPermission.teamName}</a>
										</td>
										<td>
											<a onclick="javascript:viewPopUp('${adminPermission.userId}');" href="#a">
											<c:if test="${adminPermission.infoGrade == 'A'}">
												보안
											</c:if>
											<c:if test="${adminPermission.infoGrade == 'B'}">
												보안
											</c:if>
											<c:if test="${adminPermission.infoGrade == 'C'}">
												공유
											</c:if>
											</a>
										</td>
