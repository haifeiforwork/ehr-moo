				...
				<p id="boardItemTitle"> 
					...
					<span class="rebtn"> 
						<c:if test="${permission.isSystemAdmin or boardItem.registerId eq user.userId}">
							<a class="ic_modify" title="<ikep4j:message pre='${preButton}' key='update'/>" href="<c:url value='/collpack/kms/board/boardItem/updateTemporaryItemView.do?boardId=${boardItem.boardId}&amp;itemId=${boardItem.itemId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}'/>"><span><ikep4j:message pre='${preButton}' key='update'/></span></a>
						</c:if>
						<c:choose> 
						...
						<c:if test="${not portletYn}">
							<c:if test="${not popupYn}">
								...
								<c:if test="${layoutType eq 'layoutNormal'}">
									<li><a class="button" href="<c:url value='/collpack/kms/board/boardItem/listTemporaryItemView.do?boardId=${boardItem.boardId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}&amp;gubun=${gubun}'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span></a></li>
								</c:if> 
							</c:if>
						</c:if>
					</ul> 
