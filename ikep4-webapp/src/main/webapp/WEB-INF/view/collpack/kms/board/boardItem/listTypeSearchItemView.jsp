		    ...
		    <c:otherwise>
				...
				<tr <c:if test="${announceItem.itemDisplay=='1'}">style="background-color:#f9f9f9;"</c:if>>
					...
					<td style="text-align:left;<c:if test="${announceItem.itemDisplay=='1'}">font-weight:bold;</c:if>">
						...
						<span>
						<B>
							<a name="announceItem" href="<c:url value='/collpack/kms/board/boardItem/readSearchItemView.do?itemId=${announceItem.itemId}&isKnowhow=${announceItem.isKnowhow}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}&amp;pageGubun=${pageGubun}'/>" >${announceItem.title}</a>
						</B>
						</span>	
						</span>
						</c:if>
						<c:if test="${announceItem.status != 5}">
							<a name="announceItem" href="<c:url value='/collpack/kms/board/boardItem/readSearchItemView.do?itemId=${announceItem.itemId}&isKnowhow=${announceItem.isKnowhow}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}&amp;pageGubun=${pageGubun}'/>"><span class="font_${announceItem.color}">${announceItem.title}</span></a>
						</c:if>
