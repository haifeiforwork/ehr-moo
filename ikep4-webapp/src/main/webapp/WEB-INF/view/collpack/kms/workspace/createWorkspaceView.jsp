...
(function($) {
	...
	$jq(document).ready(function() { 
		...
		setUser = function(data) {
			if(data=="")			{
				alert('<ikep4j:message  pre='${preSearch}' key='user.nodata'/>');
			} else {
...
<div class="blockDetail">
	...
	<table summary="<ikep4j:message pre="${preDetail}" key="summary" />">
		...
		<tr>	
			...
			<td colspan="3">
			<c:forEach var="workspaceTypeList" items="${workspaceTypeList}" varStatus="status">
			<input class="radio" id="typeId" name="typeId" <c:if test="${status.index==0 }">checked</c:if> type="radio" value="${workspaceTypeList.typeId}" onclick="changeType('${workspaceTypeList.typeId}');" /> ${workspaceTypeList.typeName} 
			</c:forEach>	
			</td>
		</tr>
		...
		<tr>
			<th scope="row"><font	color='#990000'>*</font></span><ikep4j:message pre='${preDetail}' key='workspaceName' /></th>
			<td colspan="3">
			<div>
