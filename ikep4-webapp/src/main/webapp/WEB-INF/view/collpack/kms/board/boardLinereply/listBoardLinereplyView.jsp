...
(function($){	 
	...
	*/	
	createLinereply = function() {    
		...
		$.post('<c:url value="/collpack/kms/board/boardLinereply/createBoardLinereply.do"/>', $("#boardLinereplyForm").serialize())
	...
	*/
	createReplyLinereply = function(linereplyId) {    
		...
		$.post('<c:url value="/collpack/kms/board/boardLinereply/createReplyBoardLinereply.do"/>', formObject.serialize())
		.success(function(data) { 
			loadBoardLinereplyList();
	...
	*/
	updateLinereply = function(linereplyId) { 
		...
		$.post('<c:url value="/collpack/kms/board/boardLinereply/updateBoardLinereply.do"/>', formObject.serialize())
		.success(function(data) { 
			loadBoardLinereplyList();
	...
	*/		
	userDeleteBoardLinereply = function(itemId, linereplyId) { 
		if(confirm("<ikep4j:message pre='${preMessage}' key='delete' />")) {
		    $.post("<c:url value='/collpack/kms/board/boardLinereply/userDeleteBoardLinereply.do'/>", {"itemId" : itemId , "linereplyId" : linereplyId}) 
		    .success(function(data) { 
		    	loadBoardLinereplyList(); 
		    })
		    .error(function(event, request, settings) { alert("error"); });
	...
	*/	
	adminDeleteBoardLinereply = function(itemId, linereplyId) { 
		if(confirm("<ikep4j:message pre='${preMessage}' key='delete' />")) {
		    $.post("<c:url value='/collpack/kms/board/boardLinereply/adminDeleteBoardLinereply.do'/>", {"itemId" : itemId , "linereplyId" : linereplyId}) 
...
//-->
	...
	<c:if test="${board.linereply eq 1 or isSystemAdmin or isKmsAdmin}"> <%-- 답글쓰기 가능 여부 --%>	
		...
		<c:choose>
		    ...
		    <div class="blockCommentInfo">
		    	<span class="blockCommentInfo_name">
		    		<c:choose>
						<c:when test="${board.isKnowhow eq 0 || board.isKnowhow eq 3}">
							<a href="#a" onclick="javascript:viewPopUpProfile('${boardLinereply.registerId}')">${boardLinereply.user.userName} </a>&nbsp;&nbsp;${boardLinereply.user.teamName}
							<img src="<c:url value='/base/images/theme/theme01/basic/bar_pageNum.gif'/>" alt="" />
						</c:when>
						<c:otherwise>
							<c:if test="${isSystemAdmin || isKmsAdmin}">
								<a href="#a" onclick="javascript:viewPopUpProfile('${boardLinereply.registerId}')">${boardLinereply.user.userName} </a>&nbsp;&nbsp;${boardLinereply.user.teamName}
								<img src="<c:url value='/base/images/theme/theme01/basic/bar_pageNum.gif'/>" alt="" />
							</c:if>
						</c:otherwise>
		    	...
		    	<span class="rebtn">
					<c:choose>
						<c:when test="${isSystemAdmin || isKmsAdmin}"> <%-- 관리자, 읽기 게시판이라면 --> 링크허용  --%>
					...
					<c:if test="${boardLinereply.indentation lt 2 and (board.linereply eq 1 or isSystemAdmin or isKmsAdmin)}">
