...
(function($){	
	$(document).ready(function() {    
		...
		sort = function(sortColumn, sortType) {
			$("#searchExcellenceItemForm input[name=sortColumn]").val(sortColumn); 
			$("#searchExcellenceItemForm input[name=sortType]").val(sortType == 'ASC' ? 'DESC' : 'ASC');	 
			$("#searchExcellenceItemButton").click();
		}; 
		//뷰모드 체인지 함수
		changeViewMode = function(changeViewMode) { 
			$("#searchExcellenceItemForm input[name=viewMode]").val(changeViewMode);
			$("#searchExcellenceItemButton").click(); 
			return false; 
		}; 
		
		$("#searchExcellenceItemButton").click(function() {   
			$("#searchExcellenceItemForm").attr({method:'GET'}).submit(); 
			return false; 
		});
		
		
		$("#checkboxAllExcellenceItem").click(function() { 
			$("#searchExcellenceItemForm input[name=boardItemIds]").attr("checked", $(this).is(":checked"));  
		});  

		
		$("#searchExcellenceItemForm select[name=pagePerRecord]").change(function() {
			//서브밋하는 코드 넣으시면 됩니다.
			//ex : $("#searchExcellenceItemButton").click(); 
			$("#searchExcellenceItemButton").click(); 
		}); 
...
<form id="searchExcellenceItemForm" method="get" action="<c:url value='/collpack/kms/board/boardItem/listExcellenceItemView.do' />">  
...
<!--mainContents Start-->
	...
	<h1 class="none"><ikep4j:message pre="${preHeader}" key="excellencePageTitle1" /></h1> 
<!--pageTitle Start-->
	<div class="tableTop">
		<div class="tableTop_bgR"></div>
		<h2><ikep4j:message pre="${preHeader}" key="excellenceTitle" /></h2>  
		<div class="listInfo">  
		...
		<div class="tableSearch"> 
			...
			<a id="searchExcellenceItemButton" name="searchExcellenceItemButton" href="#a"class="ic_search"><span><ikep4j:message pre='${preButton}' key='search'/></span></a>
		</div>			
		<div class="clear"></div>
	</div>
	<!--//tableTop End--> 
	...
	<div id="container">
		...
		<div class="blockListTable" id="listDiv"> 
			<%@ include file="/WEB-INF/view/collpack/kms/board/boardItem/listTypeExcellenceItemView.jsp"%>
			
			<!--Page Number Start-->
			<div class="pageNum">	
			<spring:bind path="searchCondition.pageIndex">
				<ikep4j:pagination searchButtonId="searchExcellenceItemButton" pageIndexInput="${status.expression}" searchCondition="${searchCondition}" /> 
				<input name="${status.expression}" type="hidden" value="${status.value}" title="<ikep4j:message pre='${preSearch}' key='${status.expression}'/>" /> 
			</spring:bind>
			<!--//Page Number End-->
			</div>
