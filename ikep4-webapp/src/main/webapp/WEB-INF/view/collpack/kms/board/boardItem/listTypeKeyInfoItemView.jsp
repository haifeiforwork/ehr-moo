		...
		<c:choose>
		    ...
		    <c:otherwise>
				...
				<tr>
					...
					<td>
						<c:if test="${announceItem.status == '0'}">
							임시저장
						</c:if>
						<c:if test="${announceItem.status == '1' || announceItem.status == '2'}">
							등록완료
						</c:if>
						<c:if test="${(announceItem.status == '3' || announceItem.status == '5') && announceItem.infoGrade != 'E'}">
							채택완료
						</c:if>
						<c:if test="${(announceItem.status == '3' || announceItem.status == '5') && announceItem.infoGrade == 'E'}">
							미공유지식
					...
					<td class="textLeft">
						...
						<span>
							...
							<c:if test="${gubun eq '2'}">
								<a name="announceItem" href="<c:url value='/collpack/kms/board/boardItem/readKeyInfoItemView.do?itemId=${announceItem.itemId}&gubun=${gubun}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}'/>">${announceItem.title}</a>
							</c:if>
					...
					<c:if test="${gubun eq '2'}">
						<td>
							<c:if test='${announceItem.infoGrade == "C"}'>공유</c:if>
							<c:if test='${announceItem.infoGrade == "A"}'>보안</c:if>
							<c:if test='${announceItem.infoGrade == "B"}'>보안</c:if>
						</td>
					</c:if>
