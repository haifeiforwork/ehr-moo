							...
							<c:if test="${not popupYn}">
								...
								<c:if test="${layoutType eq 'layoutNormal'}">
									<c:choose>
								       ...
								       <c:otherwise>
								           <li><a class="button" href="<c:url value='/collpack/kms/board/boardItem/listBoardItemView.do?isKnowhow=${boardItem.isKnowhow}&amp;boardId=${boardItem.boardId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span></a></li>
								       </c:otherwise>
									</c:choose>								
								</c:if> 
							</c:if>
