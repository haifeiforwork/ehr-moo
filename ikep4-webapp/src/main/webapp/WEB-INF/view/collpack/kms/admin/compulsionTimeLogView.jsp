...
(function($){	 
	...
	$(document).ready(function() {   
		...
		$("#excelButton").click(function() { 
			...
			if(excelBtnCnt == 0){
				$jq("#searchUserPermForm").attr("action","<c:url value='/collpack/kms/admin/compulsionTimeLogExceldown.do'/>");
				$jq("#searchUserPermForm").submit();
				$jq("#searchUserPermForm").attr("action","<c:url value='/collpack/kms/admin/compulsionTimeLogView.do'/>");
				excelBtnCnt++;
			}else{
...
<form id="searchUserPermForm" method="post" action="<c:url value='/collpack/kms/admin/compulsionTimeLogView.do' />">
                	...
                	<div class="corner_RoundBox03">
						...
						<tbody>
		                     <tr>
		                         ...
		                         <spring:bind path="searchCondition.clickFlg">
		                             <select title="${status.expression}" name="${status.expression}">
										<option value="" <c:if test="${'' eq status.value}">selected="selected"</c:if>>전체</option>
										<option value="1" <c:if test="${'1' eq status.value}">selected="selected"</c:if>>시작</option>
										<option value="2" <c:if test="${'2' eq status.value}">selected="selected"</c:if>>종료</option>
		                             </select>
		                         </spring:bind>
							...
							<c:choose>
							    ...
							    <c:otherwise>
									...
									<tr class="<c:if test="${itemDisplay eq 1}"></c:if>" name="boardItemLine" onmouseover="mouseOver(this);" onmouseout="mouseOut(this);">												
										...
										<td>
											${adminPermission.clickTime}
										</td>
										<td>
											${adminPermission.clickFlg}
										</td>
									</tr>
									</c:forEach>				      
							    </c:otherwise> 
