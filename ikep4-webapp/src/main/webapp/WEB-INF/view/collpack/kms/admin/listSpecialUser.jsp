	...
	$(document).ready(function() {   
		...
		$("#deleteWinnerButton").click(function() {  
			...
			$("#searchUserPermForm input[name=checkTarget]:checked").each(function(index) { 
				str = $(this).val().split("_");
				itemIds.push(str[0]); 
				categoryIds.push(str[1]); 
			});	
			...
			if (itemIds.length > 0 ){	
				if(confirm("<ikep4j:message pre="message.collpack.kms.admin.winner.message" key="delete" />")) {
					...
					$.post("<c:url value='/collpack/kms/admin/deleteSpecialUser.do'/>", {"userIds" : itemIds.toString(),"categoryIds" : categoryIds.toString()}) 
...
<!--pageTitle Start-->
				...
				<div id="container">
					...
					<table summary="<ikep4j:message pre='${preList}' key='summary' />">
						...
						<tbody>
							<c:choose>
							    <c:when test="${empty specialUserList}">
									<tr>
										<td colspan="6" class="emptyRecord"><ikep4j:message pre='${preSearch}' key='emptyRecord' /></td> 
									</tr>				        
							    </c:when>
							    <c:otherwise>
									<c:forEach var="specialUser" items="${specialUserList}">
									<tr  name="boardItemLine" >
										<td>
											<input name="checkTarget" class="checkbox" title="checkbox" type="checkbox" value="${specialUser.userId}_${specialUser.categoryId}" />
										</td>
										<td>
											<a href="<c:url value='/collpack/kms/admin/modifySpecialUserForm.do'/>?userId=${specialUser.userId}&categoryId=${specialUser.categoryId}" >${specialUser.userName}</a>
										</td>
										<td>
											<a href="<c:url value='/collpack/kms/admin/modifySpecialUserForm.do'/>?userId=${specialUser.userId}&categoryId=${specialUser.categoryId}" >${specialUser.jobDutyName}</a>
										</td>
										<td>
											<a href="<c:url value='/collpack/kms/admin/modifySpecialUserForm.do'/>?userId=${specialUser.userId}&categoryId=${specialUser.categoryId}" >${specialUser.teamName}</a>
										</td>
										<td>
											<span class="${specialUser.color} cal_color_box_sample"></span>
										</td>
										<td>
											<c:if test="${specialUser.categoryId=='1'}">조회목록표시</c:if>
											<c:if test="${specialUser.categoryId=='2'}">우수지식표시</c:if>
										</td>
									</tr>
									</c:forEach>				      
							    </c:otherwise> 
		...
		<!--blockButton Start-->
         <div class="blockButton"> 
             <ul>
                 <li><a class="button" id="applyWinnerButton" href="<c:url value='/collpack/kms/admin/registSpecialUserForm.do'/>"><span><ikep4j:message pre="message.collpack.kms.admin.winner.button" key="apply"/></span></a></li>
