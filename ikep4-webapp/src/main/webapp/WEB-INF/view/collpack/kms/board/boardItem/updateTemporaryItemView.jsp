...
(function($){	 
	$(document).ready(function() { 
		...
		<c:if test="${boardItem.isKnowhow eq '0'}">
			$('#expertDiv').show();
		</c:if>
...
<div id="pageTitle" class="btnline"> 
	...
	<div class="blockButton"> 
		<ul>
			...
			<li><a class="button listBoardItemButton" href="<c:url value='/collpack/kms/board/boardItem/readTemporaryItemView.do?boardId=${boardItem.boardId}&amp;itemId=${boardItem.itemId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}&amp;gubun=${gubun}'/>"><span><ikep4j:message pre='${preButton}' key='cancel'/></span></a></li>
			<c:if test="${not popupYn}">	
				<li><a class="button listBoardItemButton" href="<c:url value='/collpack/kms/board/boardItem/listTemporaryItemView.do?boardId=${boardItem.boardId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}&amp;gubun=${gubun}'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span></a></li>
			</c:if> 
		</ul>
	</div>	
</div> 
...
<form id="boardItemForm" method="post" action="<c:url value='/collpack/kms/board/boardItem/updateTemporaryItem.do'/>">
	...
	<table summary="<ikep4j:message pre="${preDetail}" key="summary" />">
		...
		<tr> 
			...
			<td>
				<input name="reportReqCk" id="reportReqCk" type="checkbox" class="checkbox" <c:if test="${boardItem.reportReq=='1'}">checked="checked"</c:if> />
			</td>
		</tr>				
		<tr>  
        ...
        <tr>
			...
			<td>
				<label>
				<input name="isKnowhow1" type="radio" class="radio" value="1" title="지식분류" <c:if test="${boardItem.isKnowhow eq '1'}">checked="checked"</c:if> onclick="javascript:isKnowChk('1');"/> 
				일반정보
				</label>
			 
				<label>
				<input name="isKnowhow1" type="radio" class="radio" value="0" title="지식분류" <c:if test="${boardItem.isKnowhow eq '0'}">checked="checked"</c:if> onclick="javascript:isKnowChk('2');"/> 
				업무노하우
				</label>
				<c:if test="${isSystemAdmin}">
				<input name="isKnowhow1" type="radio" class="radio" value="3" title="지식분류" <c:if test="${boardItem.isKnowhow eq '3'}">checked="checked"</c:if> onclick="javascript:isKnowChk('3');" /> 
				원문 게시판
				</label>
				</c:if>
			</td>
		...
		<tr id="expertDiv" style="display:none;">
			<th scope="row">전문가</th>
			<td colspan="3">
				<input id="assessor" name="assessor" title="전문가" class="inputbox w20" type="text" value="${boardItem.assessorName}" readonly/>
            	<input id="assessorId" name="assessorId" value="${boardItem.assessorId}" type="hidden"/>
