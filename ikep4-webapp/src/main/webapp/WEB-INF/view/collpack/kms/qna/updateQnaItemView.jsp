	...
	writeSubmit = function(targetForm){
		...
		if(useActXEditor == "Y"){
	    	if ($.browser.msie) {
	    		...
	    		//에디터 내 이미지 파일 링크 정보 세팅
	    		createActiveXEditorFileLink(tweBodyObj,"qnaItemForm");
...
<div class="blockDetail">
	<form id="qnaItemForm" name="qnaItemForm" method="post" action="<c:url value='/collpack/kms/qna/updateQnaItem.do'/>">
	...
	<spring:bind path="qnaItem.itemDisplay">
	...
	<table summary="게시판등록">
		...
		<%--tr>  
			...
			<td colspan="3">
				...
				type="text" 
				class="inputbox w40" 
				value="<c:forEach var="tag" items="${qnaItem.tagList}" varStatus="tagLoop"><c:if test="${tagLoop.count != 1}">, </c:if>${tag.tagName}</c:forEach>" 
				size="40" 
				title="<ikep4j:message pre='${preDetail}' key='${status.expression}' />" >
...
<div class="tableBottom">										
	...
	<div class="blockButton"> 
		<ul>
			...
			<li><a id="listBoardItemButton" class="button" href="<c:url value='/collpack/kms/qna/listQnaItemView.do'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span></a></li>
		 </ul>
	</div>
	<!--//blockButton End--> 
</div>
