						...
						<c:if test="${not portletYn}">
							<c:if test="${not popupYn}">
								...
								<c:if test="${layoutType eq 'layoutNormal'}">
									<li><a class="button" href="<c:url value='/collpack/kms/board/boardItem/listLatestItemView.do?boardId=${boardItem.boardId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}&amp;isKnowhow=${isKnowhow}'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span></a></li>
								</c:if> 
							</c:if>
						</c:if>
					</ul> 
