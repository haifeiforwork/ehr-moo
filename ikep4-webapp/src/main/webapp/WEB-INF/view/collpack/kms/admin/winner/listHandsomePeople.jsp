...
(function($){	 
	...
	$(document).ready(function() {   
		...
		viewPopUp = function(itemSeq) { 
			var pageURL = "<c:url value='/collpack/kms/admin/winner/getWinnerPeoplePopup.do' />?itemSeq=" + itemSeq;
		...
		$("#deleteWinnerButton").click(function() {  
			...
			if (itemIds.length > 0 ){	
				if(confirm("<ikep4j:message pre="message.collpack.kms.admin.winner.message" key="delete" />")) {
					...
					$.post("<c:url value='/collpack/kms/admin/winner/deleteWinnerPeople.do'/>", {"itemSeqs" : itemIds.toString()}) 
		...
		$("#excelWinnerButton").click(function() {
			...
			$jq("#searchUserPermForm").attr("action","<c:url value='/collpack/kms/admin/winner/downloadExcelWinnerPeople.do'/>");
...
<form id="searchUserPermForm" method="post" action="<c:url value='/collpack/kms/admin/winner/listHandsomePeople.do' />">
...
<!--pageTitle Start-->
		...
		<!--blockButton Start-->
         <div class="blockButton"> 
             <ul>
                 <li><a class="button" id="applyWinnerButton" href="<c:url value='/collpack/kms/admin/winner/registerWinnerPeople.do'/>"><span><ikep4j:message pre="message.collpack.kms.admin.winner.button" key="apply"/></span></a></li>
