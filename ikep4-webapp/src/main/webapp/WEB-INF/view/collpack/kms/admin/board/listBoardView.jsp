	...
	$(document).ready(function() { 
		...
		$("#adminBoardAdminTree").bind("loaded.jstree", function (event, datet) {
		    ...
		    "json_data" : {  
		    	 "data" : [{  
		    		        "data" : "Category", 
		    				"state" : "closed",
		    	 			"attr" : {"id" : "root", "dummy" : true, "boardId" : "", "boardRootId" : "1", "isKnowHow" : "${isKnowHow}"}
		    			  }], 
		    	"ajax" : {
		    		"url" : "<c:url value='/collpack/kms/admin/board/listChildrenBoard.do'/>",
		    		"data" : function (node) {
		    			//iKEP.debug(node);
		    			return { boardId :  $(node).attr("boardId") ?  $(node).attr("boardId") : "",isKnowHow:isKnowHow};
		    ...
		    "contextmenu" : {  
				items : function($item){
					var menuItem;
					if(isKnowHow == '2'){	//지식공유사이트 맵관리
						menuItem = {													
								"1-createLink" : { label : "<ikep4j:message pre='${preDetail}' key='createLink'/>",  action : function(item) { 
									location.href = "<c:url value='/collpack/kms/admin/board/createBoardView.do?isKnowHow="+ isKnowHow +"&boardRootId=0'/>" + 
									"&boardParentId=" + $(item).attr("boardId") +
									"&boardType=2";
								}},								
								"2-rename" : { label : "<ikep4j:message pre='${preDetail}' key='rename'/>",  action : function(item) {  
									this.rename(item); 
								}},
								"3-remove" : { label : "<ikep4j:message pre='${preDetail}' key='delete'/>",  action : function(item) {   
									var tree = this;  
									 
									if(confirm("<ikep4j:message pre='${preMessage}' key='delete'/>")) {
										$.get("<c:url value='/collpack/kms/admin/board/deleteBoardAjax.do?isKnowHow="+ isKnowHow +"&boardId='/>" + $(item).attr("boardId"))
						...
						menuItem = {
								...
								"4-update" : { label : "<ikep4j:message pre='${preDetail}' key='update'/>",  action : function(item) { 
									location.href = "<c:url value='/collpack/kms/admin/board/updateBoardView.do?isKnowHow="+ isKnowHow +"&boardId='/>" + $(item).attr("boardId") +
			...
			if(confirm("<ikep4j:message pre="${preMessage}" key="move" />")) { 
				$.post("<c:url value='/collpack/kms/admin/board/updateMoveBoard.do'/>",  {"isKnowHow":isKnowHow,"boardId" : boardId, "boardParentId" : boardParentId, "sortOrder" : sortOrder})
				.success(function(data) {
			...
			if(confirm("<ikep4j:message pre="${preMessage}" key="rename" />")) { 
				$.post("<c:url value='/collpack/kms/admin/board/updateBoardName.do'/>",  {"isKnowHow":isKnowHow,"boardId" : boardId, "boardName" : boardName})
				.success(function(data) {
	    ...
	    $("#adminBoardAdminTree").delegate("a", "click", function () {
	    	if($(this).parent().attr("boardId")!="")
				location.href = "<c:url value='/collpack/kms/admin/board/readBoardView.do?isKnowHow="+isKnowHow +"&boardId='/>" + $(this).parent().attr("boardId"); 
	     }); 
