...
(function($){	
	...
	$(document).ready(function() {   
		...
		$("#excelButton").click(function() { 
			...
			if(excelBtnCnt == 0){
				$jq("#searchForm").attr("action","<c:url value='/collpack/kms/perform/downloadExcelPrivateStat.do'/>");
		...
		$("#notiButton").click(function() { 
			...
			if(confirm("<ikep4j:message key='message.collpack.kms.perform.messenger.alert'/>")) {
				...
				$.ajax({
				  type: "POST",
				  url: "<c:url value='/collpack/kms/perform/notiPrivateStat.do'/>", 
				  
				  data: params,
				  success: function(data) {
					  $("#searchForm").ajaxLoadComplete();
					alert("전송이 완료되었습니다.");
				  },
				  error: function(data) {
					  $("#searchForm").ajaxLoadComplete();
...
<input name="adminUserName" type="hidden" value="${user.userName}"/>
<!--mainContents Start-->	 
<!--pageTitle Start-->
                <!--tableTop Start-->
				<div class="tableTop">
						...
						<tbody>
							<tr>
                               ...
                               <c:choose>
                               		...
                               		<c:otherwise>
                               			<input type="text" class="inputbox" title="" name="registerName" value="${user.userName}" size="10" readonly />
                               		</c:otherwise>
                               	</c:choose>
                               </td>
                               ...
                               <a href="#a" id="notiButton"><img src="<c:url value='/base/images/theme/theme01/basic/btn_msg_118.gif'/>" alt="" /></a>
                               </td>				
                           </tr>														
		                 </tbody>
		             	</table>
					...
					<table summary="">
						...
						<tr>
							...
							<th scope="col" width="6%">								
								<a onclick="sort('USER_NAME', '<c:if test="${searchCondition.sortColumn eq 'REGISTER_NAME'}">${searchCondition.sortType}</c:if>');"  href="#a">
							...
							<th scope="col" width="7%">
								<ikep4j:message pre='${prePrivate}' key='list.workPlaceName' />	
							</th>
							<th scope="col" width="*">								
							...
							<th scope="col" width="6%">
								<ikep4j:message pre='${prePrivate}' key='list.jobTitleName' />
							</th>	
							<th scope="col" width="7%">								
								<a onclick="sort('USER_STATUS', '<c:if test="${searchCondition.sortColumn eq 'USER_STATUS'}">${searchCondition.sortType}</c:if>');"  href="#a">
									<ikep4j:message pre='${prePrivate}' key='list.userStatus' />
								</a>
								<c:choose>
									<c:when test="${searchCondition.sortColumn eq 'USER_STATUS' and searchCondition.sortType eq 'DESC'}"><img alt="<ikep4j:message pre="${preSearch}" key="sortType" post="desc" />" src="<c:url value='/base/images/icon/ic_tablesort_down.gif'/>"></c:when>
									<c:when test="${searchCondition.sortColumn eq 'USER_STATUS' and searchCondition.sortType eq 'ASC'}"><img alt="<ikep4j:message pre="${preSearch}" key="sortType" post="asc" />" src="<c:url value='/base/images/icon/ic_tablesort_up.gif'/>"></c:when>
								</c:choose>
							</th>	
						</tr>
						</thead>
						<tbody>
							<c:choose>
							    <c:when test="${searchResult.emptyRecord}">
									<tr>
										<td colspan="15" class="emptyRecord"><ikep4j:message key='ui.ikep4.common.searchCondition.emptyRecord' /></td> 
									</tr>				        
							    </c:when>
							    <c:otherwise>
									...
									<tr class="<c:if test="${itemDisplay eq 1}"></c:if>" name="boardItemLine">
										...
										<td>
											<c:if test="${empty performance.jobDutyName}">
												${performance.jobTitleName}
											</c:if>
											<c:if test="${!empty performance.jobDutyName}">
												${performance.jobDutyName}
											</c:if>
										</td>	
										<td>
