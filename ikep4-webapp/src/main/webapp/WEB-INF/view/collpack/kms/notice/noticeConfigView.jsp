...
$jq(document).ready(function() {
	...
	$jq("#managermessageConfigForm${portletConfigId}").submit(function(){
		var options = {
			...
			url:'<c:url value="/collpack/kms/notice/createNoticeConfig.do"/>',
			type:'post',
			dataType : "html",
...
<h4 class="guidetitle_han">지식Notice설정</h4>
<!--blockDetail Start-->
	...
	<table summary="<ikep4j:message pre="${prefix}" key="summary" />" >
		...
		<tbody>
			<tr>
				...
				<td>
					<input name="subject" title="<ikep4j:message pre="${prefix}" key="write.subject" />" class="inputbox w50" type="text" value="${noticeconfig.subject}"/>
				</td>
			</tr>
			<tr>
				<th scope="row"><span class="colorPoint">*</span><ikep4j:message pre="${prefix}" key="write.summary" /></th>
				<td>
					<textarea name="summary" title="<ikep4j:message pre="${prefix}" key="write.summary" />" class="inputbox w90" rows="5" >${noticeconfig.summary}</textarea>
				</td>
			</tr>
			<%-- <tr>
                <th scope="row"><span class="colorPoint">*</span><ikep4j:message pre="${prefix}" key="boardid" /></th>
                <td>
                    <input name="boardId" title="<ikep4j:message pre="${prefix}" key="boardid" />" class="inputbox w90" type="text" value="${noticeconfig.boardId}"/>
                </td>
            </tr> --%>
			<tr>
				<th scope="row"><span class="colorPoint">*</span><ikep4j:message pre="${prefix}" key="itemid" /></th>
				<td>
					<input name="itemId" title="<ikep4j:message pre="${prefix}" key="itemid" />" class="inputbox w20" type="text" value="${noticeconfig.itemId}"/>
				</td>
			</tr>
			<!-- tr>
				<th scope="row"><ikep4j:message pre="${prefix}" key="target" /></th>
				<td>
					<input type="radio" name="target" class="radio" value="WINDOW" <c:if test="${noticeconfig.target == 'WINDOW'}">checked="checked"</c:if> /><ikep4j:message pre="${prefix}" key="target.window" />
				</td>
			</tr-->
			...
			<%-- <tr>
				...
				<td>
					<input type="radio" name="location" class="radio" value="left" <c:if test="${noticeconfig.location == 'left' or noticeconfig.location == null }">checked="checked"</c:if> /><ikep4j:message pre="${prefix}" key="location.left" />
					<input type="radio" name="location" class="radio" value="right" <c:if test="${noticeconfig.location == 'right'}">checked="checked"</c:if> /><ikep4j:message pre="${prefix}" key="location.right" />
			...
			<!-- tr>
				...
				<td>
					<ikep4j:message pre="${prefix}" key="img.width" /><input type="text" name="imgWidth" size="5" maxlength="3" class="radio" value="${noticeconfig.imgWidth}" />px
					<ikep4j:message pre="${prefix}" key="img.height" /><input type="text" name="imgHeight" size="5" maxlength="3" class="radio" value="${noticeconfig.imgHeight}" />px
				</td>
			</tr-->			
		</tbody>
	</table>
