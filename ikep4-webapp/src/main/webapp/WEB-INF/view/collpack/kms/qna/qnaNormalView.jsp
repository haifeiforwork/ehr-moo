...
<div class="subTitle_1a">
		...
		<div class="btn_more"><a href='<c:url value='/collpack/kms/qna/listQnaItemView.do' />'><img src='<c:url value='/base/images/common/btn_more.gif'/>' alt='more' /></a></div>
	</div>
...
<div>
	...
	<c:set var="img_td">
		...
		<c:if test="${campaignmessagesImageYn == 'Y'}">
			<img src="<c:url value='/support/fileupload/downloadFile.do'/>?fileId=${qnaconfig.imageFileId}" width="100" height="100"/>
		</c:if>
		<c:if test="${campaignmessagesImageYn != 'Y'}">
	...
	<c:set var="text_td">
		<td valign="top">
			...
			<div class="ellipsis" style="margin-top: 2px; max-height:52px; white-space:normal; font: 100%/1.5em '맑은 고딕', Tahoma, 'Dotum'; BACKGROUND: #fff;">
				<a class="item" href="<c:url value='/collpack/kms/qna/readQnaItemView.do?itemId=${qnaconfig.itemId}'/>">
					${qnaconfig.summary}
				</a>
			</div>
		</td>
	...
	<table width="100%" class="ex_me" style="table-layout: fixed;">	
		<tbody>
			<tr>
				<%-- <c:if test="${qnaconfig.location eq 'right'}">
					${text_td}
					${img_td}
				</c:if>
				<c:if test="${qnaconfig.location eq 'left'}"> --%>
					${img_td}
					${text_td}
				<%-- </c:if> --%>
			</tr>
