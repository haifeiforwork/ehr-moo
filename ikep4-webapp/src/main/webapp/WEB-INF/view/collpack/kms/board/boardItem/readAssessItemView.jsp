...
(function($){	 
	...
	moveBoardItem = function(result) {
		...
		.success(function(data) {
			location.href="<c:url value='/collpack/kms/board/boardItem/listBoardItemView.do?isKnowhow=${boardItem.isKnowhow}&boardId=${boardItem.boardId}'/>";
		})
		.error(function(event, request, settings) { alert("error"); }); 		
	}; 
	bodyTop = function() {
	...
	$(document).ready(function() { 
		...
		$("#userDeleteBoardItemButton,#userDeleteBoardItemButton1").click(function() { 
			if(confirm("<ikep4j:message pre="${preMessage}" key="deleteItem" />")) {
				...
				location.href="<c:url value='/collpack/kms/board/boardItem/userDeleteBoardItem.do?boardId=${boardItem.boardId}&itemId=${boardItem.itemId}&searchConditionString=${searchConditionString}&popupYn=${popupYn}&isKnowhow=${boardItem.isKnowhow}'/>";  
			} 
		...
		$("#adminDeleteBoardItemButton,#adminDeleteBoardItemButton1").click(function() {
			if(confirm("<ikep4j:message pre="${preMessage}" key="deleteItem" />")) {
				$("#tagResult").ajaxLoadStart(); 
				location.href="<c:url value='/collpack/kms/board/boardItem/adminDeleteBoardItem.do?boardId=${boardItem.boardId}&itemId=${boardItem.itemId}&searchConditionString=${searchConditionString}&popupYn=${popupYn}&isKnowhow=${boardItem.isKnowhow}'/>&poolIdx=" + iKEP.getUrlArguments("poolIdx"); 
			}  
		...
		$("#updateBoardItemButton").click(function() {
			if(confirm("수정 하시겠습니까?")) {
				if(${permission.isSystemAdmin || user.userId eq boardItem.expertId}){
			...
			<div class="tableBottom" style="margin-top: 6px;"> 
				...
				<div class="blockButton"> 
					<ul>
						...
						<c:if test="${not portletYn}">
							<c:if test="${not popupYn}">
								<c:if test="${permission.isSystemAdmin or (boardItem.registerId eq user.userId and boardItem.status < 3) or user.userId eq boardItem.expertId}">
								...
								<c:if test="${layoutType eq 'layoutNormal'}">
									<c:choose>
								       <c:when test="${pageGubun eq 'listPrivate'}">
								           <li><a class="button" href="<c:url value='/collpack/kms/perform/listPrivate.do?searchConditionString=${searchConditionString}'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span></a></li>
								       </c:when>
								       <c:otherwise>
								           <li><a class="button" href="<c:url value='/collpack/kms/board/boardItem/listAssessItemView.do?boardId=${boardItem.boardId}&searchConditionString=${searchConditionString}&popupYn=${popupYn}'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span></a></li>
								       </c:otherwise>
									</c:choose>								
								</c:if> 
							</c:if>
