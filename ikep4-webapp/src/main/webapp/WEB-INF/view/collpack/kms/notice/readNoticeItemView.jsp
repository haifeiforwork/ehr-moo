...
<c:forEach var="tempEcmFileData" items="${noticeItem.ecmFileDataList}" varStatus="ecmFileDataStatus">
...
(function($){
	$(document).ready(function() { 
		...
		$("#adminShareCollNoticeItemButton").click(function() {  
			iKEP.showDialog({
				title : "<ikep4j:message pre="${preMessage}" key="shareSubTeam" />",
				width:800,
				height:400,
				url: "<c:url value='/collpack/kms/notice/shareNoticeItemPop.do?itemId=${noticeItem.itemId}'/>",
				modal:true,
				callback : function() {
					location.replace("<c:url value='/collpack/kms/notice/listNoticeItemView.do'/>");
				}
			});	
		});
		...
		$("#adminDeleteBoardItemButton").click(function() {
			if(confirm("<ikep4j:message pre="${preMessage}" key="deleteItem" />")) {
				location.href="<c:url value='/collpack/kms/notice/adminDeleteNoticeItem.do?itemId=${noticeItem.itemId}'/>"; 
			}  
			return false; 
...
<form id="noticeItemForm" name="noticeItemForm" action="">
<h1 class="none"></h1>
...
<div class="blockTableReadWrap">
				...
				<div class="summaryViewInfo"> 
					<span class="blockCommentInfo_name">
						...
						<c:when test="${user.localeCode == portal.defaultLocaleCode}">
							<a href="#a" onclick="iKEP.showUserContextMenu(this, '${noticeItem.registerId}', 'bottom')">${noticeItem.registerName}&nbsp;${noticeItem.jobTitleName}</a>
						</c:when>
						<c:otherwise>
							<a href="#a" onclick="iKEP.showUserContextMenu(this, '${noticeItem.registerId}', 'bottom')">${noticeItem.registerEnglishName}&nbsp;${noticeItem.jobTitleEnglishName}</a>
						</c:otherwise>
						</c:choose>	
				    </span>
					<img src="<c:url value='/base/images/theme/theme01/basic/bar_pageNum.gif'/>" alt="" />
					<spring:bind path="noticeItem.hitCount">
		...
		<div class="tableBottom">										
			...
			<div class="blockButton"> 
				<ul>
					<c:if test="${noticePermission>1 || noticeItem.registerId eq user.userId}">
						<c:if test="${isOrganization eq 1 && noticeItem.isOwner eq 1}">
							<li>
								<a class="button" id="adminShareCollNoticeItemButton"  href="#a"><span><ikep4j:message pre='${preButton}' key='shareColl'/></span></a>
							</li>
						</c:if>
							<li>
								<a class="button" href="<c:url value='/collpack/kms/notice/updateNoticeItemView.do?itemId=${noticeItem.itemId}'/>"><span><ikep4j:message pre='${preButton}' key='update'/></span></a>
							</li>
						...
						<li>
							<a class="button" href="<c:url value='/collpack/kms/notice/listNoticeItemView.do'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span></a>
						</li>
				</ul>  
			</div>
			<!--//blockButton End-->
