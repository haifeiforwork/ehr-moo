...
(function($){	 
	...
	$(document).ready(function() {   
		...
		viewPopUp = function(itemSeq) { 
			var pageURL = "<c:url value='/collpack/kms/admin/winner/getWinnerTeamPopup.do' />?itemSeq=" + itemSeq;
		...
		$("#deleteWinnerButton").click(function() {  
			...
			if (itemIds.length > 0 ){	
				if(confirm("<ikep4j:message pre="message.collpack.kms.admin.winner.message" key="delete" />")) {
					...
					$.post("<c:url value='/collpack/kms/admin/winner/deleteWinnerTeam.do'/>", {"itemSeqs" : itemIds.toString()}) 
		...
		$("#searchUserPermButton").click(function() { 
			$jq("#searchUserPermForm").attr("action","<c:url value='/collpack/kms/admin/winner/listHandsomeTeam.do'/>");
		...
		$("#excelWinnerButton").click(function() {
			...
			$jq("#searchUserPermForm").attr("action","<c:url value='/collpack/kms/admin/winner/downloadExcelWinnerTeam.do'/>");
...
<form id="searchUserPermForm" method="post" action="<c:url value='/collpack/kms/admin/winner/listHandsomeTeam.do' />">
...
<!--pageTitle Start-->
				...
				<div class="tableTop">
					...
					<div class="tableSearch">
						...
						<spring:bind path="searchCondition.searchColumn">						
	                        <select title="${status.expression}" name="${status.expression}">
	                        	<option value="teamName" <c:if test="${'userName' eq status.value}">selected="selected"</c:if>>부서</option>
	                        </select>															
				...
				<div id="container">
					...
					<table summary="<ikep4j:message pre='${preList}' key='summary' />">
						...
						<tr>
							...
							<th scope="col" width="12%">
								<ikep4j:message pre='${preList}' key='reason' />	
							</th>
							...
							<c:choose>
							    ...
							    <c:otherwise>
									...
									<tr class="<c:if test="${itemDisplay eq 1}"></c:if>" name="boardItemLine" onmouseover="mouseOver(this);" onmouseout="mouseOut(this);">
										...
										<td>
											<a onclick="javascript:viewPopUp('${adminWinner.itemSeq}');" href="#a">${adminWinner.reason}</a>
										</td>
										<td>
		...
		<!--blockButton Start-->
         <div class="blockButton"> 
             <ul>
                 <li><a class="button" id="applyWinnerButton" href="<c:url value='/collpack/kms/admin/winner/registerWinnerTeam.do'/>"><span><ikep4j:message pre="message.collpack.kms.admin.winner.button" key="apply"/></span></a></li>
