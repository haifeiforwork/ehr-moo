...
(function($){	 
	...
	$(document).ready(function() {   
		...
		viewPopUp = function(itemSeq) { 
			var pageURL = "<c:url value='/collpack/kms/admin/winner/getWinnerAwardPopup.do' />?itemSeq=" + itemSeq;
		...
		$("#deleteWinnerButton").click(function() {  
			...
			if (itemIds.length > 0 ){	
				if(confirm("<ikep4j:message pre="message.collpack.kms.admin.winner.message" key="delete" />")) {
					...
					$.post("<c:url value='/collpack/kms/admin/winner/deleteWinnerAward.do'/>", {"itemSeqs" : itemIds.toString()}) 
		...
		$("#searchUserPermButton").click(function() { 
			$jq("#searchUserPermForm").attr("action","<c:url value='/collpack/kms/admin/winner/listAward.do'/>");
		...
		$("#excelWinnerButton").click(function() {
			...
			$jq("#searchUserPermForm").attr("action","<c:url value='/collpack/kms/admin/winner/downloadExcelWinnerAward.do'/>");
...
<form id="searchUserPermForm" method="post" action="<c:url value='/collpack/kms/admin/winner/listAward.do' />">
...
<!--pageTitle Start-->
				...
				<div class="tableTop">
					...
					<div class="tableSearch">
						...
						<spring:bind path="searchCondition.searchColumn">						
	                        <select title="${status.expression}" name="${status.expression}">
	                        	<option value="winnerName" <c:if test="${'winnerName' eq status.value}">selected="selected"</c:if>>상품명</option>
	                        </select>															
					...
					<table summary="<ikep4j:message pre='${preList}' key='summary' />">
						...
						<tr>
							...
							<th scope="col" width="12%">
								<a onclick="sort('WINNER_NAME', '<c:if test="${searchCondition.sortColumn eq 'WINNER_NAME'}">${searchCondition.sortType}</c:if>');"  href="#a">
									<ikep4j:message pre='${preList}' key='award' />
								</a>
								<c:choose>
									<c:when test="${searchCondition.sortColumn eq 'WINNER_NAME' and searchCondition.sortType eq 'DESC'}"><img alt="<ikep4j:message pre="${preSearch}" key="sortType" post="desc" />" src="<c:url value='/base/images/icon/ic_tablesort_down.gif'/>"></c:when>
									<c:when test="${searchCondition.sortColumn eq 'WINNER_NAME' and searchCondition.sortType eq 'ASC'}"><img alt="<ikep4j:message pre="${preSearch}" key="sortType" post="asc" />" src="<c:url value='/base/images/icon/ic_tablesort_up.gif'/>"></c:when>
								</c:choose>					
							</th>
							...
							<c:choose>
							    ...
							    <c:otherwise>
									...
									<tr class="<c:if test="${itemDisplay eq 1}"></c:if>" name="boardItemLine" onmouseover="mouseOver(this);" onmouseout="mouseOut(this);">
										...
										<td>
											<a onclick="javascript:viewPopUp('${adminWinner.itemSeq}');" href="#a">${adminWinner.winnerName}</a>
										</td>
										<td>
		...
		<!--blockButton Start-->
         <div class="blockButton"> 
             <ul>
                 <li><a class="button" id="applyWinnerButton" href="<c:url value='/collpack/kms/admin/winner/registerWinnerAward.do'/>"><span><ikep4j:message pre="message.collpack.kms.admin.winner.button" key="apply"/></span></a></li>
