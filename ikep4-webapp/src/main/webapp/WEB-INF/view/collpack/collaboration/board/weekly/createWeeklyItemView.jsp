	...
	$(document).ready(function() {    
		...
		var validOptions = {
			...
			submitHandler : function(form) {
				...
				// 부가 검증 처리
				var workspaceId = $("#weeklyItemForm input[name=workspaceId]").val();
				var weeklyTerm = $("#weeklyItemForm input[name=weeklyTerm]").val();
				var isSummary = $("#weeklyItemForm input[name=isSummary]").val();
				
				$jq.get("<c:url value='/collpack/collaboration/board/weekly/checkWeeklyTerm.do'/>", {"workspaceId": workspaceId,"weeklyTerm":weeklyTerm}) 
				.success(function(data) {
					//check duplicate weekly Item
					if(data!="empty"&&isSummary==0){
						if(confirm("<ikep4j:message pre="${preMessage}" key="checkConfirm3"/>\n<ikep4j:message pre="${preMessage}" key="checkConfirm4"/>")) {	
							//delete past Item
							$jq.get("<c:url value='/collpack/collaboration/board/weekly/removeDuplicateItem.do'/>", {"workspaceId": workspaceId,"itemId":data}) 
							.error(function(data) {
								alert(error);
							});
						}else{
		...
		$("#listWeeklyItemButton").click(function() { 
			$("#normalFileTb").show();
		});
		
		$("#saveWeeklyItemButton").click(function() { 	
			var tempEcmFileId = "";  
			var tempEcmFileName = ""; 
			var tempEcmFilePath = "";  
			var tempEcmFileUrl1 = "";    
...
<script language="JScript" for="twe" event="OnControlInit()">	
	...
	<form id="editorFileUploadParameter"> 
		<input name="workspaceId"  value="${weeklyItem.workspaceId}" type="hidden"/>	
...
<!-- Create Start-->
	<!--pageTitle Start-->
	<div id="pageTitle">
		<h2><ikep4j:message pre="${preDetail}" key="subTitle"/></h2>
	</div>
	<!--//pageTitle End-->
	<form id="weeklyItemForm" name="weeklyItemForm" method="post" action="<c:url value='/collpack/collaboration/board/weekly/createWeeklyItem.do'/>">
	...
	<spring:bind path="weeklyItem.isSummary">
		<input name="${status.expression}" type="hidden" value="${status.value}" title="<ikep4j:message pre='${preDetail}' key='${status.expression}'/>" />
	</spring:bind>
				<!--blockDetail Start-->
						...
						<tbody>
							<tr>
								...
								<td width="82%">
									...
									<div class="subInfo">
										<c:if test="${weeklyItem.isSummary==1}">
										...
										<c:if test="${weeklyItem.isSummary!=1}">
											<spring:bind path="weeklyItem.weeklyTerm">
												<input name="${status.expression}" type="text" class="inputbox datepicker" value="${status.value}" title="<ikep4j:message pre='${preDetail}' key='${status.expression}'/>" />
											</spring:bind>
										</c:if>
									</div>
								</td>
							...
							<tr>
								...
								<td>
									...
									<spring:bind path="weeklyItem.title">
										<input name="${status.expression}" type="text" class="inputbox w100" value="${status.value}" title="<ikep4j:message pre='${preDetail}' key='${status.expression}' />" />
							...
							<tr>
								<td colspan="2">
									...
									<textarea  id="contents" name="${status.expression}"
									class="tabletext fullEditor" cols="" rows="20"
		...
		<tr>
			...
			<img src="<c:url value="/base/images/dextupload/btn_listdelete_normal.gif"/>" name="Image4"  border="0" onclick="ecmfileDelete();" style="cursor:pointer;valign:absmiddle" />	
				...
				<div class="blockButton"> 
					<ul>
						<li><a id="saveWeeklyItemButton" class="button" href="#a"><span><ikep4j:message pre='${preButton}' key='save'/></span></a></li>
						<li><a id="listWeeklyItemButton" class="button" href="<c:url value='/collpack/collaboration/board/weekly/listWeeklyItemView.do?workspaceId=${weeklyItem.workspaceId}&weeklyTerm=${weeklyItem.weeklyTerm}'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span></a></li>
					</ul>
				</div>
				<!--//blockButton End-->						
	</form>		
