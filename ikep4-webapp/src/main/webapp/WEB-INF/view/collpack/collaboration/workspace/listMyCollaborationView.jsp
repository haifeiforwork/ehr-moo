...
<c:set var="preHeader"  value="message.collpack.collaboration.workspace.listMyCollaboration.header" />
<c:set var="preSearch"  value="message.collpack.collaboration.workspace.listMyCollaboration.search" />
<c:set var="preCommon"  value="message.collpack.common" />
<c:set var="preList"    value="message.collpack.collaboration.workspace.listMyCollaboration.list" />
<c:set var="preButton"  value="message.collpack.collaboration.workspace.listMyCollaboration.button" />
<c:set var="preScript"  value="message.collpack.collaboration.workspace.listMyCollaboration.script" />
...
(function($) {
	...
	$jq(document).ready(function() { 
		...
		$jq("#deleteMemberButton").click(function() {
			...
			if(countCheckBox>0)
			{			
				if(!confirm('<ikep4j:message pre='${preScript}' key='memberOut' />'))
				{
					return false; 		
				}
...
<form id="mainForm" name="mainForm" method="get" action="<c:url value='/collpack/collaboration/workspace/listMyCollaborationView.do' />">  
	...
	<table summary="<ikep4j:message pre="${preList}" key="summary" />">   
		...
		<thead>
			<tr>
				...
				<th scope="col">
					<a onclick="sort('isDefault', '<c:if test="${searchCondition.sortColumn eq 'isDefault'}">${searchCondition.sortType}</c:if>');"  href="#a">
						<ikep4j:message pre='${preList}' key='isDefault' />
					</a>
				</th>
			</tr>
		</thead> 
			...
			<c:choose>
			    ...
			    <c:otherwise>
					...
					<tr>
						...
						<td><ikep4j:timezone pattern="yyyy.MM.dd" date="${workspaceList.joinDate}"/></td>
						<td>
							<c:choose>
						...
						<td><a id="defaultButton" class="button_s <c:if test="${workspaceList.isDefault==1}">selected</c:if>" onclick="defaultSetting('${workspaceList.workspaceId}')" href="#a" title="<ikep4j:message pre='${preButton}' key='default'/>"><span><ikep4j:message pre='${preButton}' key='default'/></span></a></td>				
					</tr>
					</c:forEach>				      
			    </c:otherwise> 
			</c:choose>  
