...
(function($) {
	...
	$(document).ready(function() {  
		...
		$("a[name=createBoardButton]").click(function() {   
			...
			<c:if test="${board.boardType eq 0}">
			$("#boardForm").attr("action", "<c:url value='/collpack/collaboration/board/boardAdmin/createBoardTypeBoard.do'/>");
			</c:if>

			<c:if test="${board.boardType eq 1}">
			$("#boardForm").attr("action", "<c:url value='/collpack/collaboration/board/boardAdmin/createCategoryTypeBoard.do'/>");
			</c:if>	 
			<c:if test="${board.boardType eq 2}">
			$("#boardForm").attr("action", "<c:url value='/collpack/collaboration/board/boardAdmin/createLinkTypeBoard.do'/>");
			</c:if>
...
//-->
		...
		<table summary="<ikep4j:message pre='${preDetail}' key='summary' />"> 
				...
				<td colspan="3">
					...
					<div id="readPermissionListDiv" style="display:none">
						<select id="readPermissionList" name="readPermissionList" size="10" multiple="multiple" class="multi w60" title="<ikep4j:message pre='${preDetail}' key='${status.expression}'  post="readPermission" />" >  			
							<c:forEach var="user" items="${board.readPermissionList}"> 
			...
			<tr> 
				...
				<td colspan="3">
					...
					<div id="writePermissionListDiv" style="display:none">
						<select id="writePermissionList" name="writePermissionList" size="10" multiple="multiple" class="multi w60" title="<ikep4j:message pre='${preDetail}' key='${status.expression}'  post="writePermission" />" >  			
							<c:forEach var="user" items="${board.writePermissionList}"> 
								<option value="${userId}" id="${user.userId}^${user.userName}^${user.jobTitleName}^${user.teamName}^${user.mail}">
								${user.userName} ${user.jobTitleName} (${user.teamName}, ${user.mail})
								</option>
							</c:forEach>  
	...
	<div class="blockButton" id="divButton"> 
		<ul> 
			...
			<li>
				<a name="listBoardViewButton" class="button" href="<c:url value='/collpack/collaboration/board/boardAdmin/listBoardView.do?workspaceId=${board.workspaceId}&boardRootId=0'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span>
				</a>
			</li>			
		</ul>
	</div>
