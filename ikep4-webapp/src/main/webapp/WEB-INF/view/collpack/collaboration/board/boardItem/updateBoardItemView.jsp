...
<div id="pageTitle" class="btnline"> 
	...
	<div class="blockButton"> 
		<ul>
			<c:if test="${permission.isSystemAdmin or user.userId eq boardItem.registerId or (board.wiki == 1 && permission.isWritePermission)}"><%-- 쓰기 가능 권한인 경우 저장 버튼 활성화 --%>
...
<form id="boardItemForm" method="post" action="<c:url value='/collpack/collaboration/board/boardItem/updateBoardItem.do'/>">
			...
			<td colspan="3">
				...
				<div id="readPermissionListDiv" style="display:<c:if test="${boardItem.readPermission!='4'}">none</c:if>">
					<select name="readPermissionList" id="readPermissionList" class="input_select w80" size="4"	style="height:100px;" multiple>
						<c:forEach var="item" items="${boardItem.aclReadPermissionList.groupDetailList}">
							<option value="G:${item.groupId}"><c:out value='${item.groupName}'/></option>
						</c:forEach>
						<c:forEach var="item" items="${boardItem.aclReadPermissionList.assignUserDetailList}">
							<option value="U:${item.userId}"><c:out value='${item.userName}'/>/<c:out value='${item.jobTitleName}'/>/<c:out value='${item.teamName}'/></option>
						</c:forEach>					
					</select>						
