...
<c:set var="preDetail"  value="message.collpack.collaboration.announce.boardItem.createBoardView.detail" /> 
	...
	writeSubmit = function(targetForm){
		...
		if(useActXEditor == "Y"){
	    	if ($.browser.msie) {
	    		...
	    		//에디터 내 이미지 파일 링크 정보 세팅
	    		createActiveXEditorFileLink(tweBodyObj,"announceItemForm");
...
<script language="JScript" for="twe" event="OnControlInit()">
	var form = document.announceItemForm;
...
<div class="blockDetail">
	<form id="announceItemForm" name="announceItemForm" method="post" action="<c:url value='/collpack/collaboration/board/announce/updateAnnounceItem.do'/>">
	...
	<table summary="게시판등록">
		...
		<tr> 
			...
			<td>
				...
				<input id="itemDisplayCheck" name="itemDisplayCheck" type="checkbox" class="checkbox" value="1" size="40" <c:if test="${announceItem.itemDisplay==1}">checked="checked"</c:if> title="<ikep4j:message pre='${preDetail}' key='itemDisplay' />"/>
		...
		<%--tr>  
			...
			<td colspan="3">
				...
				type="text" 
				class="inputbox w40" 
				value="<c:forEach var="tag" items="${announceItem.tagList}" varStatus="tagLoop"><c:if test="${tagLoop.count != 1}">, </c:if>${tag.tagName}</c:forEach>" 
				size="40" 
				title="<ikep4j:message pre='${preDetail}' key='${status.expression}' />" >
	...
	<spring:bind path="announceItem.workspaceId">
		<input type="hidden" name="${status.expression}" type="hidden" value="${status.value}">
	</spring:bind>
	</form>
</div>
<c:if test="${ecmrole && user.essAuthCode != 'E9'}">
...
<div class="tableBottom">										
	...
	<div class="blockButton"> 
		<ul>
			...
			<li><a id="listBoardItemButton" class="button" href="<c:url value='/collpack/collaboration/board/announce/listAnnounceItemView.do?workspaceId=${announceItem.workspaceId}'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span></a></li>
		 </ul>
	</div>
	<!--//blockButton End--> 
</div>
