						...
						<span>
							<c:choose>
								<c:when test="${announcePermission>0}">
									<a name="announceItem" href="<c:url value='/collpack/collaboration/board/announce/readAnnounceItemView.do?itemId=${announceItem.itemId}&workspaceId=${workspaceId}'/>">${announceItem.title}</a>
								</c:when>
								<c:otherwise>
									${announceItem.title}
								</c:otherwise>
