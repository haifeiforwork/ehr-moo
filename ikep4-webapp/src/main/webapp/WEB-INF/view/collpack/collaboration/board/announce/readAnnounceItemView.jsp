	...
	$(document).ready(function() { 
		...
		$("#adminShareCollAnnounceItemButton").click(function() {  
			iKEP.showDialog({
				...
				width:800,
				height:400,
				url: "<c:url value='/collpack/collaboration/board/announce/shareAnnounceItemPop.do?itemId=${announceItem.itemId}&workspaceId=${announceItem.workspaceId}'/>",
				modal:true,
				callback : function() {
					location.replace("<c:url value='/collpack/collaboration/board/announce/listAnnounceItemView.do'/>?workspaceId=${announceItem.workspaceId}");
				}
			});	
		});
		...
		$("#adminDeleteBoardItemButton").click(function() {
			if(confirm("<ikep4j:message pre="${preMessage}" key="deleteItem" />")) {
				location.href="<c:url value='/collpack/collaboration/board/announce/adminDeleteAnnounceItem.do?itemId=${announceItem.itemId}&workspaceId=${announceItem.workspaceId}'/>"; 
			}  
			return false; 
				...
				<ul>
					<c:if test="${announcePermission>1 || announceItem.registerId eq user.userId}">
						<c:if test="${isOrganization eq 1 && announceItem.isOwner eq 1}">
							...
							<li>
								<a class="button" href="<c:url value='/collpack/collaboration/board/announce/updateAnnounceItemView.do?itemId=${announceItem.itemId}&workspaceId=${announceItem.workspaceId}&workspaceName=${workspaceName}'/>"><span><ikep4j:message pre='${preButton}' key='update'/></span></a>
							</li>
						...
						<li>
							<a class="button" href="<c:url value='/collpack/collaboration/board/announce/listAnnounceItemView.do?workspaceId=${announceItem.workspaceId}'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span></a>
						</li>
				</ul>  
			</div>
			<!--//blockButton End-->
