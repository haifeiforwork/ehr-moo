...
<c:forEach var="mblog" items="${mblogList}" varStatus="loopStatus">
	<li id="timelineMblogId_${mblog.mblogId}" name="timelineAddonCode_${mblog.addonCode}">
		...
		<div class="microblog_con">
			...
			<span class="microblog_id"><a name='addonLink' href="<c:url value='/support/fileupload/downloadFile.do?fileId=${mblog.sourceLink}' />" >{file:<c:out value="${mblog.addonCode}"/>}</a></span>
			<span class="microblog_name"><c:out value="${mblog.fileRealName}"/></span>
