...
(function($) {
	...
	$(document).ready(function() {
		...
		if(param != undefined) {
			...
			$jq.post("<c:url value='/socialpack/microblogging/addon/createAddon.do'/>",
					{'addonType':'0','sourceLink':param.arguUrl}, 
					function(data) {
						//alert(data);

						document.getElementById("contents").value = param.arguText + data; 
						$jq('#contents').focus();
				...
				if(checkValid())
				{
					...
					$.ajax({
						...
						success : function(data) {
							...
							if(opener && opener.callbackMBlog){
								if(opener["callbackMBlog"])	opener.callbackMBlog();
								self.close();
							}
    ...
    $(document.body).click(function(event) { divHide("divSuggest"); });
		...
		<div class="corner_RoundBox05">
			...
			<div class="microblog_write_list"  id="addonDiv">
				...
				<form id="mblogForm" name="mblogForm" method="post" action="">
					...
					<textarea id="contents" name="contents" title="<ikep4j:message pre='${preLabel}' key='contents' />" cols="" rows="3"  class="w100" ></textarea>
