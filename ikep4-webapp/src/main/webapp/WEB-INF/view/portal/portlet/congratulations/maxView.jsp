...
<c:set var="preMain" value="ui.portal.portlet.congratulations.maxView.main"/>
<c:set var="preList" value="ui.portal.portlet.congratulations.maxView.list"/>
...
<div class="tableList_1" id="${portletConfigId}">
	...
	<table summary="<ikep4j:message pre='${preMain}' key='birthdayTableSummary' />">
		...
		<tbody>
			...
			<c:if test="${birthday.type == 'BIRTHDAY'}">
			<c:set var="birthdayCnt" value="${birthdayCnt+1}" />
			<tr>
				<c:choose>
				<c:when test="${currentDate == birthday.birthdayMmDdType}">
				<th class="today" scope="row" width="90%">
					<div class="ellipsis">
				...
				<td class="textRight today" width="10%">
				</c:when>
				<c:otherwise>
