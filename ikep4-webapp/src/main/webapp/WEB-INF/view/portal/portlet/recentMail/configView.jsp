...
(function($) {
	...
	$jq(document).ready(function() { 
		...
		$jq("#save_${portletConfigId}").click(function() {
			var reloadUrl = '<c:url value="/portal/portlet/recentMail/normalView.do"/>?portletId=${portletId}';
...
<div id="${portletConfigId}" class="tableList_1">
	...
	<div class="poEdit">
		...
		<div class="textRight">
			<a id="save_${portletConfigId}" class="button" href="#a"><span><ikep4j:message pre='${portletList}' key='button.create' /></span></a></div>
	</div>
				...
				<c:choose>
				    ...
				    <c:otherwise>
						<c:forEach var="mail" items="${mailList}" varStatus="status">
							...
							<tr>
								<th width="*" scope="row">
									<div class="ellipsis"><a href="#a" onclick="showDatail_${portletConfigId}('<c:url value='/support/mail/viewMail.do'/>?mailId=${mail.mailId}', '${fn:replace(fn:replace(mail.title,"\"",""),"'","")}')">
									${mail.title}</a></div>
								</th>	
								<td width="55" class="textCenter"><div class="ellipsis">${mail.fromName}</div></td>
