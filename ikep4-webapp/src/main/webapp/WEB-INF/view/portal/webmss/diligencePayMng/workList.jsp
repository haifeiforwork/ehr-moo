...
(function($){
	$(document).ready(function(){
		...
		$("a[name=linkField]").click(function(){
			...
			$("#ajaxForm").attr("action", "<c:url value='/portal/moorimmss/diligencePayMng/workView.do'/>");
			$("#ajaxForm").submit();
		});
		...
		$.setCommonData = function(){
			...
			<c:forEach items="${resultMap.ET_AWART}" var="result">
			$("#ajaxForm").append("<input type='hidden' name='etAwartKey' value='<c:out value="${result.KEY}"/>'/>");
			$("#ajaxForm").append("<input type='hidden' name='etAwartValue' value='<c:out value="${result.VALUE}"/>'/>");
			</c:forEach>
			
			$("#ajaxForm").append("<input type='hidden' name='etAwartCnt' value='<c:out value="${fn:length(resultMap.ET_AWART)}"/>'/>");
		};
...
<form id="workForm" name="workForm" method="post" action="<c:url value='/portal/moorimmss/diligencePayMng/workList.do'/>" >
<div id="wrap">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" id="blktable">
			...
			<thead>
				<tr>
					...
					<th scope="col" width="10%">근무일수</th>
					<th scope="col" width="10%">근무시간</th>
				</tr>
			</thead>
			<tbody>
				<c:choose>
					...
					<c:otherwise>
						<c:forEach var="result" items="${etList}">
							<tr>
								...
								<td><c:out value="${result.ABWTG}"/></td>
								<td>
