...
(function($){
	$(document).ready(function(){
		...
		$("a[name=linkField]").click(function(){
			...
			$("#ajaxForm").append("<input type='hidden' name='searchParam' value='?startDate=<c:out value='${resultMap.EX_BEGDA}'/>&endDate=<c:out value='${resultMap.EX_ENDDA}'/>&imPernr=<c:out value='${params.imPernr}'/>'/>");
			
			$("#ajaxForm").attr("method", "post");
			$("#ajaxForm").attr("action", "<c:url value='/portal/moorimmss/diligencePayMng/dayOffView.do'/>");
			$("#ajaxForm").submit();
		});
...
<form id="dayOffForm" name="dayOffForm" method="post" action="<c:url value='/portal/moorimmss/diligencePayMng/dayOffList.do'/>" >
<div id="wrap">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" id="blktable">
			...
			<thead>
				<tr>
					...
					<th scope="col" width="90px">휴무종료일</th>
					<th scope="col" width="10%">휴무유형</th>
					<th scope="col" width="10%">휴무일수</th>
					<th scope="col" width="10%">휴무시간</th>
				</tr>
			</thead>
			<tbody>
				<c:choose>
