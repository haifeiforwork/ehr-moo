	...
	$(document).ready(function() {
		...
		$.bindHssRowClickHandler = function() {
			$(".list_table > tbody> tr").click(function() {
				...
				$("#ajaxForm").attr("action", "<c:url value='/portal/evaluation/technical/technicalHSSView.do'/>");
				$("#ajaxForm").submit();
			})
			.mouseover(function() {
				if($(this).find('.emptyRecord').size() > 0) return false;
		...
		$.getAndDrawHSSList = function(userId) {
			...
			$.ajax({
				url : "<c:url value='/portal/evaluation/technical/technicalHSSList.do'/>"
				, type : "post"
...
<div id="wrap">
	...
	<h1 class="title">기능직평가 평가표 조회</h1>
