...
(function($){
	$(document).ready(function() {
		...
		$.initFixedTable = function() {
			...
			$('#qualTable > thead > tr').eq(1).find('th')
					.eq(7).css('border-right-width', '0px').next().css('border-right-width', '0px')
				.end().end()
					.eq(11).css('border-right-width', '0px').next().css('border-right-width', '0px');
			...
			<c:if test="${not empty qualList}">
				<c:forEach var="result" items="${qualList}">
					<tr>
						...
						<td></td>
						<td><input type="text" name="fsoac" value="${result.FSOAC}" disabled style="width: 60px;text-align:center;" /></td>
						<td></td>
						<td><input type="text" name="apsco1" value="${result.APSCO1}" disabled style="width: 60px;text-align:center;"></td>
						<td></td>
						<td><input type="text" name="seopa" value="${result.SEOPA}" disabled style="width: 60px;text-align:center;" /></td>
						<td></td>
						<td><input type="text" name="apsco2" value="${result.APSCO2}" disabled style="width: 40px;text-align:center;"></td>
						<td><input type="text" name="ptsum" value="${result.PTSUM}" disabled style="width: 40px;text-align:center;"></td>
					</tr>
				</c:forEach>
			</c:if>
		</tbody>
			...
			<c:if test="${not empty quanList}">
				<c:forEach var="result" items="${quanList}">
					<tr>
						...
						<td><textarea name="outpt" disabled class="width_100" rows="5"><c:out value="${result.OUTPT}" /></textarea></td>
						<td><input type="text" name="fsoac" value="${result.FSOAC}" disabled style="width: 60px;text-align:center;" /></td>
						<td><input type="text" name="fsilv" value="${result.FSILV}" disabled style="width: 60px;text-align:center;" /></td>
						<td><input type="text" name="fstqt" value="${result.FSTQT}" disabled style="width: 60px;text-align:center;" /></td>
						<td><input type="text" name="apsco1" value="${result.APSCO1}" disabled style="width: 60px;text-align:center;"></td>
						<td><input type="text" name="seopa" value="${result.SEOPA}" disabled style="width: 60px;text-align:center;" /></td>
						<td><input type="text" name="seilv" value="${result.SEILV}" disabled style="width: 60px;text-align:center;" /></td>
						<td><input type="text" name="setqt" value="${result.SETQT}" disabled style="width: 60px;text-align:center;" /></td>
						<td><input type="text" name="apsco2" value="${result.APSCO2}" disabled style="width: 40px;text-align:center;"></td>
						<td><input type="text" name="ptsum" value="${result.PTSUM}" disabled style="width: 40px;text-align:center;"></td>
					</tr>
				</c:forEach>
