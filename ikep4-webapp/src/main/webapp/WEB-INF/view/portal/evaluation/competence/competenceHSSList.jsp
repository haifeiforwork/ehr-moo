	...
	$(document).ready(function() {
		...
		$.bindHssRowClickHandler = function() {
			$(".list_table > tbody> tr").click(function() {
				...
				$("#ajaxForm").attr("action", "<c:url value='/portal/evaluation/competence/competenceHSSView.do'/>");
				$("#ajaxForm").submit();
			})
			.mouseover(function() {
				if($(this).find('.emptyRecord').size() > 0) return false;
		...
		$.getAndDrawHSSList = function(userId) {
			...
			$.ajax({
				url : "<c:url value='/portal/evaluation/competence/competenceHSSList.do'/>"
				, type : "post"
