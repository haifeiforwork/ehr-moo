...
(function($){
	...
	$(document).ready(function() {
		...
		$.makeFormData = function(data) {
			...
			formData.push({name : "action", value : "HSS_VIEW"});
				...
				$.ajax({
					...
					, success : function(result) {
						if(result.MSGTY == 'W' || result.MSGTY == 'E') {
							...
							$("#ajaxForm").attr("action", "<c:url value='/portal/evaluation/privilege/privilegeHSSView.do'/>");
							$("#ajaxForm").submit();
						}
					}
		...
		$.getAndDrawHSSList = function(userId) {
			...
			$.ajax({
				url : "<c:url value='/portal/evaluation/privilege/privilegeHSSList.do'/>"
				, type : "post"
...
<div id="wrap">
	...
	<h1 class="title">촉탁/별정직 평가표 조회</h1>
