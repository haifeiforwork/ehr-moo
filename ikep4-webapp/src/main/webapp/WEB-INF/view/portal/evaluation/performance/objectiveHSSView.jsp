...
(function($){
	$(document).ready(function() {
		...
		$.initFixedTable = function() {
			...
			//정량목표 테이블 조정
			$ft_r.eq(0).find('tr').eq(1).find('th')
					.eq(7).css('border-right-width', '0px').next().css('border-right-width', '0px')
				.end().end()
					.eq(11).css('border-right-width', '0px').next().css('border-right-width', '0px');
		...
		$.initView = function() {
			...
			$('[name=evalu_score_sum2]').val($.SumScore('apsco2'));
		...
		$('#btnBack').click(function(e) {
			e.preventDefault();
			$.CallListPage({
				SUBMIT_ACTION : "<c:url value='/portal/evaluation/performance/objectiveHSS.do'/>",
			...
			<c:if test="${not empty qualList}">
				<c:forEach var="result" items="${qualList}">
					<tr>
						...
						<td></td>
						<td><input type="text" name="fsoac" disabled style="width: 60px;text-align:center;" value="${result.FSOAC}" data-semid="EHAP1" /></td>
						<td></td>
						<td><input type="text" name="apsco1" disabled style="width: 60px;text-align:center;" value="${result.APSCO1}"></td>
						<td></td>
						<td><input type="text" name="seopa" disabled style="width: 60px;text-align:center;" value="${result.SEOPA}" data-semid="EHAP1" /></td>
						<td></td>
						<td><input type="text" name="apsco2" disabled style="width: 40px;text-align:center;" value="${result.APSCO2}"></td>
			...
			<c:if test="${not empty quanList}">
				<c:forEach var="result" items="${quanList}">
					<tr>
						...
						<td><textarea name="mcfbk" disabled class="width_100" rows="5"><c:out value="${result.MCFBK}" /></textarea></td>
						<td><textarea name="outpt" disabled class="width_100" rows="5"><c:out value="${result.OUTPT}" /></textarea></td>
						<td><input type="text" name="fsoac" disabled style="width: 60px;text-align:center;" value="${result.FSOAC}" data-semid="FHAP1" /></td>
						<td><input type="text" name="fsilv" disabled style="width: 60px;text-align:center;" value="${result.FSILV}" data-semid="FHAP2" /></td>
						<td><input type="text" name="fstqt" disabled style="width: 60px;text-align:center;" value="${result.FSTQT}" data-semid="FHAP3" /></td>
						<td><input type="text" name="apsco1" disabled style="width: 60px;text-align:center;" value="${result.APSCO1}"></td>
						<td><input type="text" name="seopa" disabled style="width: 60px;text-align:center;" value="${result.SEOPA}" data-semid="FHAP1" /></td>
						<td><input type="text" name="seilv" disabled style="width: 60px;text-align:center;" value="${result.SEILV}" data-semid="FHAP2" /></td>
						<td><input type="text" name="setqt" disabled style="width: 60px;text-align:center;" value="${result.SETQT}" data-semid="FHAP3" /></td>
						<td><input type="text" name="apsco2" disabled style="width: 40px;text-align:center;" value="${result.APSCO2}"></td>
						<td><input type="text" name="ptsum" disabled style="width: 40px;text-align:center;" value="${result.PTSUM}"></td>
					</tr>
				</c:forEach>
		...
		<table class="contents_table no_border">
			...
			<tbody>
				<tr>
					...
					<td></td>
					<td></td>
					<td class="text_center"><input type="text" name="evalu_score_sum2" class="border_gray" disabled style="width: 40px;text-align:center;"></td>
