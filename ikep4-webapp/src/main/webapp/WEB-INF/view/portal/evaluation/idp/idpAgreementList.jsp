...
(function($){
	$(document).ready(function() {
		...
		$.makeFormData = function(data) {
			...
			formData.push({name : "action", value : "IDP_AGREEMENT_VIEW"});
		...
		$.setParam = function(data) {
			...
			$("#ajaxForm").append("<input type=\"hidden\" name=\"action\" value=\"IDP_AGREEMENT_VIEW\"/>");
			...
			$.ajax({
				...
				, success : function(result) {
					if(result.MSGTY == 'W' || result.MSGTY == 'E') {
						...
						$("#ajaxForm").attr("action", "<c:url value='/portal/evaluation/idp/idpAgreementView.do'/>");
						$("#ajaxForm").submit();
					}
				}
...
<div id="wrap">
	...
	<input type="hidden" name="action" value="IDP_AGREEMENT" />
	<h1 class="title">IDP(${currentYear}) 합의</h1>
