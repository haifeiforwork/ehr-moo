...
(function($){
	$(document).ready(function() {
		...
		$.businessProcess = function(params) {
			...
			$.ajax({
				url : "<c:url value='/portal/evaluation/competence/competenceReview2Proc.do'/>"
				, type : "post"
				...
				, success : function(result) {
					if(result.ES_RETURN.MSGTY == 'E') {
						...
						if(params.isCallList) {
							$.CallListPage({
								SUBMIT_ACTION : "<c:url value='/portal/evaluation/competence/competenceReview2List.do'/>",
		...
		$('#reviewTable > tbody').find('a').click(function(e) {
			...
			var empno = $(this).closest('tr').find('td').eq(1).text(),
				url = iKEP.getContextRoot()
					  + "/portal/evaluation/competence/popCompetenceindividualEvaluationList.do?empno="
					  + empno;
