...
(function($){
	$(document).ready(function() {
		...
		$.calcAllReviewScore = function() {
			...
			$('#reviewTable > tbody > tr').each(function() {
				evalScore = parseFloat($(this).find('td:nth-child(5)').text());
				selectScore = 0;
		...
		$.fn.calcReviewScore = function() {
			var _selScore = $(this).val(),
				_prevScore = parseFloat($(this).closest('tr').find('td:nth-child(5)').text());
		...
		$.pageReload = function() {
			$('#ajaxForm')
				.attr("method", "POST")
				.attr("action", "<c:url value='/portal/evaluation/performance/objectiveReviewList.do'/>")
				.submit();
		}
		...
		$.makeFormData = function(button) {
			...
			$.each(boardList, function(index, item) {
				...
				formData.push({name:'reviews['+index+'].rvwsc', value:$reviewTableTR.eq(index).find('td:last').text()});
		...
		$.businessProcess = function(params) {
			...
			$.ajax({
				url : "<c:url value='/portal/evaluation/performance/objectiveReviewProc.do'/>"
				, type : "post"
...
<div id="wrap">
	...
	<form id="evaluationForm" name="evaluationForm" method="post" action="<c:url value='/portal/evaluation/performance/objectiveReviewList.do'/>">
	<h1 class="title">업적평가 1차 Review</h1>
