...
(function($){
	$(document).ready(function() {
		...
		$.drawChartSpider = function() {
			...
			var options = {
				series:{
			        spider:{
						...
						legMax: 100,
			            legs: {
			            		font: "bold 8px Malgun Gothic, '맑은 고딕', 'AppleGothic', 'Dotum', 'sans-serif'",
								data: label,
								fillStyle: "black",
								legScaleMax:0,
			                	legScaleMin:0,
			...
			var p = $.plot($("#chartSpider"),data,options);
		}
		...
		$.drawChartStick = function(options) {
			...
			$('.flot-y-axis').find('.tickLabel').each(function() {
				$(this)
					.css('top', parseInt($(this).css('top'))-3)
					.css('left', '0px');
			});
			...
			$.each(p.getData()[0].data, function(i, el) {
				var o = p.pointOffset({x: el[0], y: el[1]});
				$('<div class="data-point-label" style="width:10px;word-break: break-all;line-height: 12px;">' + ticks[i].substring(0, 6) + '</div>').css( {
					position: 'absolute',
					left: o.left - 7,
...
<div id="wrap">
	...
	<div class="add_tabcon print_cover" style="display:block;">
		<span class="print_title">2018년도 다면진단 피드백 리포트</span>
		<table class="list_table info" style="margin:0 auto;">
			<colgroup>
			...
			<tbody>
				<tr>
					<th>소속</th>
					<td class="text_left">${params.ORGTX}</td>
				</tr>
				<tr>
					<th>진단대상자</th>
					<td class="text_left">${params.MAPENM}</td>
				</tr>
			</tbody>
		</table>
		<span class="print_logo"><img src="${pageContext.request.contextPath}/base/images/evaluation/print_logo.png" /></span>
	</div>
	...
	<div class="add_tabcon" style="display:block;">
		...
		<div class="chart_box" style="text-align: center;">
			<div id="chartSpider" style="width:500px;height:350px;display:inline-block;"></div>
		</div>
