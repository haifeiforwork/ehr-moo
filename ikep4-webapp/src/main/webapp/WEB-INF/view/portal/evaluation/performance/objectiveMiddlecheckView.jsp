...
(function($){
	$(document).ready(function() {
		...
		$.initView = function() {
			...
			$('[name=mdchk]').each(function() { $(this).AutoHeightResizeTextarea(); });
		...
		$.viewMode = function() {
			...
			$('textarea[name="mdchk"]').each(function(index) { $(this).prop("disabled", true); });
		...
		$.isNanModelRow = function() {
			var isEmpty = false;
			$('[name=mdchk]').each(function() { if(!$(this).val()) isEmpty = true; });
		...
		$.beforSubmitRenameForModelAttribute = function() {
			...
			$('textarea[name="mdchk"]').each(function(index) { $(this).attr("name", "goals[" + index + "].mdchk"); });
		}

		$.afterSubmitRenameForView = function() {
			$('[name^=goals]').each(function(index) { $(this).attr("name", $(this).attr("name").split(".")[1]).prop("disabled", true); });
			$('textarea[name="mdchk"]').each(function(index) { $(this).prop("disabled", false); });
		}
		...
		$.businessProcess = function(params) {
			...
			$.ajax({
				url : "<c:url value='/portal/evaluation/performance/objectiveMiddlecheckProc.do'/>"
				, type : "post"
					...
					if(result.ES_RETURN.MSGTY == 'E') {
						...
						if(params.isCallList) {
							$.CallListPage({
								...
								PARAM_ACTION : "OBJECTIVE_MIDDLECHECK",
		...
		$.bindButtonCompleteHandler = function() {
			$('#btnComplete').click(function(e) {
				...
				if(confirm("중간점검이 완료되어 Feedback이 진행됩니다.\n계속하시겠습니까?")) {
		...
		<c:if test="${params.ACHVSTS ge '05'}">
		$.viewMode();
		</c:if>
...
<div id="wrap">
	...
	<form id="evaluationForm" name="evaluationForm" method="post" action="<c:url value='/portal/evaluation/performance/objectiveMiddlecheckProc.do'/>">
	<h2 class="title">정량</h2>
	<table id="qualTable" class="contents_table">
		<colgroup>
			<c:choose>
				<c:when test="${params.ACHVSTS ge '07'}">
					<col style="width: 18%;" />
					<col style="width: 18%;" />
					<col style="width: 18%;" />
					<col style="width: 9%;" />
			...
			<c:if test="${not empty qualList}">
				<c:forEach var="result" items="${qualList}">
					<tr>
						...
						<td><textarea name="mdchk" class="width_100" rows="5"><c:out value="${result.MDCHK}" /></textarea></td>
						<c:if test="${params.ACHVSTS ge '07'}">
							<td><textarea disabled class="width_100" rows="5"><c:out value="${result.MCFBK}" /></textarea></td>
						</c:if>
					</tr>
				</c:forEach>
			</c:if>
