...
(function($){
	$(document).ready(function() {
		...
		$.fn.calcScore = function() {
			...
			$tr.find('[name=apsco1]').val(scoreResult);
			$tr.find('[name=apsco2]').val(scoreResult2);
		...
		$.initView = function() {
			...
			<c:if test="${params.APSTS lt '2'}">
				...
				$('[name=fsoac]').each(function() { $(this).calcScore() });
			</c:if>
		...
		$.viewMode = function() {
			...
			$('[name=fsoac]').each(function() { $(this).prop("disabled", true) });
			$('[name=fsilv]').each(function() { $(this).prop("disabled", true) });
			$('[name=fstqt]').each(function() { $(this).prop("disabled", true) });
		...
		$.isNanModelRow = function() {
			...
			$('[name=fsoac]').each(function() { if($(this).val() == '') isEmpty = true; });
			$('[name=fsilv]', '#quanTable').each(function() { if($(this).val() == '') isEmpty = true; });
			$('[name=fstqt]', '#quanTable').each(function() { if($(this).val() == '') isEmpty = true; });
		...
		$.beforSubmitRenameForModelAttribute = function() {
			...
			$('[name="apsco1"]').each(function(index) { $(this).attr("name", "goals[" + index + "].apsco1").prop("disabled", false); });
			$('[name="apsco2"]').each(function(index) { $(this).attr("name", "goals[" + index + "].apsco2"); });
		...
		$.afterSubmitRenameForView = function() {
			...
			$('[name="apsco2"]').each(function(index) { $(this).prop("disabled", false); });
			...
			<c:if test="${not empty qualList}">
				<c:forEach var="result" items="${qualList}">
					<tr>
						...
						<td>
							<select class="width_100" name="fsoac" data-semid="EHAP1">
								<option value="">선택</option>
								<option value="S" <c:if test="${result.FSOAC eq 'S'}">selected</c:if>>S</option>
								<option value="A" <c:if test="${result.FSOAC eq 'A'}">selected</c:if>>A</option>
								<option value="B" <c:if test="${result.FSOAC eq 'B'}">selected</c:if>>B</option>
								<option value="C" <c:if test="${result.FSOAC eq 'C'}">selected</c:if>>C</option>
								<option value="D" <c:if test="${result.FSOAC eq 'D'}">selected</c:if>>D</option>
							</select>
							<input type="hidden" name="seopa" value="${result.SEOPA}" />
						</td>
						<td>
						...
						<td>
							<input type="text" name="apsco1" value="${result.APSCO1}" disabled style="width: 60px;text-align:center;">
			...
			<c:if test="${not empty quanList}">
				<c:forEach var="result" items="${quanList}">
					<tr>
						...
						<td><textarea name="outpt" disabled class="width_100" rows="5"><c:out value="${result.OUTPT}" /></textarea></td>
						<td>
							<select class="width_100" name="fsoac" data-semid="FHAP1">
								<option value="">선택</option>
								<option value="S" <c:if test="${result.FSOAC eq 'S'}">selected</c:if>>S</option>
								<option value="A" <c:if test="${result.FSOAC eq 'A'}">selected</c:if>>A</option>
								<option value="B" <c:if test="${result.FSOAC eq 'B'}">selected</c:if>>B</option>
						...
						<td>
							<select class="width_100" name="fsilv" data-semid="FHAP2">
								<option value="">선택</option>
								<option value="S" <c:if test="${result.FSILV eq 'S'}">selected</c:if>>S</option>
								<option value="A" <c:if test="${result.FSILV eq 'A'}">selected</c:if>>A</option>
								<option value="B" <c:if test="${result.FSILV eq 'B'}">selected</c:if>>B</option>
								<option value="C" <c:if test="${result.FSILV eq 'C'}">selected</c:if>>C</option>
								<option value="D" <c:if test="${result.FSILV eq 'D'}">selected</c:if>>D</option>
							</select>
							<input type="hidden" name="seilv" value="${result.SEILV}" />
						</td>
						<td>
							<select class="width_100" name="fstqt" data-semid="FHAP3">
								<option value="">선택</option>
								<option value="S" <c:if test="${result.FSTQT eq 'S'}">selected</c:if>>S</option>
								<option value="A" <c:if test="${result.FSTQT eq 'A'}">selected</c:if>>A</option>
								<option value="B" <c:if test="${result.FSTQT eq 'B'}">selected</c:if>>B</option>
								<option value="C" <c:if test="${result.FSTQT eq 'C'}">selected</c:if>>C</option>
								<option value="D" <c:if test="${result.FSTQT eq 'D'}">selected</c:if>>D</option>
							</select>
							<input type="hidden" name="setqt" value="${result.SETQT}" />
						</td>
						<td>
							<input type="text" name="apsco1" value="${result.APSCO1}" disabled style="width: 60px;text-align:center;">
							<input type="hidden" name="apsco2" value="${result.APSCO2}" />
