		...
		<c:forEach var="popup" items="${popupList}" varStatus="status">
			if($jq.cookie("IKEP_POPUP_${popup.popupId}") != 'none') {
				if(${popup.windowYn == 'Y'}) {
					...
					//Popup type
	    			//iKEP.popupOpen('<c:url value="/portal/admin/screen/popup/mainPopup.do?popupId=${popup.popupId}"/>', {width:popupWidth, height:${popup.popupHeight}, top:popupTop, left:popupLeft, resizable:true, scrollbar:false}, 'popup${status.count}');
	    			//window.open('<c:url value="/portal/admin/screen/popup/mainPopup.do?popupId=${popup.popupId}"/>', {width:popupWidth, height:${popup.popupHeight}, top:popupTop, left:popupLeft, resizable:true, scrollbar:false}, 'popup${status.count}');
	    			window.open( '<c:url value="/portal/admin/screen/popup/mainPopup.do?popupId=${popup.popupId}"/>' ,'popup${status.count}','resizable=yes,scrollbars=yes,width='+popupWidth+',height=${popup.popupHeight},top='+popupTop+', left='+popupLeft);
...
<div id="topMenu">
	...
	<ul>
		...
		<c:set var="childCount" value="0"/>
		<li>
			<a class="${menu.iconId}" style="cursor:pointer;"
				<c:choose>
		...
		<c:set var="childCount" value="${childCount+1}"/>
				...
				<li>
					<a 
						...
						href="<c:url value='${ikep4j:urlConverter(menu.url, user)}'/>"
						<c:if test="${menu.menuId == '100000788182'}">
							onclick="mailHRCountRefresh()"
						</c:if>
						</c:when>
...
<!--//topMenu End-->
		<div class="quicktxt" >
			<ul>
				<li class="quicktxt_personal"></li>
				<li class="quicktxt_recent"></li>
				<li class="quicktxt_favorite"></li>	
			</ul>
