	...
	$(document).ready(function(){
		...
		$("#btnApprove, #btnReject").click(function(){
			...
			if(process){
				...
				$jq.ajax({
					url : "<c:url value='/portal/moorimess/frequentEduOnInput.do'/>",
		...
		refUrlPop = function(targetUrl) { 
			if(targetUrl.indexOf("http://") == -1){
				targetUrl = 'http://'+targetUrl;
			}
			window.open(targetUrl,'tUrl','width=1000,height=650,location=1,menubar=1,scrollbars=1,toolbar=1,resizable=1');
		};
...
<div id="wrap">
				...
				<tbody>
					<tr>
						...
						<td class="list03_td">
							...
							<font style="color:#e07a0c;">
								<c:out value="${resultInit.EX_NEWEDU}"/>
							</font>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="list03_td">
							<c:forEach items="${resultInit.ET_ETYPE }" var="result">
					...
					<tr>
						...
						<td class="list03_td">
							<c:forEach items="${resultInit.ET_INSCK }" var="result">
					...
					<tr>
						<th class="list03_td_bg">수강목적 및 기대효과</th>
						<td class="list03_td">
							<textarea style="width:95%" rows="6"><c:out value="${resultInit.EX_OPINION3 }"/></textarea>
						</td>
					</tr>
					<tr>
						<th class="list03_td_bg">참조URL</th>
						<td class="list03_td">
							<a href="javascript:refUrlPop('<c:out value="${exDlist.URL}"/>');">
								<c:out value="${exDlist.URL}"/>
							</a>
						</td>
					</tr>
				</tbody>
	...
	<div class="table_box">
		<c:set var="etElist" value="${resultInit.ET_ELIST }"/>
		<p class="f_title" style="padding-bottom:10px">사외교육이수현황</p>
		<span style="display:inline-block; .display:inline; *zoom:1"  class="list01">
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<caption></caption>
				<colgroup>
				...
				<tbody>
					<c:choose>
						<c:when test="${empty etElist}">
							<tr>
								<td colspan="5">최근 3년간 사외교육역량교육 이력이 없습니다.</td>
							</tr>
						</c:when>
						<c:otherwise>
							<c:forEach var="result" items="${etElist}">
								<tr>
									<td><c:out value="${result.E_STEXT }"/></td>
									<td><c:out value="${result.L_STEXT }"/></td>
									<td><c:out value="${result.ETIME }"/></td>
									<td><c:out value="${result.NHOURS }"/></td>
									<td class="f_right"><fmt:formatNumber value="${result.IKOSTC}" groupingUsed="true"/></td>
								</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>
