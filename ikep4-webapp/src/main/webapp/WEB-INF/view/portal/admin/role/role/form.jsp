...
// 										},
		...
		<form id="roleForm" name="roleForm" method="post" onsubmit="return false;" action="">
			...
			<div class="blockDetail">
				<table summary="<ikep4j:message pre="${uiPrefix}" key="tableSummary.1" />">
					<caption></caption>
					<colgroup>
						<col width="15%"/>
						<col width="10%"/>
					...
					<tbody>
						<c:choose>
							<c:when test="${not empty role.roleId}">
								<tr>
									<th scope="row" colspan="2"><span class="colorPoint">*</span> <ikep4j:message pre="${uiPrefix}" key="roleId" /></th>
									<td>
										${role.roleId}
									</td>
								</tr>									
						...
						<tr>
							<th scope="row" rowspan="2"><span class="colorPoint">*</span> <ikep4j:message pre="${uiPrefix}" key="roleName" /></th>
							<th><span class="colorPoint">*</span> <ikep4j:message pre="${uiPrefix}" key="roleKoreanName" /></th>
							<td>
								<div>
									<input type="text" name="roleName" id="roleName" value="${role.roleName}" size="50" class="inputbox" />
								</div>
							</td>
						</tr>
						<tr>
							<th><span class="colorPoint">*</span> <ikep4j:message pre="${uiPrefix}" key="roleEnglishName" /></th>
							<td>
								<div>
									<input type="text" name="roleEnglishName" id="roleEnglishName" value="${role.roleEnglishName}" size="50" class="inputbox" />
								</div>
							</td>
						</tr>
						<tr>
							<th scope="row" colspan="2"><span class="colorPoint">*</span> <ikep4j:message pre="${uiPrefix}" key="description" /></th>
							<td>
								<div>
									<input type="text" id="description" name="description" value="${role.description}" size="50" class="inputbox" />
								</div>
							</td>
						</tr>					
						<tr>
							<th scope="row" colspan="2"><span class="colorPoint">*</span><ikep4j:message pre="${uiPrefix}" key="roleType" /></th>
							<td><div>
								<select id="roleTypeId" name="roleTypeId" title="<ikep4j:message pre="${uiPrefix}" key="roleType" />" style="width:30%;" >
									<c:forEach var="result" items="${roleTypeList}" varStatus="loopStatus">
										<option value="<c:out value="${result.roleTypeId}"/>" <c:if test="${result.roleTypeId==role.roleTypeId}">selected</c:if>>
											<c:out value="${result.roleTypeName}"/>
										</option>
									</c:forEach>
								</select></div>
							</td>
