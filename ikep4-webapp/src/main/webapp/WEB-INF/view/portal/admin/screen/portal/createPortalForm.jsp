...
<c:set var="preAlert" value="message.portal.admin.screen.portal.createPortalForm"/>
<c:set var="prefix" value="ui.portal.admin.screen.portal.createPortalForm"/>
...
(function($) {
    ...
    $jq(document).ready(function() {
		...
		$jq("#saveButton").click(function() {
			...
			$jq("#createPortalForm").submit();
		});
		
		$jq("#sharePortalId").change(function() {
			if(this.value == '') {
				$jq("#resourceTr").hide();	
			} else {
				$jq("#resourceTr").show();
		...
		var validOptions = {
			...
			submitHandler : function(form) {
				...
				*/
				...
				$jq.ajax({
					url : "<c:url value='/portal/admin/screen/portal/createPortal.do'/>",
					type : "post",
		...
		var validator = new iKEP.Validator("#createPortalForm", validOptions);
	...
	disableTarget = function(value) {
		...
		if(adminCount < 3 && ($jq("#createUserId").val() == null || $jq("#createUserId").val() == '')) {
...
<form id="createPortalForm" method="post" action="">
			...
			<tbody>
				<tr>
					...
					<td width="82%">
						...
						<input type="text" id="${status.expression}" name="${status.expression}" class="inputbox w100" value="" title="<ikep4j:message pre='${prefix}' key='portalName'/>"/>
				...
				<tr>
					...
					<td width="82%">
						...
						<input type="text" id="${status.expression}" name="${status.expression}" class="inputbox w100"  value="" title="<ikep4j:message pre='${prefix}' key='description'/>"/>
						<label for="${status.expression}" class="serverError">${status.errorMessage}</label>
						</spring:bind>
						</div>			
					</td>
				...
				<tr>
					...
					<td width="82%">
						...
						<input type="text" id="${status.expression}" name="${status.expression}" class="inputbox w100" value="" title="<ikep4j:message pre='${prefix}' key='portalHost'/>"/>
						<label for="${status.expression}" class="serverError">${status.errorMessage}</label>
						</spring:bind>
						</div>							
					</td>
				...
				<tr>
					...
					<td width="82%">
						...
						<input type="text" id="${status.expression}" name="${status.expression}" class="inputbox w100" value="" title="<ikep4j:message pre='${prefix}' key='portalHostAlias'/>"/>
						<label for="${status.expression}" class="serverError">${status.errorMessage}</label>
						</spring:bind>
						</div>		
				...
				<tr style="display:none">
					...
					<td width="82%">
						...
						<input type="text" id="${status.expression}" name="${status.expression}" class="inputbox w100" value="/" title="<ikep4j:message pre='${prefix}' key='portalPath'/>"/>
						<label for="${status.expression}" class="serverError">${status.errorMessage}</label>
						</spring:bind>
						</div>					
					</td>
				...
				<tr>
					...
					<td width="82%">
						<label><input type="radio" class="radio" id="defaultPortal" name="defaultOption" value="1" checked="checked" title="<ikep4j:message pre='${prefix}' key='defaultPortal'/>"/><ikep4j:message pre='${prefix}' key='defaultPortal'/></label>&nbsp;
						<label><input type="radio" class="radio" id="noDefaultPortal" name="defaultOption" value="0" title="<ikep4j:message pre='${prefix}' key='noDefaultPortal'/>"/><ikep4j:message pre='${prefix}' key='noDefaultPortal'/></label>								
					</td>
				</tr>
				<tr>
					<th width="18%" scope="row"><ikep4j:message pre='${prefix}' key='isUse'/></th>
					<td width="82%">
						<label><input type="radio" class="radio" name="active" value="1" checked="checked" title="<ikep4j:message pre='${prefix}' key='use'/>" onclick="disableTarget(1);" /><ikep4j:message pre='${prefix}' key='use'/></label>&nbsp;
						<label><input type="radio" class="radio" name="active" value="0" title="<ikep4j:message pre='${prefix}' key='noUse'/>" onclick="disableTarget(0);" /><ikep4j:message pre='${prefix}' key='noUse'/></label>								
					</td>
				</tr>
				<tr>
					<th width="18%" scope="row"><ikep4j:message pre='${prefix}' key='cacheYn'/></th>
					<td width="82%">
						<label><input type="radio" class="radio" name="cacheYn" value="Y" checked="checked" title="<ikep4j:message pre='${prefix}' key='use'/>" /><ikep4j:message pre='${prefix}' key='use'/></label>&nbsp;
						<label><input type="radio" class="radio" name="cacheYn" value="N" title="<ikep4j:message pre='${prefix}' key='noUse'/>" /><ikep4j:message pre='${prefix}' key='noUse'/></label>								
					</td>
				</tr>
				<tr>
					...
					<td width="82%">
						<select name="defaultLocaleCode" title="<ikep4j:message pre='${prefix}' key='defaultLocale'/>">
							<c:forEach var="localeCode" items="${localeCodeList}">
								<option value="${localeCode.localeCode}">${localeCode.localeName}</option>	
							</c:forEach>
						</select>						
					</td>
				</tr>
				<tr>
					<th width="18%" scope="row"><ikep4j:message pre='${prefix}' key='resourceManagement'/></th>
					<td width="82%">
						<select name="sharePortalId" id="sharePortalId">
							<option value="">- <ikep4j:message pre='${prefix}' key='select'/> -</option>
							<c:forEach var="portalItem" items="${portalList}">
								<option value="${portalItem.portalId}">${portalItem.portalName}</option>	
							</c:forEach>
						</select>
						<span class="colorPoint">※ <ikep4j:message pre='${prefix}' key='selectMessage'/></span>						
					</td>
				</tr>
				<tr id="resourceTr" style="display: none;">
					<th width="18%" scope="row"><ikep4j:message pre='${prefix}' key='resourceSelect'/></th>
					<td width="82%">
						<input type="checkbox" id="sharePortlet" name="sharePortlet" class="checkbox valign_middle" title="<ikep4j:message pre='${prefix}' key='checkBox1'/>" value="Y"/> <ikep4j:message pre='${prefix}' key='checkBox1'/>
						<input type="checkbox" id="shareMenu" name="shareMenu" class="checkbox valign_middle" title="<ikep4j:message pre='${prefix}' key='checkBox2'/>" value="Y"/> <ikep4j:message pre='${prefix}' key='checkBox2'/>
						<input type="checkbox" id="shareQuickMenu" name="shareQuickMenu" class="checkbox valign_middle" title="<ikep4j:message pre='${prefix}' key='checkBox3'/>" value="Y"/> <ikep4j:message pre='${prefix}' key='checkBox3'/>
						<input type="checkbox" id="shareSystem" name="shareSystem" class="checkbox valign_middle" title="<ikep4j:message pre='${prefix}' key='checkBox4'/>" value="Y"/> <ikep4j:message pre='${prefix}' key='checkBox4'/>
						<input type="checkbox" id="shareRole" name="shareRole" class="checkbox valign_middle" title="<ikep4j:message pre='${prefix}' key='checkBox5'/>" value="Y"/> <ikep4j:message pre='${prefix}' key='checkBox5'/>
						<input type="checkbox" id="shareRoleType" name="shareRoleType" class="checkbox valign_middle" title="<ikep4j:message pre='${prefix}' key='checkBox6'/>" value="Y"/> <ikep4j:message pre='${prefix}' key='checkBox6'/>
						<input type="checkbox" id="shareJobTitle" name="shareJobTitle" class="checkbox valign_middle" title="<ikep4j:message pre='${prefix}' key='checkBox7'/>" value="Y"/> <ikep4j:message pre='${prefix}' key='checkBox7'/>
						<input type="checkbox" id="shareJobPosition" name="shareJobPosition" class="checkbox valign_middle" title="<ikep4j:message pre='${prefix}' key='checkBox8'/>" value="Y"/> <ikep4j:message pre='${prefix}' key='checkBox8'/>
						<input type="checkbox" id="shareJobRank" name="shareJobRank" class="checkbox valign_middle" title="<ikep4j:message pre='${prefix}' key='checkBox9'/>" value="Y"/> <ikep4j:message pre='${prefix}' key='checkBox9'/>
						<input type="checkbox" id="shareJobClass" name="shareJobClass" class="checkbox valign_middle" title="<ikep4j:message pre='${prefix}' key='checkBox10'/>" value="Y"/> <ikep4j:message pre='${prefix}' key='checkBox10'/>
						<input type="checkbox" id="shareJobDuty" name="shareJobDuty" class="checkbox valign_middle" title="<ikep4j:message pre='${prefix}' key='checkBox11'/>" value="Y"/> <ikep4j:message pre='${prefix}' key='checkBox11'/><br/>
						<span class="colorPoint">※ <ikep4j:message pre='${prefix}' key='checkBoxMessage1'/></span><br/>
						<span class="colorPoint">※ <ikep4j:message pre='${prefix}' key='checkBoxMessage2'/></span>						
					</td>
				</tr>
				<tr>
