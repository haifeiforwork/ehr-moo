...
<c:set var="jobPositionUiPrefix" value="ui.portal.admin.code.jobPosition"/>
				...
				//<!--
					...
					function getView(jobPositionCode, fieldName, itemTypeCode, tr) {
					...
					function checkCode() {
						...
						if($jq("#jobPositionCode").val()=='') {
							alert("<ikep4j:message pre="${jobPositionListPrefix}" key="alert.noCheckValue" />");
						} else {
							$jq.ajax({
								url : '<c:url value="checkCode.do" />',     
								data : {code: $jq("#jobPositionCode").val()},     
								type : "post",
								success : function(result) {     
									
									if(result == 'duplicated') {
										alert("<ikep4j:message pre="${jobPositionListPrefix}" key="alert.isDuplicated" />");
										$jq("#codeCheck").val("modify");
									} else {
										alert("<ikep4j:message pre="${jobPositionListPrefix}" key="alert.isAvailable" />");
					...
					function deleteForm() {
						if ($jq("#jobPositionCode").val() == "") {
							alert("<ikep4j:message pre="${jobPositionListPrefix}" key="alert.cantDelete" />");
							return;
						}
						
						if(confirm("<ikep4j:message pre="${jobPositionListPrefix}" key="confirm.wannaDelete" />")) {
							$jq.ajax({
								url : '<c:url value="deleteJobPosition.do" />',
								data : $jq("#jobPosition").serialize(),
								type : "post",
								success : function(result) {
									alert("<ikep4j:message pre="${jobPositionListPrefix}" key="alert.deleteCompleted" />");
									getList();
								},
								error:function(){
									alert("<ikep4j:message pre="${jobPositionListPrefix}" key="alert.failedToDelete" />");
								}
							});
						} else {
							return;
					...
					// 순서를 위로
					function setUp(jobPositionCode, sortOrder) {
						$jq.ajax({     
							url : '<c:url value="goUp.do" />',     
							data : {code: jobPositionCode, sortOrder: sortOrder},     
							type : "post",     
							success : function(result) {
								getList();
							} 
					...
					// 순서를 아래로
					function setDown(jobPositionCode, sortOrder) {
							...
							<div id="listDiv">
								<form id="searchForm" name="searchForm" method="post" onsubmit="getList(); return false;" action="">
									...
									<div class="tableTop">
										...
										<h2><ikep4j:message pre="${jobPositionUiPrefix}" key="pageTitle" /></h2>
										<div class="listInfo"> 
											...
											<spring:bind path="searchCondition.searchColumn">  
												<select name="${status.expression}" title='<ikep4j:message pre='${preSearch}' key='${status.expression}' />'>
													<option value="code" <c:if test="${'code' eq status.value}">selected="selected"</c:if> >
														<ikep4j:message pre="${jobPositionUiPrefix}" key="jobPositionCode" />
													</option>
													<option value="title" <c:if test="${'title' eq status.value}">selected="selected"</c:if> >
														<ikep4j:message pre="${jobPositionUiPrefix}" key="jobPositionName" />
													</option>
												</select>	
											</spring:bind>		
...
<%-- 													<a onclick="javascript:sort('SORT_ORDER', '<c:if test="${searchCondition.sortColumn eq 'SORT_ORDER'}">${searchCondition.sortType}</c:if>');"  href="#a"> --%>
													<ikep4j:message pre="${jobPositionUiPrefix}" key="num" />
<%-- 													</a> --%>
												</th>
												<th scope="col">
<%-- 													<a onclick="javascript:sort('JOB_POSITION_CODE', '<c:if test="${searchCondition.sortColumn eq 'JOB_POSITION_CODE'}">${searchCondition.sortType}</c:if>');"  href="#a"> --%>
													<ikep4j:message pre="${jobPositionUiPrefix}" key="jobPositionCode" />
<%-- 													</a> --%>
												</th>
												<th scope="col">
<%-- 													<a onclick="javascript:sort('JOB_POSITION_NAME', '<c:if test="${searchCondition.sortColumn eq 'JOB_POSITION_NAME'}">${searchCondition.sortType}</c:if>');"  href="#a"> --%>
													<ikep4j:message pre="${jobPositionUiPrefix}" key="jobPositionName" />
<%-- 													</a> --%>
												</th>
												<th scope="col">
<%-- 													<a onclick="javascript:sort('JOB_POSITION_ENGLISH_NAME', '<c:if test="${searchCondition.sortColumn eq 'JOB_POSITION_ENGLISH_NAME'}">${searchCondition.sortType}</c:if>');"  href="#a"> --%>
													<ikep4j:message pre="${jobPositionUiPrefix}" key="jobPositionEnglishName" />
<%-- 													</a> --%>
												</th>
												<th scope="col"><ikep4j:message pre='${jobPositionUiPrefix}' key='sortOrder' /></th>
											</tr>
										</thead>
										<tbody>
											<c:choose>
												<c:when test="${searchResult.emptyRecord}">
													<tr>
														<td colspan="5" class="emptyRecord"><ikep4j:message pre='${jobPositionListPrefix}' key='emptyRecord' /></td> 
													</tr>				        
											    </c:when>
											    <c:otherwise>
													<c:forEach var="jobPosition" items="${searchResult.entity}" varStatus="status">
														<tr>
															<td class="textCenter">${jobPosition.num}</td>
															<td class="textCenter"><a href="#a" onclick="getView('${jobPosition.jobPositionCode}', 'jobPositionName', 'PO', this)">${jobPosition.jobPositionCode}</a></td>
															<td class="textCenter"><a href="#a" onclick="getView('${jobPosition.jobPositionCode}', 'jobPositionName', 'PO', this)">${jobPosition.jobPositionName}</a></td>
															<td class="textCenter"><a href="#a" onclick="getView('${jobPosition.jobPositionCode}', 'jobPositionName', 'PO', this)">${jobPosition.jobPositionEnglishName}</a></td>
															<td class="textCenter">
																<a href="#" onclick="setUp('${jobPosition.jobPositionCode}', '${jobPosition.sortOrder}')">
																	<img src="<c:url value='/base/images/icon/ic_tablesort_up.gif' />" alt="UP" />
																</a>
																<a href="#" onclick="setDown('${jobPosition.jobPositionCode}', '${jobPosition.sortOrder}')">
					...
					<div class="subTitle_2 noline">
						<h3><ikep4j:message pre="${jobPositionUiPrefix}" key="subTitle.detail" /></h3>
					</div>
					<!--//subTitle_2 End--> 
					...
					<div class="blockButton">
						<ul>
							<li><a class="button" id="newFormButton" href="#a"><span><ikep4j:message pre="${jobPositionUiPrefix}" key="button.new" /></span></a></li>
							<li><a class="button" id="submitButton" href="#a"><span><ikep4j:message pre="${jobPositionUiPrefix}" key="button.save" /></span></a></li>
							<li><a class="button" id="deleteButton" href="#a"><span><ikep4j:message pre="${jobPositionUiPrefix}" key="button.delete" /></span></a></li>
						</ul>
					</div>
					<!--//blockButton End-->