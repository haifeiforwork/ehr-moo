			...
			function(){
				$jq.ajax({
					...
					success : function(data) {
						if(data=="success") { 
							alert("비밀번호가 a1234567 로 초기화 되었습니다.\n기능직 외의 사용자는 SAP비밀번호를 동일하게 변경해 주십시요.");
						} else {
							alert("error");
						}
					},
...
//-->
				...
				<input type="hidden" name="passwordChangeYn" id="passwordChangeYn" value="" />
						...
						<table summary="<ikep4j:message pre='${userUiPrefix}' key='tableSummary' />">
							...
							<tbody>
								<tr>
									...
									<td class="textLeft" width="35%">
									<a class="button" id="newPassword"><span><ikep4j:message pre="${userUiPrefix}" key="button.initPassword" /></span></a>
									</td>
								...
								<tr>
                                    ...
                                    <td class="textLeft" width="35%">
                                        <div>
                                        <input type="text" id="birthday" name="birthday" class="inputbox datepicker" title="<ikep4j:message pre="${userUiPrefix}" key="birthday" />" style="width:30%" value="${user.birthday}" readonly="readonly" class="inputbox" />
								...
								<tr>
									...
									<td class="textLeft" id="tdPasswordChangeYn">
										<input type="checkbox" name = "passwordChangeYnCheck" id = "passwordChangeYnCheck" />
									</td>
								</tr>
									...
									<td class="textLeft" width="35%">
										<select id="timezoneId" name="timezoneId" title="<ikep4j:message pre='${userUiPrefix}' key='timezone' />" style="width:100%;" >
											<c:forEach var="result" items="${timezoneList}" varStatus="loopStatus">
												<option value="<c:out value="${result.timezoneId}"/>" <c:if test='${result.timezoneId=="GMT000000009"}'>selected</c:if>>
													<c:out value="${result.timezoneName}"/>
												</option>
											</c:forEach>
										</select>
									</td>
								...
								<tr>
									...
									<td class="textLeft" width="35%">										
										<input type="hidden" name="mailPassword" value="${user.mailPassword}" />
									</td>
								</tr>								
								<tr>
								...
								<tr><!-- 직책 미입력시 에러나서 필수값으로 함. 무림제지 2012.8.27 -->
									<th scope="col" width="15%"><span class="colorPoint">*</span> <ikep4j:message pre="${userUiPrefix}" key="jobduty" /></th>
									<td class="textLeft" width="35%">
										<select id="jobDutyCode" name="jobDutyCode" title="<ikep4j:message pre='${userUiPrefix}' key='jobduty' />" style="width:100%;" >
											...
											<c:forEach var="result" items="${jobDutyList}" varStatus="loopStatus">
												<c:choose>
													<c:when test="${userLocaleCode == 'ko'}">
														<option value="<c:out value="${result.jobDutyCode}"/>" <c:if test="${result.jobDutyCode==null}">selected</c:if>>
									...
									<td class="textLeft" width="35%">
										<select id="jobClassCode" name="jobClassCode" title="<ikep4j:message pre='${userUiPrefix}' key='jobclass' />" style="width:100%;" >
											<c:forEach var="result" items="${jobClassList}" varStatus="loopStatus">
												<c:choose>
													<c:when test="${userLocaleCode == 'ko'}">
														<option value="<c:out value="${result.jobClassCode}"/>" <c:if test="${result.jobClassCode=='S0005'}">selected</c:if>>
                    ...
                    <input type="hidden" id="userStatus" name="userStatus" value="C" />						
...
<%-- 					<input type="hidden" id="leadingGroupId" name="leadingGroupId" value="${user.leadingGroupId}" /> --%>
					<input type="hidden" id="nationCode" name="nationCode" value="KOR"/>
					<!--blockButton Start-->
