...
<c:set var="prefix" value="message.portal.admin.screen.systemAdmin.updateSystemAdminForm"/>
...
(function($) {
	$jq(document).ready(function() {
	    ...
	    $jq('#saveButton').click(function() {
			...
			$jq("#searchForm").attr("action", "<c:url value='/portal/admin/screen/systemAdmin/updateSystemAdmin.do'/>");
			$jq("#searchForm")[0].submit();
		});
		
	 	//className,resourceName,operationName,"권한설정 옵션"
		iKEP.loadSecurity("${searchCondition.resourceName}","${searchCondition.resourceName}","MANAGE","User,Group,ExpUser", 25);
	...
	readItemView = function (resourceName) {
		...
		$jq("#searchForm").attr("action", "<c:url value='/portal/admin/screen/systemAdmin/readSystemAdmin.do'/>");
	    $jq("#searchForm")[0].submit();
	};
...
<div class="tableTop">
	...
	<div class="listInfo">
		<spring:bind path="searchCondition.pagePerRecord">  
			<select name="${status.expression}" title="<ikep4j:message pre="${prefix}" key="${status.expression}" />" onchange="updateForm();">
		...
		<div class="totalNum">
			${searchResult.pageIndex}/${searchResult.pageCount}<ikep4j:message pre="${prefix}" key="page" /> (<ikep4j:message pre="${prefix}" key="info" /><span>${searchResult.recordCount}</span>)
		</div>
	</div>					
	<div class="tableSearch">
		...
		<a class="ic_search" id="searchBoardItemButton" name="searchBoardItemButton" href="#a" onclick="updateForm();">
	...
	<div>
		...
		<div class="blockListTable">					
			...
			<spring:bind path="searchCondition.pageIndex">
				<ikep4j:pagination searchFormId="searchForm" ajaxEventFunctionName="updateForm" pageIndexInput="${status.expression}" searchCondition="${searchCondition}" />
				<input name="${status.expression}" type="hidden" value="${status.value}" title="<ikep4j:message pre="${prefix}" key="pageIndex" />" />
			</spring:bind> 
			<!--//Page Number End-->
		</div>
		<!--//blockListTable End-->
