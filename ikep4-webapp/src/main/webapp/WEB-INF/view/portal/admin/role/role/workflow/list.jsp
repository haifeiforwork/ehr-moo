...
<c:set var="uiPrefix" value="ui.portal.admin.role.role"/>
				...
				//<!--
					...
					function getView(roleId) {
						...
						var searchForm = document.getElementById("searchForm");
						searchForm.action = '<c:url value="form.do" />';
						searchForm.submit();
					}
						...
						<div id="listDiv">
							<form id="searchForm" name="searchForm" method="post" onsubmit="return false;" action="">
								...
								<div class="tableTop">
									...
									<h2><ikep4j:message pre="${uiPrefix}" key="list.pageTitle" /></h2>
									<div class="listInfo"> 
									...
									<div class="tableSearch"> 
										<spring:bind path="searchCondition.searchColumn">  
											<select name="${status.expression}" title='<ikep4j:message pre='${preSearch}' key='${status.expression}' />'>
												...
												<option value="title" <c:if test="${'title' eq status.value}">selected="selected"</c:if> >
													<ikep4j:message pre="${uiPrefix}" key="roleName" />
												</option>
											</select>	
										</spring:bind>		
										...
										<a href="#a" id="searchBoardItemButton" name="searchBoardItemButton" onclick="return false;" class="ic_search">
									...
									<thead>
										<tr>
											...
											<th scope="col">
												...
												<ikep4j:message pre="${uiPrefix}" key="description" />
												</a>
											</th>
											<th scope="col">
									...
									<tbody>
										<c:choose>
											<c:when test="${searchResult.emptyRecord}">
												<tr>
													<td colspan="5" class="emptyRecord"><ikep4j:message pre='${messagePrefix}' key='empty' /></td> 
												</tr>				        
										    </c:when>
										    <c:otherwise>
												<c:forEach var="role" items="${searchResult.entity}" varStatus="status">
													<tr>
														...
														<td class="textCenter">${role.roleName}</td>
														<td class="textLeft">${role.description}</td>
