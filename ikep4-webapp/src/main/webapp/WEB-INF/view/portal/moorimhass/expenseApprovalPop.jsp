	...
	$(document).ready(function(){
		...
		$("#btnApprove, #btnReject").click(function(){
			...
			if(process){
				...
				$jq.ajax({
					url : "<c:url value='/portal/moorimhass/expenseOnInput.do'/>",
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<tbody>
				...
				<tr>
					...
					<td class="list03_td">
						<fmt:parseDate var="dateString" value="${exList.REQDT}" pattern="yyyyMMdd" />
						<fmt:formatDate value="${dateString}" pattern="yyyy-MM-dd" />
					</td>
					...
					<td class="list03_td">
						...
						<c:forEach items="${resultCode.ET_REQGN }" var="result">
							<c:if test="${result.KEY eq  exList.REQGN}">
				...
				<tr>
					<th class="list03_td_bg">부임/파견일</th>
					<td class="list03_td">
						<fmt:parseDate var="dateString" value="${exList.ZBUDT}" pattern="yyyyMMdd" />
						<fmt:formatDate value="${dateString}" pattern="yyyy-MM-dd" />
					</td>
					<th class="list03_td_bg">이사일자</th>
					<td class="list03_td">
						<fmt:parseDate var="dateString" value="${exList.ZMODT}" pattern="yyyyMMdd" />
						<fmt:formatDate value="${dateString}" pattern="yyyy-MM-dd" />
					</td>
				</tr>
				<tr>
					...
					<td class="list03_td">
						...
						<c:forEach items="${resultCode.ET_MOVTY }" var="result">
							<c:if test="${result.KEY eq  exList.MOVTY}">
								<c:set var="VALUE" value="${result.VALUE }"/>
							</c:if>
						</c:forEach>
						<c:out value="${VALUE }"/>
					</td>
					<th class="list03_td_bg">숙소구분</th>
					<td class="list03_td">
						<c:set var="VALUE" value=""/>

						<c:forEach items="${resultCode.ET_SLEEP }" var="result">
							<c:if test="${result.KEY eq  exList.SLEEP}">
								<c:set var="VALUE" value="${result.VALUE }"/>
							</c:if>
						</c:forEach>
						<c:out value="${VALUE }"/>
				...
				<tr>
					...
					<td class="list03_td">
						...
						<c:forEach items="${resultCode.ET_MARRY }" var="result">
							<c:if test="${result.KEY eq  exList.MARRY}">
								<c:set var="VALUE" value="${result.VALUE }"/>
							</c:if>
						</c:forEach>
						<c:out value="${VALUE }"/>
				...
				<tr>
					...
					<td class="list03_td f_right">
						<fmt:formatNumber value="${exList.MOVMT}" groupingUsed="true"/>
					</td>
					<td class="list03_td" colspan="2"></td>
				</tr>
			</tbody>
