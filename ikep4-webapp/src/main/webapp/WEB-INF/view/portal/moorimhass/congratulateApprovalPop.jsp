	...
	$(document).ready(function(){
		...
		$("#btnApprove, #btnReject").click(function(){
			...
			if(process){
				...
				$jq.ajax({
					url : "<c:url value='/portal/moorimhass/congratulateOnInput.do'/>",
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<tbody>
				...
				<tr>
					...
					<td class="list03_td">
						<c:out value="${exList.ZCACRN }"/>
					</td>
					<th class="list03_td_bg">대상자명</th>
					<td class="list03_td">
						<c:out value="${exList.ENAME }"/>
					</td>
				</tr>
				<tr>
					<th class="list03_td_bg">경조기간</th>
					<td class="list03_td">
						<fmt:parseDate var="dateString" value="${exList.BEGDT}" pattern="yyyyMMdd" />
						<fmt:formatDate value="${dateString}" pattern="yyyy-MM-dd" />
						&nbsp;~&nbsp;
						<fmt:parseDate var="dateString" value="${exList.ENDDT}" pattern="yyyyMMdd" />
						<fmt:formatDate value="${dateString}" pattern="yyyy-MM-dd" />
					</td>
					<th class="list03_td_bg">휴가일수</th>
					<td class="list03_td">
						<c:out value="${exList.ZVACDT }"/>
					</td>
				</tr>
				<tr>
					<th class="list03_td_bg">경조금</th>
					<td class="list03_td f_right">
						<fmt:formatNumber value="${exList.CACMT}" groupingUsed="true"/>
					</td>
					<th class="list03_td_bg">화환(조화)지급</th>
					<td class="list03_td">
						<c:out value="${exList.ZGARLD }"/>
					</td>
				</tr>
			</tbody>
		</table>
