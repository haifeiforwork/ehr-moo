	...
	$(document).ready(function(){
			...
			$jq.ajax({
				...
				success : function(result) {
					if($.trim(result.EX_RESULT) == "S"){
						...
						$("#ajaxForm").attr("action", "<c:url value='/portal/moorimess/personalMng/certificateEmpRegView.do'/>");
						$("#ajaxForm").submit();
					}else{
		...
		$("#btnDelete").click(function(){
			...
			if(confirm("정말로 삭제하시겠습니까?")){
				...
				$jq.ajax({
					url : "<c:url value='/portal/moorimess/personalMng/certificateEmpDelete.do'/>",
					data : $("#ajaxForm").serialize(),
					type : "post",
					async: false,
					success : function(result) {
...
<form id="certificateEmpForm" name="certificateEmpForm" method="post" action="<c:url value='/portal/moorimess/personalMng/certificateEmpList.do'/>">
<div id="wrap">
			...
			<tbody>
				<c:choose>
					<c:when test="${empty resultList}">
						<tr>
							<td colspan="10">재직증명서 신청 내역이 없습니다.</td>
						</tr>
					</c:when>
					<c:otherwise>
