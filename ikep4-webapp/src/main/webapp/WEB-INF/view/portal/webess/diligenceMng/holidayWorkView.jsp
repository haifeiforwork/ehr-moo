	...
	$(document).ready(function(){
		...
		$.initSet = function(){
			if("<c:out value="${params.dataLoad}"/>" == "Y"){
				...
				$("input[name=reason]").val("<c:out value="${params.PREAS}"/>");
				$("input[name=time]").val("<c:out value="${params.STIME}"/>");
		...
		$.setParam = function(eventId){
			...
			$("#ajaxForm").find("span.etcInfo").append("<input type=\"hidden\" name=\"reason\" value=\""+$("#holidayWorkForm").find("input[name=reason]").val()+"\"/>");
			$("#ajaxForm").find("span.etcInfo").append("<input type=\"hidden\" name=\"time\" value=\""+$("#holidayWorkForm").find("input[name=time]").val()+"\"/>");
...
<form id="holidayWorkForm" name="holidayWorkForm" method="post" action="" >
<div id="wrap">
			...
			<tbody>
				<tr>
					...
					<td class="list03_td">
						<fmt:parseDate var="dateString" value="${params.BEGDA}" pattern="yyyyMMdd" />
						<fmt:formatDate value="${dateString}" pattern="yyyy-MM-dd" />
						<input type="hidden" name="holidayWorkDate" id="holidayWorkDate" value="" />&nbsp;<span class="day"></span>
					</td>
