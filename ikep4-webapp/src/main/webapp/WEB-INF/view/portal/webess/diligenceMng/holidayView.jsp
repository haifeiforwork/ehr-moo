	...
	$(document).ready(function(){
		...
		$("#startDate, #endDate").change(function(){
			if($(this).val() != ""){
				$jq.ajax({
					...
					data : "startDate="+$("#startDate").val()+"&endDate="+$("#endDate").val()+"&holidayType="+$("input[name=holidayType]").val(),
					type : "post",
		...
		$.initSet = function(){
			if("<c:out value="${params.dataLoad}"/>" == "Y"){
				...
				$("input[name=holidayType]").val("<c:out value='${params.AWART}'/>");
				$("input[name=holidayTypeNm]").val("<c:out value='${params.ATEXT}'/>");
		...
		$.setParam = function(eventId){
			...
			$("#ajaxForm").find("span.etcInfo").append("<input type=\"hidden\" name=\"holidayType\" value=\""+$("#holidayForm").find("input[name=holidayType]").val()+"\"/>");
			$("#ajaxForm").find("span.etcInfo").append("<input type=\"hidden\" name=\"holidayTypeNm\" value=\""+$("#holidayForm").find("input[name=holidayTypeNm]").val()+"\"/>");
				...
				<tbody>
					<c:forEach var="appr" items="${resultMapList.ET_LINE}" varStatus="status">
						<tr>
							...
							<td <c:if test="${appr.EFLAG eq 'X' && params.exFlag eq 'C'}"> style="background-color:#eff7fb"</c:if>>
								<span class="APPNR">
									<c:out value="${appr.APPNR}"/>
								</span>
								<c:if test="${appr.EFLAG eq 'X' && params.exFlag eq 'C' }">
			...
			<tbody>
				<tr>
					...
					<td class="list03_td">
						...
						<input type="hidden" name="holidayType"   id="holidayType"   value="" />
						<input type="hidden" name="holidayTypeNm" id="holidayTypeNm" value=""/>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="list03_td">
						<c:out value="${params.DESC1 }"/>
						<input type="hidden" name="desc1" value=""/>
					</td>
				</tr>
				<tr>
					<td class="list03_td">
						<c:out value="${params.DESC2 }"/>
