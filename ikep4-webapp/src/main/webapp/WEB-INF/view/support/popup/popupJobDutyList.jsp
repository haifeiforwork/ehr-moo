...
<c:set var="preHeader"  value="ui.support.popup.jobduty.header" /> 
<c:set var="preList"    value="ui.support.popup.jobduty.list" />
<c:set var="preDetail"  value="ui.support.popup.jobduty.detail" />
					...
					<div class="tableTop">  
						...
						<div class="tableSearch"> 
							<spring:bind path="searchCondition.searchColumn">  
								<select name="${status.expression}" title='<ikep4j:message pre='${preSearch}' key='${status.expression}' />'>
									<option value="title" <c:if test="${'title' eq status.value}">selected="selected"</c:if> ><ikep4j:message pre='${preList}' key='jobDutyName' post=''/></option>
								</select>	
							</spring:bind>		
							...
							<tr>
								...
								<th scope="col" width="95%">
									<a onclick="javascript:sort('JOB_DUTY_NAME', '<c:if test="${searchCondition.sortColumn eq 'JOB_DUTY_NAME'}">${searchCondition.sortType}</c:if>');"  href="#a">
										<ikep4j:message pre='${preList}' key='jobDutyName' />
										<c:if test="${searchCondition.sortColumn eq 'JOB_DUTY_NAME'}">
							    ...
							    <c:otherwise>
									...
									<tr>
										<td>
											<c:choose>
							    				<c:when test="${isMulti == 'Y'}">
							    					<input type="checkbox" name="chk" value="${jobDuty.jobDutyCode}"/>
												</c:when>
							    				<c:otherwise>
							    					<input type="radio" name="chk" value="${jobDuty.jobDutyCode}"/>
							    				</c:otherwise>
							    			</c:choose>
							    			<input type="hidden" name="chk2" value="${jobDuty.jobDutyName}"/>
										</td>
										<td class="textLeft">
											<div class="ellipsis">${jobDuty.jobDutyName}</div>
										</td>
									</tr>
									</c:forEach>				      
							    </c:otherwise> 
