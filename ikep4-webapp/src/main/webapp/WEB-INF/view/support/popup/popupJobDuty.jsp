...
(function($) {
	...
	getList = function() {
		...
		$jq.ajax({     
			url : '<c:url value="getPopupJobDutyList.do" />?isMulti=${isMulti}',     
			...
			if($sel.length <1) {
				...
				return;	
			      ...
			      $jq("input[name=chk]").each(
			    		function() {
			    			if(this.checked) {
			    				data.push({jobDutyCode: this.value, jobDutyName: $jq("input[name=chk2]").get(i).value});
			    			}
			    			i++;
			    		}
			      );
