...
<c:set var="preHeader"  value="ui.support.popup.group.header" /> 
<c:set var="preList"    value="ui.support.popup.group.list" />
<c:set var="preDetail"  value="ui.support.popup.group.detail" />
	...
	$jq(document).ready(function() { 
		...
		$jq("#checkedAll").click(function() {
	 		...
	 		if ($jq("#checkedAll").is(":checked")) {
	 			$jq('input[name=groupId]:not(checked)').attr("checked", true);
			}else{
		   		$jq('input[name=groupId]:checked').attr("checked", false);
		    }
					...
					<div class="tableTop">  
						...
						<div class="tableSearch"> 
							<spring:bind path="searchCondition.searchColumn">  
								<select name="${status.expression}" title='<ikep4j:message pre='${preSearch}' key='${status.expression}' />'>
									<option value="title" <c:if test="${'title' eq status.value}">selected="selected"</c:if> ><ikep4j:message pre='${preList}' key='groupName' post=''/></option>
								</select>	
							</spring:bind>		
							    ...
							    <c:otherwise>
									...
									<tr>
										<td>
							    			...
							    			<input type="hidden" name="parentGroupId" value="${group.parentGroupId}"/>
										</td>
										<td class="textLeft">
