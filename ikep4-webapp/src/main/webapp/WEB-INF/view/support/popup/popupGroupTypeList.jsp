					...
					<div class="tableTop">  
						...
						<div class="tableSearch"> 
							<spring:bind path="searchCondition.searchColumn">  
								<select name="${status.expression}" title='<ikep4j:message pre='${preSearch}' key='${status.expression}' />'>
									<option value="title" <c:if test="${'title' eq status.value}">selected="selected"</c:if> ><ikep4j:message pre='${preList}' key='groupName2' post=''/></option>
									<option value="typetitle" <c:if test="${'typetitle' eq status.value}">selected="selected"</c:if> ><ikep4j:message pre='${preList}' key='groupTypeName' post=''/></option>
								</select>	
							</spring:bind>		
					...
					<table summary="<ikep4j:message pre='${preList}' key='main.title' />">
						...
						<thead>
							<tr>
								...
								<th scope="col" width="50%">
									<a onclick="javascript:sort('GROUP_NAME', '<c:if test="${searchCondition.sortColumn eq 'GROUP_NAME'}">${searchCondition.sortType}</c:if>');"  href="#a">
										<ikep4j:message pre='${preList}' key='groupName2' />
										<c:if test="${searchCondition.sortColumn eq 'GROUP_NAME'}">
								...
								<th scope="col" width="45%">
									<a onclick="javascript:sort('GROUP_TYPE_NAME', '<c:if test="${searchCondition.sortColumn eq 'GROUP_TYPE_NAME'}">${searchCondition.sortType}</c:if>');"  href="#a">
										<ikep4j:message pre='${preList}' key='groupTypeName' />
										<c:if test="${searchCondition.sortColumn eq 'GROUP_TYPE_NAME'}">
											<c:if test="${searchCondition.sortType eq 'DESC'}">
												<img src="<c:url value="/base/images/icon/ic_tablesort_up.gif" />" alt="up" />
											</c:if>
											<c:if test="${searchCondition.sortType eq 'ASC'}">
						...
						<tbody>
							...
							<c:choose>
							    ...
							    <c:otherwise>
									<c:forEach var="group" items="${searchResult.entity}" varStatus="status">
									<tr>
										<td>
											<c:choose>
							    				<c:when test="${isMulti == 'Y'}">
							    					<input type="checkbox" name="chk" value="${group.groupId}"/>
												</c:when>
							    				<c:otherwise>
							    					<input type="radio" name="chk" value="${group.groupId}"/>
							    				</c:otherwise>
							    			</c:choose>
							    			<input type="hidden" name="chk2" value="${group.groupName}"/>
							    			<input type="hidden" name="chk3" value="${group.groupTypeId}"/>
							    			<input type="hidden" name="chk4" value="${group.groupTypeName}"/>
										</td>
										<td class="textLeft">
											<div class="ellipsis">${group.groupName}</div>
										</td>
										<td class="textLeft">
											<div class="ellipsis">${group.groupTypeName}</div>
										</td>
									</tr>
									</c:forEach>				      
							    </c:otherwise> 
