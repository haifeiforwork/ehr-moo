...
(function($) {
	...
	getList = function() {
		...
		$jq.ajax({     
			url : '<c:url value="getPopupGroupList.do" />?isMulti=${isMulti}',     
	...
	$jq(document).ready(function() { 
		...
		$jq("#applyBtn").click(function() {
			...
			var $sel = $jq("input[name=groupId]:checked");
			if($sel.length <1) {
				...
				return;	
			      ...
			      var data = new Array();
			      var i=0;
			      $jq("input[name=groupId]").each(
			    		function() {
			    			if(this.checked) {
			    				data.push({type:"group", code: this.value, name:$(this).parent().next().find("div").text(), parent: $jq("input[name=parentGroupId]").get(i).value});
			    			}
			    			i++;
			    		}
			      );
