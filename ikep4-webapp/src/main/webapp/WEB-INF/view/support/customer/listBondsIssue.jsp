...
<form id="searchBoardItemForm"	method="post" action="<c:url value='/support/customer/customerBondsIssue/bondsIssueList.do'/>" >					
		      ...
		      <!--blockButton Start-->
                     ...
                     <div class="blockButton" style="position:absolute; right:0; bottom:0px; padding-top:10px;"> 
                         <ul>
                             <li><a class="button" href="<c:url value='/support/customer/customerBondsIssue/createBondsIssueView.do'/>"><span>등록</span></a></li>
                         </ul>
                     </div>
                    </c:if>
                        ...
                        <tbody>
                        	<c:choose>
								<c:when test="${searchResult.emptyRecord}">
									<tr>
										<td colspan="7"
											class="emptyRecord">등록된 이슈내역이 없습니다.
										</td>
									</tr>
								</c:when>
								<c:otherwise>
									<c:forEach var="bondsIssue" items="${searchResult.entity}"
										varStatus="status">
										<tr onmouseover="mouseOver(this);" onmouseout="mouseOut(this);">
											<%-- <td>${searchCondition.recordCount
												-((searchCondition.pageIndex - 1) *
												searchCondition.pagePerRecord) - status.index}</td> --%>
										    <td>${bondsIssue.counselDate}</td>
											<td>${bondsIssue.claimGubun}</td>
											<td><a href="<c:url value='/support/customer/customerBondsIssue/detailBondsIssue.do?seqNum=${bondsIssue.seqNum}'/>">${bondsIssue.customer}</a></td>
											<td class="textLeft"><a href="<c:url value='/support/customer/customerBondsIssue/detailBondsIssue.do?seqNum=${bondsIssue.seqNum}'/>">${bondsIssue.subject}&nbsp;
											<c:if test="${bondsIssue.linereplyCount > 0}"><span class="colorPoint">(${bondsIssue.linereplyCount})</span></c:if></a></td>
											
											<td>${bondsIssue.counselor}</td>
											<%-- <td>${bondsIssue.client}</td> --%>
											
											<td>${bondsIssue.factory}</td>
											<td>${bondsIssue.hitCount}</td>
										</tr>
									</c:forEach>
								</c:otherwise>
							</c:choose>																															
