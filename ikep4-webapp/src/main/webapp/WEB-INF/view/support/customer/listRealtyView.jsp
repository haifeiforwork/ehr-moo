	...
	(function($) {
		...
		.click(
				...
				$jq('#btn_excelUp').click(function(event) { 
					var url =  "<c:url value='/support/customer/customerRealty/realtyExcelForm.do'/>";			
					iKEP.popupOpen(url, {width:800, height:260 ,resizable:false}, 'realtyUpload');
				});
...
<form id="searchBoardItemForm"	method="post" action="<c:url value='/support/customer/customerRealty/realtyList.do'/>" >
    ...
    <div class="blockButton" style="position:absolute; right:0; bottom:0px; padding-top:10px;"> 
        <ul>
            <li><a class="button" href="<c:url value='/support/customer/customerRealty/createRealtyView.do'/>"><span>등록</span>
			</a></li>
        </ul>
    </div>
			...
			<caption></caption>
                     ...
                     <c:choose>
						<c:when test="${searchResult.emptyRecord}">
							<tr>
								<td colspan="4"
									class="emptyRecord">등록된 부동산정보가 없습니다.
								</td>
							</tr>
						</c:when>
						<c:otherwise>
							<c:forEach var="realty" items="${searchResult.entity}" varStatus="status">
								<tr onmouseover="mouseOver(this);" onmouseout="mouseOut(this);">
									<td><ikep4j:timezone date='${realty.registDate}'/></td>
									<td><a href="<c:url value='/support/customer/customerRealty/detailRealty.do?seqNum=${realty.seqNum}'/>">${realty.customerName}</a></td>
									<td>
									<div class="ellipsis"><a href="#a" onclick="iKEP.showUserContextMenu(this, '${realty.registerId}', 'bottom')">${realty.user.userName} </a></div>
									</td>
									<td>${realty.hitCount}</td>
								</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>	              																														
			 ...
			 <div class="blockButton"> 
				        <ul>
							...
							<li><a class="button" href="#a" id="btn_excelUp"><span>엑셀 업로드</span></a></li>
				        </ul>
				    </div>
				    <div class="blockBlank_10px"></div>	
