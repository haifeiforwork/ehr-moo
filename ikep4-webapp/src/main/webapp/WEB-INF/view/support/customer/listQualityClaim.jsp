...
<form id="searchBoardItemForm"	method="post" action="<c:url value='/support/customer/customerQualityClaim/qualityClaimList.do'/>" >					
		      ...
		      <!--blockButton Start-->
                     <%-- <div class="blockButton" style="position:absolute; right:0; bottom:0px; padding-top:10px;"> 
                         <ul>
                             <li><a class="button" href="<c:url value='/support/customer/customerQualityClaim/createQualityClaimView.do'/>"><span>등록</span></a></li>
                         </ul>
                     </div> --%>
					...
					<table summary="게시판설명">
                        ...
                        <tbody>
                        	<c:choose>
								...
								<c:otherwise>
									<c:forEach var="qualityClaim" items="${searchResult.entity}"
										varStatus="status">
										<tr onmouseover="mouseOver(this);" onmouseout="mouseOut(this);">
											<%-- <td>${searchCondition.recordCount
												-((searchCondition.pageIndex - 1) *
												searchCondition.pagePerRecord) - status.index}</td> --%>
										    <td>${qualityClaim.counselDate}</td>
											<td>${qualityClaim.claimGubun}</td>
											<td><a href="<c:url value='/support/customer/customerQualityClaim/detailQualityClaim.do?seqNum=${qualityClaim.seqNum}'/>">${qualityClaim.customer}</a></td>
											<td class="textLeft"><a href="<c:url value='/support/customer/customerQualityClaim/detailQualityClaim.do?seqNum=${qualityClaim.seqNum}'/>">${qualityClaim.subject}&nbsp;
											<c:if test="${qualityClaim.linereplyCount > 0}"><span class="colorPoint">(${qualityClaim.linereplyCount})</span></c:if></a></td>
											
											<td>${qualityClaim.counselor}</td>
											<td>${qualityClaim.client}</td>
											<td>${qualityClaim.factory}</td>
											<td>${qualityClaim.hitCount}</td>
										</tr>
									</c:forEach>
								</c:otherwise>
							</c:choose>																															
