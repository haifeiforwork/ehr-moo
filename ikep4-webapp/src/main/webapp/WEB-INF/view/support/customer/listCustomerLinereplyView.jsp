...
(function($){	 
	...
	*/	
	createLinereply = function() {    
		...
		$.post('<c:url value="/support/customer/customerLinereply/createLinereply.do"/>', $("#boardLinereplyForm").serialize())
	...
	*/
	createReplyLinereply = function(linereplyId) { 
		...
		$.post('<c:url value="/support/customer/customerLinereply/createReplyLinereply.do"/>', formObject.serialize())
		.success(function(data) { 
			loadBoardLinereplyList();
		})
		.error(function(event, request, settings) { alert("error"); $("#blockComment").ajaxLoadComplete();});
	...
	*/
	updateLinereply = function(linereplyId) { 
		...
		$.post('<c:url value="/support/customer/customerLinereply/updateLinereply.do"/>', formObject.serialize())
		.success(function(data) { 
			loadBoardLinereplyList();
		})
	...
	*/		
	userDeleteBoardLinereply = function(itemId, linereplyId) {
		...
		var itemType = $("#itemTypeTemp").val();
		if(confirm("<ikep4j:message pre='${preMessage}' key='delete' />")) { 
			$("#blockComment").ajaxLoadStart();
		    $.post("<c:url value='/support/customer/customerLinereply/userDeleteLinereply.do'/>", {"itemId" : itemId , "linereplyId" : linereplyId,"itemType": itemType}) 
		    .success(function(data) { 
		    	loadBoardLinereplyList(); 
		    })
		    .error(function(event, request, settings) { alert("error"); $("#blockComment").ajaxLoadComplete();});
	...
	*/	
	adminDeleteBoardLinereply = function(itemId, linereplyId) {
		...
		$("#blockComment").ajaxLoadStart();
		if(confirm("<ikep4j:message pre='${preMessage}' key='delete' />")) {
		    $.post("<c:url value='/support/customer/customerLinereply/adminDeleteLinereply.do'/>", {"itemId" : itemId , "linereplyId" : linereplyId, "itemType": itemType})  
		    .success(function(data) { 
		    	loadBoardLinereplyList(); 
		    })
		    .error(function(event, request, settings) { alert("error"); $("#blockComment").ajaxLoadComplete(); });
...
//-->
	...
	<div class="blockComment_t">
		<ikep4j:message pre='${preList}' key='linereply' /> <span class="comment_num"><c:if test="${not searchResult.emptyRecord}">(${linereplyCount})</c:if></span>  
	</div>  
		...
		<c:choose>
		   ...
		   <%--  <div class="blockCommentPhoto">  
				<c:choose>
					...
					<c:otherwise>  
						 <a href="#a" class="${varStatus.first ? 'firstLineItem' : ''}"><img src="<c:url value='${boardLinereply.profilePicturePath}'/>" alt="Image" onerror="this.src='<c:url value='/base/images/common/photo_50x50.gif'/>'" /></a>					 
					</c:otherwise> 	 
				</c:choose> 
			</div>	 --%>	    	
		    	...
		    	<span class="blockCommentInfo_name">  
							...
							<c:choose>
								<c:when test="${user.localeCode == portal.defaultLocaleCode}"> 
									<a href="#a" onclick="javascript:viewPopUpProfile('${boardLinereply.registerId}')">${boardLinereply.registerName} ${boardLinereply.jobTitleName}</a>
								</c:when>
								<c:otherwise> 
									<a href="#a" onclick="javascript:viewPopUpProfile('${boardLinereply.registerId}')">${boardLinereply.userEnglishName} ${boardLinereply.jobTitleEnglishName}</a>
								</c:otherwise>
							</c:choose>							 
		    	</span>
		    	<span>
		    		<c:choose>
						<c:when test="${user.localeCode == portal.defaultLocaleCode}"> 
							${boardLinereply.teamName}
						</c:when>
						<c:otherwise> 
							${boardLinereply.teamEnglishName}
						</c:otherwise>
					</c:choose>
		    	</span>
			...
			<c:choose>
				...
				<c:otherwise>
					...
					<div id="${boardLinereply.linereplyId}UpdateForm"  name="updateForm" class="blockComment_rewrite modify" style="display:none"> 
			...
			<div id="${boardLinereply.linereplyId}ReplyForm" name="replyForm" class="blockComment_rewrite" style="display:none">
				...
				<form name="replyForm" onsubmit="return false;">  
					<input name="linereplyParentId" type="hidden" value="${boardLinereply.linereplyId}" title="<ikep4j:message pre='${preList}'  key='linereplyParentId'/>" />			
					 <input type="hidden" id="itemTypeTemp" name="itemTypeTemp" value="${itemType}" />
