...
<form id="searchBoardItemForm" method="post"
	action="<c:url value='/support/customer/customerCommon/manInfo.do'/>">
			...
			<spring:bind path="searchCondition.searchColumn">
				<select name="${status.expression}"
					...
					<option value="company"
						<c:if test="${'company' eq status.value}">selected="selected"</c:if>>
						고객명
					</option>
					<option value="man"
						<c:if test="${'man' eq status.value}">selected="selected"</c:if>>
						<ikep4j:message pre='${prefix}' key='search.man' />
					</option>
					<option value="id"
						<c:if test="${'id' eq status.value}">selected="selected"</c:if>>
						고객ID						
					</option>						
				</select>
			</spring:bind>
	...
	<div class="tableDesciption" style="position:relative;">
     ...
     <!--blockButton Start-->
       <div class="blockButton" style="position:absolute; right:0; bottom:0px; padding-top:10px;"> 
           <ul>
               <li><a class="button" href="<c:url value='/support/customer/customerCommon/createManInfoView.do'/>"><span>등록</span></a></li>
           </ul>
       </div>
			...
			<thead>
				<tr>
					...
					<th scope="col" width="20%">
						<a onclick="sort('COMPANY', '<c:if test="${searchCondition.sortColumn eq 'COMPANY'}">${searchCondition.sortType}</c:if>');"  href="#a">
						고객명
						</a>
					<c:choose>
						<c:when test="${searchCondition.sortColumn eq 'COMPANY' and searchCondition.sortType eq 'DESC'}"><img alt="<ikep4j:message pre="${preSearch}" key="sortType" post="desc" />" src="<c:url value='/base/images/icon/ic_tablesort_down.gif'/>"></c:when>
						<c:when test="${searchCondition.sortColumn eq 'COMPANY' and searchCondition.sortType eq 'ASC'}"><img alt="<ikep4j:message pre="${preSearch}" key="sortType" post="asc" />" src="<c:url value='/base/images/icon/ic_tablesort_up.gif'/>"></c:when>
					</c:choose> 
					</th>
					<th scope="col" width="*%">
					...
					<c:otherwise>
						<c:forEach var="manInfoItem" items="${searchResult.entity}"
							...
							<tr onmouseover="mouseOver(this);" onmouseout="mouseOut(this);">
								...
								<td><a href="<c:url value='/support/customer/customerCommon/detailManInfo.do?seqNum=${manInfoItem.seqNum}'/>">${manInfoItem.customerName}</a></td>
								<td><a href="<c:url value='/support/customer/customerCommon/detailManInfo.do?seqNum=${manInfoItem.seqNum}'/>">${manInfoItem.man}</a></td>
								...
								<td><ikep4j:timezone date="${manInfoItem.registDate}"/></td>
								<td>${manInfoItem.hitCount}</td>
							</tr>
						</c:forEach>
					</c:otherwise>
				</c:choose>
