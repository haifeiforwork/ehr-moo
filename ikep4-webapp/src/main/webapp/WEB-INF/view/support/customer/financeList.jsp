		...
		$(document)
				.ready(
					...
					/* 	$("a.addButton").click(function(){
							...
							var url = iKEP.getContextRoot() + "/support/customer/customerFinance/popupBusinessNo.do";
							var options = {
								width:700,
								height:440,
								resizable:true,						 		
						...
						goAdd = function() {
							...
							var url = '<c:url value='/support/customer/customerFinance/modifyFinance.do'/>';
							var title = '고객정보 등록'
							var width = 780;
							var height = 580;
...
<form id="searchBoardItemForm"	method="post" action="<c:url value='/support/customer/customerFinance/financeList.do'/>" >					
	...
	<div class="tableDesciption" style="position:relative;">
		...
		<tbody>
	   	 ...
	   	 <div class="blockButton" style="position:absolute; right:0; bottom:0px; padding-top:10px;"> 
		        <ul>
		            <li><a class="button" href="<c:url value='/support/customer/customerFinance/modifyFinance.do'/>"><span>등록</span>
					</a></li>
		        </ul>
		 </div>
			...
			<caption></caption>
                     ...
                     <tbody>
	                     <c:choose>
							...
							<c:otherwise>
								<c:forEach var="finance" items="${searchResult.entity}" varStatus="status">
									<tr onmouseover="mouseOver(this);" onmouseout="mouseOut(this);">
										<td><ikep4j:timezone date='${finance.registDate}'/></td>
										<td><a href="<c:url value='/support/customer/customerFinance/detailFinance.do?id=${finance.id}'/>">${finance.name}</td>
										<td>
										<div class="ellipsis"><a href="#a" onclick="iKEP.showUserContextMenu(this, '${finance.registerId}', 'bottom')">${finance.registerName}</a></div>
										</td>
										<td>${finance.hitCount}</td>
									</tr>
								</c:forEach>
							</c:otherwise>
						</c:choose>	     																														
