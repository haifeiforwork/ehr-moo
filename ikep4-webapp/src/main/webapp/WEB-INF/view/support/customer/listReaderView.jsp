...
<div class="contentIframe">
	...
	<div class="blockListTable">
		<div id="listDiv">
			<form id="searchForm" name="searchForm" method="post"  action="<c:url value='/support/customer/customerBasicInfo/listReaderView.do'/>">
				...
				<spring:bind path="searchCondition.id">
					<input name="${status.expression}" type="hidden" value="${status.value}" />
				</spring:bind>
				<spring:bind path="searchCondition.divCode">
					<input name="${status.expression}" type="hidden" value="${status.value}" />
				</spring:bind>
					...
					<div class="tableSearch"> 
						<spring:bind path="searchCondition.searchColumn">  
							<select name="${status.expression}" title='<ikep4j:message pre='${preSearch}' key='${status.expression}' />'>
								<option value="READER_NAME" <c:if test="${'READER_NAME' eq status.value}">selected="selected"</c:if>>이름</option>
								<option value="READER_TEAM_NAME" <c:if test="${'READER_TEAM_NAME' eq status.value}">selected="selected"</c:if>>부서</option>
							</select>		
						</spring:bind>		
