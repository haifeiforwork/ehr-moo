...
<!--
	...
	(function($) {
		...
		$(document).ready( function() {
				...
				parent.setLicurrent('#counselHistory');
...
<form id="searchBoardItemForm"	method="post" action="<c:url value='/support/customer/customerCounselHistory/counselHistoryList.do'/>" >
			...
			<spring:bind path="searchCondition.searchColumn">
				<select name="${status.expression}"
					...
					<option value="customerId"
						<c:if test="${'customerId' eq status.value}">selected="selected"</c:if>>
						고객ID
					</option>
				</select>
			</spring:bind>
    ...
    <div class="blockButton" style="position:absolute; right:0; bottom:0px; padding-top:10px;"> 
        <ul>
            <li><a class="button" href="<c:url value='/support/customer/customerCounselHistory/createCounselHistoryView.do'/>"><span>등록</span>
			</a></li>
        </ul>
    </div>
		...
		<table summary="게시판설명">
			<caption></caption>
                     ...
                     <c:choose>
						...
						<c:otherwise>
							<c:forEach var="counselHistory" items="${searchResult.entity}" varStatus="status">
								...
								<%-- 	<td>${searchCondition.recordCount
									...
									<td><a href="<c:url value='/support/customer/customerCounselHistory/detailCounselHistory.do?seqNum=${counselHistory.seqNum}'/>">${counselHistory.customer}</a></td>
									<td class="textLeft"><a href="<c:url value='/support/customer/customerCounselHistory/detailCounselHistory.do?seqNum=${counselHistory.seqNum}'/>">${counselHistory.subject}&nbsp;
									<c:if test="${counselHistory.linereplyCount > 0}"><span class="colorPoint">(${counselHistory.linereplyCount})</span></c:if></a></td>
									
									<td>${counselHistory.counselor}</td>
									<td>${counselHistory.client}</td>
									<td>
									<div class="ellipsis"><a title="${counselHistory.registerName} ${counselHistory.user.jobTitleName} ${counselHistory.user.teamName}" href="#a" onclick="iKEP.showUserContextMenu(this, '${counselHistory.registerId}', 'bottom')">${counselHistory.user.userName} ${counselHistory.user.jobTitleName}</a></div>
									</td>
									<td>${counselHistory.hitCount}</td>									
								</tr>
							</c:forEach>
						</c:otherwise>
					</c:choose>	              																														
