...
<form id="QualityClaimForm" method="post" action="<c:url value='/support/customer/customerQualityClaim/modifyQualityClaim.do'/>" enctype="multipart/form-data" >
	<input type="hidden" name="seqNum" value="${qualityClaim.seqNum}" />
					...
					<table summary="상담이력 상세조회">
						...
						<tbody>
						   <tr>
								...
								<td colspan="5"><input name="subject" title="제목" value="${qualityClaim.subject}" class="inputbox w100" type="text" /></td>
							</tr>
							<tr>
								<spring:bind path="qualityClaim.counselDate">
                                <th scope="row"><span class="colorPoint">*</span>상담일</th>
								<td>
                                	<input name="counselDate" title="상담일" value="${qualityClaim.counselDate}"  class="inputbox datepicker" type="text" value="<ikep4j:timezone date='${qualityClaim.counselDate}'/>" size="15" /> 
								...
								<td><input name="counselor" title="상담자" value="${qualityClaim.counselor}"  class="inputbox w90" type="text" /></td>
								<th scope="row">영업담당자</th>
								<td><input name="salesman" title="영업담당자" value="${qualityClaim.salesman}"  class="inputbox w90" type="text" /></td>
							</tr>
							<tr>
                                <th scope="row"><span class="colorPoint">*</span>고객명</th>
								<td><input name="customer" title="고객명" value="${qualityClaim.customer}"  class="inputbox w100" type="text" /></td>
								<th scope="row"><span class="colorPoint">*</span>피상담자</th>
								<td><input name="client" title="피상담자"  value="${qualityClaim.client}"  class="inputbox w90" type="text" /></td>
								<th scope="row">연락처</th>
								<td><input name="clientPhone" title="연락처" value="${qualityClaim.clientPhone}"  class="inputbox w90" type="text" /></td>
							</tr>
                            <tr>
                                <th scope="row">가공처명</th>
								<td><input name="fabrication" title="가공처명" value="${qualityClaim.fabrication}"  class="inputbox w100" type="text" /></td>
								<th scope="row">담당자</th>
								<td><input name="charge" title="담당자" value="${qualityClaim.charge}"  class="inputbox w90" type="text" /></td>
								<th scope="row">연락처</th>
								<td><input name="chargePhone" title="연락처" value="${qualityClaim.chargePhone}"   class="inputbox w90" type="text" /></td>
							</tr>
							<tr>
								<th scope="row">지종</th>
								<td colspan="5"><input name="jijong" title="사용지종" value="${qualityClaim.jijong}"  class="inputbox w100" type="text" /></td>
							</tr>
							<tr>
								<th scope="row">평량</th>
								<td colspan="5"><input name="weight" title="사용지종" value="${qualityClaim.weight}"  class="inputbox w100" type="text" /></td>
							</tr>
                            <tr>
								<th scope="row"><span class="colorPoint">*</span>상담내용</th>
								<td colspan="5"><textarea name="content" class="w100" title="상담내용" cols="" rows="10">${qualityClaim.content}</textarea></td>
							</tr>
                            <tr>
								<th scope="row">조치사항 및<br />조치결과</th>
								<td colspan="5"><textarea name="actions"   class="w100" title="조치사항 및 조치결과" cols="" rows="10">${qualityClaim.actions}</textarea></td>
							</tr>
                            <tr>
								<th scope="row">클레임<br />발생현황</th>
								<td colspan="5"><textarea name="claims"  class="w100" title="클레임 발생현황" cols="" rows="10">${qualityClaim.claims}</textarea></td>
							</tr>
                            <tr>
								<th scope="row">설비현황</th>
								<td colspan="5"><textarea name="facilityHistory" class="w100" title="설비현황" cols="" rows="10">${qualityClaim.facilityHistory}</textarea></td>
							</tr>
                            
                            <tr>
								<th scope="row">주인쇄물</th>
								<td colspan="5"><input name="mainPrint" title="주인쇄물" value="${qualityClaim.mainPrint}"  class="inputbox w100" type="text" /></td>
							</tr>
                            <tr>
								<th scope="row">선호회사</th>
								<td colspan="5"><input name="mainCompany" title="선호회사" value="${qualityClaim.mainCompany}"  class="inputbox w100" type="text" /></td>
							</tr>
                            <tr>
								<th scope="row">주요거래처</th>
								<td colspan="5"><input name="mainConnection" title="주요거래처" value="${qualityClaim.mainConnection}"  class="inputbox w100" type="text" /></td>
							</tr>
                            <tr>
