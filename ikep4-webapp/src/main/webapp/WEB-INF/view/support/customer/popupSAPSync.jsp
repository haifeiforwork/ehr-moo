...
<!--
	...
	(function($) {
		...
		fnCaller = function (params, dialog) { // dialog Controll
			...
			sendData = function(sapId,id){
				
				if(id == undefined){
					id = "";
				}
				 var url = "<c:url value='/support/customer/customerBasicInfo/detailBasicInfoForSAP.do?sapId='/>"+sapId+"&id="+id;
				
				parent.mainFrame.contentIframe.location.href=url;
				dialog.close();
			}
...
<form id="searchBoardItemForm" method="post" action="<c:url value='/support/customer/customerBasicInfo/popupSAPSync.do'/>">
<input id="id" name="id" type="hidden" value="${id}"/>
		...
		<div class="tableSearch">
			<spring:bind path="searchCondition.searchColumn">
				<select name="${status.expression}"
					...
					<option value="sapId"
						<c:if test="${'sapId' eq status.value}">selected="selected"</c:if>>
						<%-- <ikep4j:message pre='${prefix}' key='search.businessEmployee' /> --%> SAP ID
					</option>
					<option value="regno"
						<c:if test="${'regno' eq status.value}">selected="selected"</c:if>>
						<%-- <ikep4j:message pre='${prefix}' key='search.businessEmployee' /> --%> 사업자 등록번호
					</option>
				</select>
			</spring:bind>
    ...
    <table summary="고객검색">
        ...
        <thead>
            <tr>            	
                ...
                <th scope="col" width="20%">SAP 고객ID</th>
				...
				<c:otherwise>
					<c:forEach var="basicInfo" items="${searchResult.entity}" varStatus="status">
						<tr onmouseover="mouseOver(this);" onmouseout="mouseOut(this);">
							...
							<td  class="textLeft"><a href="javascript:sendData(${basicInfo.sapId},${id});">${basicInfo.name}</a></td>
    ...
    <div class="directive"> 
        <ul>
            <li><span>등록하려는 고객을 검색하여 클릭하세요.</span></li>
            <li><span>SAP에 찾으려는 고객이 없으면 아래의 새로운 고객명을 입력하고 등록 버튼을 누르세요.</span></li>												
        </ul>
    </div>
    <div class="mb10"></div>
    <!--//directive End-->
