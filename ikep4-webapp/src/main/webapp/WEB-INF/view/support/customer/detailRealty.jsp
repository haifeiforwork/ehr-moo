...
(function($){ 
	...
	$jq(document).ready(function() {
		...
		for(i=0;i<tempno.length;i++){
			$jq("#realtyBody").append(
				"<tr>"+
					...
					"<td style=\"text-align:right;\">"+commanTrn(temppyeongSpace[i])+"</td>"+
					"<td style=\"text-align:right;\">"+commanTrn(temppyeongBuilding[i])+"</td>"+
					"<td style=\"text-align:right;\">"+commanTrn(tempmeterSpace[i])+"</td>"+
					"<td style=\"text-align:right;\">"+commanTrn(tempmeterBuilding[i])+"</td>"+
					"<td style=\"text-align:right;\">"+commanTrn(tempmeterSum[i])+"</td>"+
					"<td style=\"text-align:right;\">"+commanTrn(temptotalSum[i])+"</td>"+
					"<td>"+temprightDate[i]+"</td>"+
					"<td>"+tempcollateral[i]+"</td>"+
					"<td style=\"text-align:right;\">"+commanTrn(temprightSum[i])+"</td>"+
					"<td>"+tempdebt[i]+"</td>"+
					"<td>"+tempnote[i]+"</td>"+
				"</tr>"
			);
		...
		$("#createPrintButton").click(function() {			
			var url = iKEP.getContextRoot() + '/support/customer/customerRealty/detailRealtyPrint.do?seqNum=${realty.seqNum}';
		...
		$("#readerListViewButton").click( function() {  
			var url = "<c:url value='/support/customer/customerBasicInfo/listReaderView.do?id=${realty.seqNum}&divCode=RT'/>";
...
<div class="blockButton">
	<ul>
		<c:choose>
			<c:when test="${realty.registerId eq user.userId || isAdmin eq true}">

				<li><a id="createPrintButton" class="button" href="#a"><span>인쇄</span>
				</a>
				</li>
				<%--    <li><a class="button" href="<c:url value='/support/customer/customerRealty/modifyRealtyView.do?seqNum=${realty.seqNum}'/>"><span>수정</span></a></li>
				...
				<li><a class="button"
					href="<c:url value='/support/customer/customerRealty/realtyList.do'/>"><span>목록</span>
				</a>
				</li>
			</c:when>
			<c:otherwise>
...
<div class="blockButton">
	<ul>
		<c:choose>
			<c:when test="${realty.registerId eq user.userId || isAdmin eq true}">
				<li><a class="button"
					href="<c:url value='/support/customer/customerRealty/modifyRealtyView.do?seqNum=${realty.seqNum}'/>"><span>수정</span>
				</a>
				</li>
				<li><a class="button deleteButton" href="#a"><span>삭제</span>
				</a>
				</li>
				<li><a class="button"
					href="<c:url value='/support/customer/customerRealty/realtyList.do'/>"><span>목록</span>
