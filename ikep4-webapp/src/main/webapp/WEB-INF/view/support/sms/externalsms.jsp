...
$jq(document).ready(function() {	
	var tempSendTargets = "${sendTarget}".split(";");
	for(i=0;i<tempSendTargets.length;i++){
		appendPhone('','',tempSendTargets[i]);
	}
	

	//사용자아이디 클릭후 팝업호출시(favorite) 클릭된 사용자의 이름,핸드폰번호,아이디를 수신자 정보로 셋팅
	var paramName = '${searchCondition.paramName}';
	var paramMobile = filterDigit('${searchCondition.paramMobile}');
	var paramReceiverId = '${searchCondition.paramReceiverId}';
	...
	$jq("#btnSendList").click(function() {
		...
		$iframe.attr("src", "${contextRoot}/support/sms/listSms.do");		
...
function appendPhone(id, name, phoneNo) {
	...
	if(lastSpanId == '' || lastSpanId < 'no04' ) {
		...
		$jq("<td><a href='#a' onclick=\"del_receiverphoneno('"+paramId+"')\"><img src=\"${contextRoot}/base/images/icon/ic_sms_x.gif\" alt='삭제' /></a></td>").appendTo($newTr);		
	}
...
function sendSms(){
	...
	//new part
	iKEP.extSmsAdd('${user.userId}', receiverIds, receiverPhonenos, contents, "1", ${smsCount});
...
function configSms() {
	var url = '${contextRoot}/support/smsConfig/listSmsConfig.do';
...
<div id="sms">
	...
	<div class="smsLogo"><img src="${contextRoot}/base/images/common/logo_sms.png" alt="Moorim" /></div>
				...
				<table summary="<ikep4j:message pre='${preLabel}' key='summary' />">
					...
					<tbody id="ulSendList">
						<tr id="tr01">
							...
							<td width="10"><a href="#a" onclick="del_receiverphoneno('1')"><img src="${contextRoot}/base/images/icon/ic_sms_x.gif" alt="del" /></a></td>
						</tr>						
						<tr id="tr02">
							<td><input type="hidden" id="rd02" title="" />02</td>
							<td><span id="rn02"></span></td>
							<td><span id="no02"></span></td>
							<td><a href="#a" onclick="del_receiverphoneno('2')"><img src="${contextRoot}/base/images/icon/ic_sms_x.gif" alt="del" /></a></td>
						</tr>
						<tr id="tr03">
							<td><input type="hidden" id="rd03" title="" />03</td>
							<td><span id="rn03"></span></td>
							<td><span id="no03"></span></td>
							<td><a href="#a" onclick="del_receiverphoneno('3')"><img src="${contextRoot}/base/images/icon/ic_sms_x.gif" alt="del" /></a></td>
						</tr>
						<tr id="tr04">
							<td><input type="hidden" id="rd04" title="" />04</td>
							<td><span id="rn04"></span></td>
							<td><span id="no04"></span></td>
							<td><a href="#a" onclick="del_receiverphoneno('4')"><img src="${contextRoot}/base/images/icon/ic_sms_x.gif" alt="del" /></a></td>
						</tr>
					...
					<tbody>																
					 	<c:forEach var="sms" items="${smsRecentBottom}" varStatus="smsLoopCount">	 		
							<tr class="${smsLoopCount.count}" style='cursor:pointer' onclick="appendPhone('${sms.receiverId}','${sms.receiverName}', '${sms.receiverPhoneno}')" onmouseover="swap_img_over(${smsLoopCount.count})" onmouseout="swap_img_out(${smsLoopCount.count})">
								...
								<td>${sms.receiverPhoneno} <img id="smsRecentImg_${smsLoopCount.count}" src="${contextRoot}/base/images/icon/ic_s_minus.gif" alt="" /></td>
							</tr>
						</c:forEach>																							
					</tbody>
				</table>
