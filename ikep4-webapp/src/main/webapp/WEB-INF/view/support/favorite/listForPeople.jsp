...
(function($) {
	...
	getList = function(isScrollSet) {
		...
		$jq.ajax({     
			url : '<c:url value="/support/favorite/getSubListForPeople.do" />',     
	...
	goDelete = function(favoriteId, obj) {

		if(confirm('<ikep4j:message pre='${preMessage}' key='delete' />')) {
			$jq.ajax({     
				url : '<c:url value="/support/favorite/deleteFavorite.do" />',     
				data :  {favoriteId:favoriteId},     
				type : "post",     
				success : function(result) {  
					
					$jq(obj).parent().parent().parent().parent().remove();
					
					$jq("#recordCount").val(eval($jq("#recordCount").val())-1);
					$jq("#currentCount").val(eval($jq("#currentCount").val())-1);
...
<div id="wrapper">
		...
		<div id="blockMain">
			...
			<div id="mainContents" class="conPPS">
				...
				<h1><ikep4j:message pre='${preDetail}' key='favoritePeople' /></h1>
