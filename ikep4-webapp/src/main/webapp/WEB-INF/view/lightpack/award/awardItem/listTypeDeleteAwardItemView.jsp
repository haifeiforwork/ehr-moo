							...
							<c:otherwise>
							 	<div class="ellipsis">
								 	<span class="indent_${indent}">
								 		...
								 		<a class="awardItem"  <c:if test="${awardItem.readYn=='1' || awardItem.readFlg=='0'}" >style="color:#5D5D5D;"</c:if><c:if test="${awardItem.readYn=='0' && awardItem.readFlg=='1'}" >style="color:#050099;"</c:if> href="<c:url value='/lightpack/award/awardItem/readDeleteAwardItemView.do?awardId=${awardItem.awardId}&amp;itemId=${awardItem.itemId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}'/>"><c:if test="${award.wordHead == 1}"><c:if test="${!empty awardItem.wordName}">[${awardItem.wordName}]</c:if></c:if> ${awardItem.title}</a>
								 		<c:if test="${awardItem.linereplyCount gt 0}"><span class="colorPoint">(${awardItem.linereplyCount})</span></c:if>
								 		<c:if test="${awardItem.itemDisplay eq 1 and awardItem.indentation eq 0}"></span></c:if>	
						...
						<c:choose>
					 		<c:when test="${award.anonymous eq 1}">
								...
								<span>
									...
									<c:if test="${awardItem.awardId!='100006240370' && awardItem.awardId!='100006259597'}">${awardItem.displayName}</c:if>
								</span>
							</c:when>  
							<c:otherwise>
