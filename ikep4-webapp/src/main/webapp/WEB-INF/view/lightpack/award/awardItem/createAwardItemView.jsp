			...
			<td colspan="3"> 
				<div>
					...
					<input name="${status.expression}" type="text" class="inputbox" style="width: 40%;" value="${status.value}" size="40" title="<ikep4j:message pre='${preDetail}' key='${status.expression}' />"/> 
					 ...
					 <c:if test="${award.contentsReadPermission eq 1 || award.awardId=='100006240370'}">
		...
		<tr>  
			...
			<td>
				<div>
					...
					type="text" 
					class="inputbox datepicker" 
					value="<ikep4j:timezone date='${awardItem.startDate}'/>" size="10" 
					title="<ikep4j:message pre='${preDetail}' key='${status.expression}' />"
					/>   
					<c:forEach items="${status.errorMessages}" var="error">
				    	<label for="${status.expression}" class="serverError"> ${error}</label>
					...
					type="text" 
					class="inputbox datepicker" 
					value="<ikep4j:timezone date='${awardItem.endDate}'/>" size="10" 
					title="<ikep4j:message pre='${preDetail}' key='${status.expression}' />"
					/>  
					<c:forEach items="${status.errorMessages}" var="error">
				    	<label for="${status.expression}" class="serverError"> ${error}</label>
					...
					<spring:bind path="awardItem.itemForever">   
						<input 
							...
							value="${status.value}" 
							size="40" 
							<c:if test="${status.value eq 1 || award.awardId=='100006240370'}">checked="checked"</c:if>
							title="<ikep4j:message pre='${preDetail}' key='${status.expression}' />"
							/> 
