...
<c:set var="preHeader"  value="ui.lightpack.award.awardAdmin.updateAwardView" /> 
<c:set var="preDetail"  value="ui.lightpack.award.awardAdmin.updateAwardView.detail" />
...
setReadUser = function(data) {   
	...
	setReadGroupAddress = function(data) { 
		...
		$.each(data, function(index, value) {  
				$.tmpl(iKEP.template.groupAwardOption, this).appendTo($sel).data("data", this); 
		});
	...
	setAddressInit = function() {
		...
		<c:forEach var="user" items="${award.readUserList}"> 
			...
			$.tmpl(iKEP.template.userAwardOption, addrObj)
				.appendTo($sel)
				.data("data", addrObj);
		</c:forEach>
		...
		<c:forEach var="group" items="${award.readGroupList}"> 
			...
			$.tmpl(iKEP.template.groupAwardOption, addrObj).appendTo($sel).data("data", addrObj);
			</c:if>
			<c:if test="${group.hierarchied eq 1}">
			$.tmpl(iKEP.template.higroupAwardOption, addrObj).appendTo($sel).data("data", addrObj);
			</c:if>
		...
		<c:forEach var="group" items="${award.writeGroupList}"> 
			addrObj = $.parseJSON('{"type":"group","code":"${group.groupId}","name":"${group.groupName}"}');
			$.tmpl(iKEP.template.groupAwardOption, addrObj)
				.appendTo($sel)
				.data("data", addrObj);
		</c:forEach>
	...
	$(document).ready(function() {  
		...
		$("a[name=updateAwardButton]").click(function() {  
			...
			$("#selectAdminUserList option").each(function(index) {  
				...
				$("<input name='adminUserList[" + index + "].userId'       type='hidden' value='"+ userInfo[0] +"' />").appendTo( "#awardForm" );   
				$("<input name='adminUserList[" + index + "].userName'     type='hidden' value='"+ userInfo[1] +"' />").appendTo( "#awardForm" );   
				$("<input name='adminUserList[" + index + "].jobTitleName' type='hidden' value='"+ userInfo[2] +"' />").appendTo( "#awardForm" );   
				$("<input name='adminUserList[" + index + "].teamName'     type='hidden' value='"+ userInfo[3] +"' />").appendTo( "#awardForm" );   
				$("<input name='adminUserList[" + index + "].mail'         type='hidden' value='"+ userInfo[4] +"' />").appendTo( "#awardForm" );   
			});
			
			$("#selectAdminGroupList option").each(function(index) {
				var groupInfo = $(this).attr("id").split("^");
				
				$("<input name='adminGroupList[" + index + "].groupId'   type='hidden' value='"+ groupInfo[0] +"' />").appendTo( "#awardForm" );   
				$("<input name='adminGroupList[" + index + "].groupName' type='hidden' value='"+ groupInfo[1] +"' />").appendTo( "#awardForm" );    
				
				if($(this).hasClass("hierarchied")) {
					$("<input name='adminGroupList[" + index + "].hierarchied' type='hidden' value='1' />").appendTo( "#awardForm" );    
				} else {
					$("<input name='adminGroupList[" + index + "].hierarchied' type='hidden' value='0' />").appendTo( "#awardForm" );  
				}  
			...
			$("#addrAdminRoleList option").each(function(index) {
				var roleInfo = $(this).attr("value"); 
				$("<input name='adminRoleList[" + index + "].roleId'     type='hidden' value='"+ roleInfo +"' />").appendTo( "#awardForm" );   

			}); 
			
			<c:if test="${award.awardType eq 0}">
			$("#awardForm").attr("action", "<c:url value='/lightpack/award/awardAdmin/updateAwardTypeAward.do'/>");
			</c:if>
			<c:if test="${award.awardType eq 1}">
			$("#awardForm").attr("action", "<c:url value='/lightpack/award/awardAdmin/updateLinkTypeAward.do'/>");
			</c:if>
			<c:if test="${award.awardType eq 2}">
			$("#awardForm").attr("action", "<c:url value='/lightpack/award/awardAdmin/updateCategoryTypeAward.do'/>");
			</c:if>	 
		...
		$("#searchItemButton").click( function() {  
			var url = "<c:url value='/lightpack/award/awardAdmin/listCategoryView.do?awardId=${award.awardId}'/>";
...
//-->
	...
	<div class="blockDetail"> 
			...
			<spring:bind path="award.awardId">
				<input name="${status.expression}" type="hidden" value="${status.value}" title="<ikep4j:message pre='${preDetail}' key='${status.expression}'/>" /> 
			</spring:bind>	 
			<spring:bind path="award.sortOrder">
				<input name="${status.expression}" type="hidden" value="${status.value}" title="<ikep4j:message pre='${preDetail}' key='${status.expression}'/>" /> 
			</spring:bind>	 
						...
						<td>
							...
							<div class="mt5">
									...
									<select name="addrReadRoleList" id="addrReadRoleList" size="5" multiple="multiple" class="multi w60"  >
										<c:if test="${!empty award.readRoleList}">
										<c:forEach var="item" items="${award.readRoleList}">
				...
				<td>
					...
					<div class="mt5">
							...
							<select name="addrWriteRoleList" id="addrWriteRoleList" size="5" multiple="multiple" class="multi w60"  >
								<c:if test="${!empty award.writeRoleList}">
								<c:forEach var="item" items="${award.writeRoleList}">
									<option value="<c:out value='${item.roleId}'/>"><c:out value='${item.roleName}'/></option>
								</c:forEach>
								</c:if>
							</select>
			...
			<tr class="adminPermissionTableRow"> 
				...
				<td> 
					...
					<div class="mt5">  
						...
						<span id="selectadminUserListCount" class="totalNum_s"><ikep4j:message pre='${preDetail}' key='all' /> ${fn:length(award.adminUserList)}<ikep4j:message pre='${preDetail}' key='myung' /></span> 
					</div>	 
				</td>   
			</tr>
			<tr class="adminPermissionTableRow">  
				...
				<td> 
					...
					<div class="mt5">  
						...
						<span  id="selectAdminGroupListCount" class="totalNum_s"><ikep4j:message pre='${preDetail}' key='all' /> ${fn:length(award.adminGroupList)} <ikep4j:message pre='${preDetail}' key='group' /></span> 
				 	</div>	   
				</td>
			</tr> 
			<tr class="adminPermissionTableRow">
				...
				<td>
					...
					<div class="mt5">
							...
							<select name="addrAdminRoleList" id="addrAdminRoleList" size="5" multiple="multiple" class="multi w60"  >
								<c:if test="${!empty award.adminRoleList}">
								<c:forEach var="item" items="${award.adminRoleList}">
									<option value="<c:out value='${item.roleId}'/>"><c:out value='${item.roleName}'/></option>
								</c:forEach>
								</c:if>
							</select>
	...
	<div class="blockButton" id="divButton"> 
		<ul> 
			<li><a name="updateAwardButton"   class="button" href="#a"><span><ikep4j:message pre='${preButton}' key='save'/></span></a></li>
