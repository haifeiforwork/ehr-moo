...
<div id="tagResult">
	...
	<ul> 
		<li><a class="button" title="<ikep4j:message pre='${preButton}' key='update'/>" href="<c:url value='/lightpack/award/awardItem/updateDeleteAwardItemView.do?awardId=${awardItem.awardId}&amp;itemId=${awardItem.itemId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}'/>"><span><ikep4j:message pre='${preButton}' key='update'/></span></a></li>
		<li><a class="button" href="<c:url value='/lightpack/award/awardItem/deleteAwardItemListView.do?awardId=${awardItem.awardId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}'/>"><span><ikep4j:message pre='${preButton}' key='list'/></span></a></li>
	</ul> 
	</div>
	...
	<div class="blockTableReadWrap">
		...
		<div class="blockTableRead">
			<div class="blockTableRead_t">
				...
				<c:if test="${award.wordHead == 1}"><c:if test="${!empty awardItem.wordName}">[${awardItem.wordName}]</c:if></c:if> ${awardItem.title}		
				</p> 	
				</spring:bind> 
							...
							<span class="summaryViewInfo_name">
								<c:choose>
									<c:when test="${user.localeCode == portal.defaultLocaleCode}"> 
										...
										<c:if test="${awardItem.awardId!='100006240370' && awardItem.awardId!='100006259597'}">
			...
			<div class="blockTableRead_c" id="awardItemContent">
				...
				<div class="tableTag" id="tagForm_${awardItem.itemId}">     <!-- 게시물 id --> 
				      ...
				      <div class="tableTag">
				        ...
				        <span class="rebtn">
				         <c:if test="${user.userId == awardItem.registerId or permission.isSystemAdmin}">  <!--권한 체크 등록자랑 세션userID랑 같으면 태그 수정 가능-->
