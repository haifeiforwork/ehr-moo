...
<c:if test="${ not empty boardItemList and fn:length(boardItemList) gt 1 }"> 
		<div class="blockReply_t"><ikep4j:message pre='${preDetail}' key='replyItemList' /><span class="comment_num">(${fn:length(boardItemList)})</span></div> 
		<ul> 
			<c:forEach var="replyItem" items="${boardItemList}">
			<li>    
				<span class="indent_${replyItem.indentation}"> 
					<c:choose> 
						...
						<c:when test="${replyItem.itemDelete eq 1 and (replyItem.itemId ne itemId)}"><%-- 논리적으로 삭제된 글 --%>
							<c:choose>
						 		<c:when test="${permission.isSystemAdmin}"><%--  관리자, 작성자일 경우  --%>
						 			<a href="<c:url value='/lightpack/board/boardItem/readBoardItemView.do?boardId=${replyItem.boardId}&amp;itemId=${replyItem.itemId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}'/>"><span class="deletedItem">[<ikep4j:message pre='${preDetail}' key='itemDelete' post="deleteContents"/>]</span> ${replyItem.title}</a>
								</c:when>
						 		<c:otherwise>
						...
						<c:when test="${replyItem.itemDelete eq 0 and (replyItem.itemId ne itemId)}"><%-- 논리적으로 삭제된 글 --%>
						 	<a href="<c:url value='/lightpack/board/boardItem/readBoardItemView.do?boardId=${replyItem.boardId}&amp;itemId=${replyItem.itemId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}'/>">${replyItem.title}</a>
					    </c:when>   
					</c:choose>	 
