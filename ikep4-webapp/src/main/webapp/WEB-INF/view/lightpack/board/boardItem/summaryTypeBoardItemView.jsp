		    ...
		    <c:otherwise>
				...
				<tr class="bgWhite boardItemLine"> 
					<c:if test="${permission.isSystemAdmin}"> <%--[관리자]일괄 삭제를 위한 체크박스 허용--%>
						<td style="width:20px;"><input name="checkboxBoardItem" class="checkbox" title="checkbox" type="checkbox" value="${boardItem.itemId}" /></td>
					</c:if>	 			
					<td>
							...
							<span>
								<c:choose>
							 		<c:when test="${board.anonymous eq 1}">
										<span><!--<ikep4j:message pre='${preList}' key='anonymous'/>-->${boardItem.registerName}</span>
									</c:when>  
									<c:otherwise>
										...
										<c:choose>
											<c:when test="${user.localeCode == portal.defaultLocaleCode}"> 
												<span class="summaryViewInfo_name"><a href="#a" onclick="iKEP.showUserContextMenu(this, '${boardItem.registerId}', 'bottom')">${boardItem.user.userName}<!--  ${boardItem.user.jobTitleName} --></a></span>
												<!--<img src="<c:url value='/base/images/theme/theme01/basic/bar_pageNum.gif'/>" alt="" /> ${boardItem.user.teamName}-->
											</c:when>
											<c:otherwise> 
												<span class="summaryViewInfo_name"><a href="#a" onclick="iKEP.showUserContextMenu(this, '${boardItem.registerId}', 'bottom')">${boardItem.user.userEnglishName}<!-- ${boardItem.user.jobTitleEnglishName} --></a></span>
						...
						<c:if test="${boardItem.itemDelete != 1 || permission.isSystemAdmin  }">
