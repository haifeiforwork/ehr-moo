	...
	$(document).ready(function() { 
		...
		new iKEP.Validator("#boardItemForm", {   
			...
			messages : {
				...
				startDate : {direction : "bottom", required : "<ikep4j:message key="NotNull.boardItem.startDate" />", dateLTE : "<ikep4j:message pre='${preMessage}' key='dateBetween' />"},
				endDate   : {direction : "bottom", required : "<ikep4j:message key="NotNull.boardItem.endDate" />", dateGTE : "<ikep4j:message pre='${preMessage}' key='dateBetween' />"} 
			},   
	...
	<div class="blockButton"> 
		<ul>
			<%-- <c:if test="${permission.isSystemAdmin or user.userId eq boardItem.registerId}"> --%><%-- 쓰기 가능 권한인 경우 저장 버튼 활성화 --%>
				<c:if test="${boardItem.tempSave eq '1'}">
			...
			<%-- </c:if>   --%>
			<li><a class="button listBoardItemButton" href="<c:url value='/lightpack/board/boardItem/readBoardItemView.do?boardId=${boardItem.boardId}&amp;itemId=${boardItem.itemId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}'/>"><span><ikep4j:message pre='${preButton}' key='cancel'/></span></a></li>
...
<form id="boardItemForm" method="post" action="<c:url value='/lightpack/board/boardItem/updateBoardItem.do'/>" enctype="multipart/form-data">
	...
	<input name="itemParentId" 	type="hidden" value="${boardItem.itemParentId}"         />  	
	<input name="itemDisplay"           type="hidden" value="${boardItem.itemDisplay}" />
	<input name="itemForever"           type="hidden" value="${boardItem.itemForever}" /> 
	<input name="readerMailFlag"           type="hidden" value="${boardItem.readerMailFlag}" /> 
	...
	<table summary="<ikep4j:message pre="${preDetail}" key="summary" />">
						...
						<spring:bind path="boardItem.title">
							...
							<select id="categoryId" name=categoryId >
								<c:forEach var="code" items="${boardItemCategoryList}">
									<option value="${code.categoryId}" <c:if test="${code.categoryId eq boardItem.wordId}">selected="selected"</c:if>>${code.categoryName}</option>
								</c:forEach>
							</select>
							</c:if> 
							<input 
		...
		<c:if test="${board.boardId=='100006240370'}">
			<tr>
				<th>사업장</th>
				<td colspan="3">
					<input name="workplaceCheck" class="checkbox" title="checkbox" type="checkbox" value="전체" style="vertical-align:middle;" />전체
		...
		<tr>  
			...
			<td>
				<div> 
					...
					value="<ikep4j:timezone date='${boardItem.startDate}' />" 
					size="10" 
					title="<ikep4j:message pre='${preDetail}' key='${status.expression}' />"
					/>  
					<c:forEach items="${status.errorMessages}" var="error">
					...
					type="text" 
					class="inputbox datepicker" 
					value="<ikep4j:timezone date='${boardItem.endDate}' />" 
					size="10" 
					title="<ikep4j:message pre='${preDetail}' key='${status.expression}' />"
					/>  
					<c:forEach items="${status.errorMessages}" var="error">
		...
		<c:if test="${board.contentsReadPermission eq 1}">
		<c:if test="${boardItem.step eq 0}"><!-- 답글의 수정은 독서자 설정을 못하고 부모글을 따라감 -->
		<tr>
			<th scope="row">독서자</th>
			<td colspan="3">
			<select name="readerList" id="readerList" class="input_select" size="4"	style="height:100px;width:60%" multiple>
			<c:forEach var="item" items="${boardItem.boardItemReaderList}">
			...
			<td colspan="3">
				<spring:bind path="boardItem.readerMailFlag">   
						<input 
							...
							/> 
						메일발송
				</spring:bind><c:if test="${boardItem.step ne 0}">&nbsp;&nbsp;&nbsp;※ 답글의 독서자는 원본글과 동일하게 설정됩니다. 답글의 메일발송여부만 수정가능.</c:if>
			</td>
		</tr>
		</c:if>
		...
		<tr>  
			...
			<td colspan="4" class="ckeditor">
				...
				<input id="contents" name="contents" type="hidden" value="${boardItem.contents}" /-->
				<!-- 에디터 부분 끝 --> 
