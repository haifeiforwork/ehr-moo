...
//<!--
	...
	function getList() {
		...
		$jq("#searchForm").attr("action", "<c:url value='/lightpack/overtimework/overtimeworkInOutUnidentifiedList.do' />");
	...
	$jq(document).ready(function() {
		...
		$jq("#excelButton").click(function() { 
			...
			$jq("#searchForm").attr("action","<c:url value='/lightpack/overtimework/downloadExcelOvertimeworkInOutUnidentifiedList.do'/>");
			$jq("#searchForm")[0].submit();
		return false; 
		});
		...
		$jq("#createPrintButton").click(function() {    
			...
			var url = iKEP.getContextRoot() + '/lightpack/overtimework/overtimeworkInOutUnidentifiedListPrint.do';
			
			iKEP.popupOpen(url, {width:800, height:560}, "overtimeworkPrint");
		});
	});
	
	function detailView(overtimeworkId) {
		$jq("#overtimeworkId").val(overtimeworkId);
		$jq("#regForm").attr("action", "<c:url value='/lightpack/overtimework/overtimeworkInOutUpdateForm.do' />");
		$jq("#regForm")[0].submit();
	}
	...
	<div class="blockListTable">
		<div id="listDiv">
			<form id="regForm" name="regForm" method="post" action="">
				<input type="hidden" name="overtimeworkId" id="overtimeworkId" value=""/> 
			</form>
						...
						<c:choose>
							...
							<c:otherwise>
								<spring:bind path="searchCondition.workPlaceName">                             			
								   ...
								   <spring:bind path="searchCondition.teamCode">
								       <select title="teamCode" name="${status.expression}" id="${status.expression}">
								       	<option value="all" <c:if test="${empty searchCondition.teamCode}">selected="selected"</c:if>>전체</option>
								           <c:forEach var="teamInfo" items="${teamList}">
								           	<option value="${teamInfo.teamCode}" <c:if test="${searchCondition.teamCode eq teamInfo.teamCode}">selected="selected"</c:if>>${teamInfo.teamName}</option>
								           </c:forEach>
								       </select>
								...
								<c:forEach var="overtimework" items="${searchResult.entity}" varStatus="status">
									...
									<tr onmouseover="mouseOver(this);" onmouseout="mouseOut(this);">
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<a style="cursor: pointer;" onclick="detailView(${overtimework.overtimeworkId});" >${overtimework.num}</a>
										</td>
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<a style="cursor: pointer;" onclick="detailView(${overtimework.overtimeworkId});" ><fmt:formatDate value="${overtimework.registDate}" pattern="yyyy-MM-dd" /></a>
										</td>
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<a style="cursor: pointer;" onclick="detailView(${overtimework.overtimeworkId});" ><fmt:formatDate value="${overtimework.registDate}" pattern="HH:mm:ss" /></a>
										</td>
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<a style="cursor: pointer;" onclick="detailView(${overtimework.overtimeworkId});" ><c:if test="${overtimework.inoutFlag=='I'}">Office In</c:if>
											<c:if test="${overtimework.inoutFlag=='O'}">Office Out</c:if></a>
										</td>
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<a style="cursor: pointer;" onclick="detailView(${overtimework.overtimeworkId});" >${overtimework.companyCodeName}</a>
										</td>
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<a style="cursor: pointer;" onclick="detailView(${overtimework.overtimeworkId});" >${overtimework.workPlaceName}</a>
										</td>
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<a style="cursor: pointer;" onclick="detailView(${overtimework.overtimeworkId});" >${overtimework.groupName}</a>
										</td>
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<a style="cursor: pointer;" onclick="detailView(${overtimework.overtimeworkId});" >${overtimework.userName} <c:if test="${!empty overtimework.jobDutyName}">${overtimework.jobDutyName}</c:if><c:if test="${empty overtimework.jobDutyName}">${overtimework.jobTitleName}</c:if></a>
										</td>
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<a style="cursor: pointer;" onclick="detailView(${overtimework.overtimeworkId});" ><c:if test="${overtimework.checkFlag=='Y'}">확인</c:if>
											<c:if test="${overtimework.checkFlag=='N'}">미확인</c:if></a>
										</td>
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<a style="cursor: pointer;" onclick="detailView(${overtimework.overtimeworkId});" ><fmt:formatDate value="${overtimework.checkDate}" pattern="yyyy-MM-dd" /></a>
										</td>
										<td class="textCenter" style="border-left:1px solid #e0e0e0;border-right:1px solid #e0e0e0;">
											<a style="cursor: pointer;" onclick="detailView(${overtimework.overtimeworkId});" ><fmt:formatDate value="${overtimework.checkDate}" pattern="HH:mm:ss" /></a>
										</td>
