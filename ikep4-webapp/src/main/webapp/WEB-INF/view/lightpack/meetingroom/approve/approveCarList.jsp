...
//<!--
	...
	function getList() {
		$jq("#searchForm").attr("action", "<c:url value='approveCarList.do' />");
	...
	<div class="blockListTable">
		<div id="listDiv">
			<form id="searchForm" name="searchForm" method="post" onsubmit="getList(); return false;" action="">
				<input type="hidden" id="roomOrTool" name="roomOrTool" value="tool"/>
				...
				<div class="tableTop">
					...
					<h2>자원 <ikep4j:message pre='${preDetail}' key='reserveAgreement' /></h2>
					<div class="listInfo"> 
						...
						<c:choose>
						    ...
						    <c:otherwise>
								<c:forEach var="approve" items="${searchResult.entity}" varStatus="status">
									<tr>
										...
										<td class="textCenter">
											<c:choose>
												...
												<c:when test="${approve.approveStatus == 'W'}">
													<c:choose>
														<c:when test="${approve.isManager == 1}">
															<c:choose>
																<c:when test="${approve.managerId == user.userId && !empty approve.subManagerId}">
																	<a class="button_ic" id="assignButton" href="#a" onclick="firstAssign('${approve.scheduleId}'); return false;"><span><ikep4j:message pre='${preDetail}' key='approval' /></span></a>
																	<a class="button_ic" id="rejectButton" href="#a" onclick="rejectForm('${approve.scheduleId}'); return false;"><span><ikep4j:message pre='${preDetail}' key='reject' /></span></a>
																</c:when>
																<c:when test="${approve.managerId == user.userId && empty approve.subManagerId}">
																	<a class="button_ic" id="assignButton" href="#a" onclick="assign('${approve.scheduleId}'); return false;"><span><ikep4j:message pre='${preDetail}' key='approval' /></span></a>
																	<a class="button_ic" id="rejectButton" href="#a" onclick="rejectForm('${approve.scheduleId}'); return false;"><span><ikep4j:message pre='${preDetail}' key='reject' /></span></a>
																</c:when>
																<c:otherwise>
																	<ikep4j:message pre='${preDetail}' key='wait' />
												...
												<c:when test="${approve.approveStatus == 'S'}">
													<c:choose>
														<c:when test="${approve.isManager == 1}">
															<c:choose>
																<c:when test="${approve.subManagerId == user.userId && !empty approve.lastManagerId}">
																	<a class="button_ic" id="assignButton" href="#a" onclick="secondAssign('${approve.scheduleId}'); return false;"><span><ikep4j:message pre='${preDetail}' key='approval' /></span></a>
																	<a class="button_ic" id="rejectButton" href="#a" onclick="rejectForm('${approve.scheduleId}'); return false;"><span><ikep4j:message pre='${preDetail}' key='reject' /></span></a>
																</c:when>
																<c:when test="${approve.subManagerId == user.userId && empty approve.lastManagerId}">
																	<a class="button_ic" id="assignButton" href="#a" onclick="assign('${approve.scheduleId}'); return false;"><span><ikep4j:message pre='${preDetail}' key='approval' /></span></a>
												...
												<c:when test="${approve.approveStatus == 'L'}">
													<c:choose>
														<c:when test="${approve.isManager == 1}">
															<c:if test="${approve.lastManagerId == user.userId}">
																<a class="button_ic" id="assignButton" href="#a" onclick="assign('${approve.scheduleId}'); return false;"><span><ikep4j:message pre='${preDetail}' key='approval' /></span></a>
																<a class="button_ic" id="rejectButton" href="#a" onclick="rejectForm('${approve.scheduleId}'); return false;"><span><ikep4j:message pre='${preDetail}' key='reject' /></span></a>
															</c:if>
															<c:if test="${approve.lastManagerId != user.userId}">
									...
									<tr id="contentsTR_${approve.scheduleId}" style="display:none">
										<td></td>
										<td></td>
										<td style="text-align:left"><pre>${fn:replace( approve.contents, crlf, "<br/>")}</pre></td>
										<td></td>
										<td></td>
										<td></td>
										<td>${approve.rejectReason}</td>
									</tr>
								</c:forEach>
						    </c:otherwise>
						</c:choose>
