...
(function($) {
	...
	drawMeetingRoomList = function() {
		...
		if(currentReserveList!=null) {
			...
			var template = $("#tmplMeetingRoomItemWeek").template();
			...
			$container.append($.tmpl($("#tmplMeetingRoomItemEmptyWeek").template()));
		}
	}
	...
	getList = function() {
		...
		var data = { buildingId : $("#buildingId").val() };
		if(startDate && endDate)
			$.extend(data, {startDate : startDate.getTime(), endDate : endDate.getTime()});
		
		$.get("<c:url value="/lightpack/meetingroom/week/meetingRoomList.do" />", data)
			.success(function(result) {
