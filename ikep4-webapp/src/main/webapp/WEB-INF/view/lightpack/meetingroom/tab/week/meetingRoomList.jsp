...
goDayTab = function(meetingRoomId, day) {    
	...
	$jq("building").val($jq("buildingId").val());
	$jq("floor").val($jq("floorId").val());
	
	var url  = '<c:url value="/lightpack/meetingroom/day/meetingRoomMain.do"/>';
	$jq.ajax({     
		url : url,    
		type : "post",
		data : {
		...
		$("table.meeting_schedule").click(function(event) {
			...
			if(el.tagName.toLowerCase() == "td") {
				...
				if($el.hasClass("empty")) {
					...
					goDayTab($el.parent().data("meeting_room_id"), $el.data("day"));
				}
			}
		});
	});
			...
			<c:forEach var="timeList" items="${timeList}" varStatus="status">
				<c:forEach var="time" items="${timeList}" varStatus="status">
					...
					<tr data-building_name="${time.buildingName}" data-floor_name="${time.floorName}" data-meeting_room_name="${time.meetingRoomName}" data-meeting_room_id="${time.meetingRoomId}">
						<th class="ellipsis textLeft" width="17%" title="${time.buildingName}">${time.buildingName}</th>
						<th class="ellipsis textLeft" width="8%" title="${time.floorName}">${time.floorName}</th>
