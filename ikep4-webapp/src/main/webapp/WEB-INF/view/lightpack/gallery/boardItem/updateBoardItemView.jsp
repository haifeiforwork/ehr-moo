...
<c:set var="preHeader"  value="ui.lightpack.gallery.boardItem.updateBoardView.header" /> 
<c:set var="preDetail"  value="ui.lightpack.gallery.boardItem.updateBoardView.detail" />
...
(function($){	 
	...
	$(document).ready(function() { 
		...
		$("a.saveBoardItemButton").click(function() {
			if( ($jq('input[name="oldFileId"]').val()==null || $jq("input[name=oldFileId]").val()=="")&& ( $jq('input[name="file"]').val()==null || $jq("input[name=file]").val()=="") )
			{
	...
	checkEditorFileSize = function (){
			    		
		var form = document.boardItemForm;
		// 첨부파일 용량 제한
		if(document.twe.IsOverAttachmentLimit()) {
			alert('<ikep4j:message pre='${preMessage}' key='editor.limitSize' />');
			return false;
		}
		// 첨부파일 갯수 제한
		if(document.twe.GetAttachmentCount()>limitFileCount) {
...
<script language="JScript" for="twe" event="OnControlInit()">
	var form = document.boardItemForm;
	document.twe.AttachmentLimitSize = limitFileSize; // 5MByte
...
<form id="editorFileUploadParameter" action="null"> 
	<input name="targetUserId"  value="${targetUserId}" type="hidden"/> 
...
<div id="pageTitle" class="btnline"> 
	...
	<div class="blockButton"> 
		<ul>
			<c:if test="${targetUserId eq boardItem.registerId}"><%-- 쓰기 가능 권한인 경우 저장 버튼 활성화 --%>
				<li><a class="button saveBoardItemButton" href="#a"><span><ikep4j:message pre='${preButton}' key='save'/></span></a></li>
			</c:if>  
			<li><a class="button" href="<c:url value='/lightpack/gallery/boardItem/readBoardItemView.do?targetUserId=${targetUserId}&amp;itemId=${boardItem.itemId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}'/>"><span><ikep4j:message pre='${preButton}' key='cancel'/></span></a></li>
...
<form id="boardItemForm" method="post" action="<c:url value='/lightpack/gallery/boardItem/updateBoardItem.do'/>"  enctype="multipart/form-data">
	...
	<table summary="<ikep4j:message pre="${preDetail}" key="summary" />">
		...
		<tr>
			...
			<td colspan="3"><input name="file" type="file" class="file w70" title=""/><br /><br />

			<c:if test="${!empty boardItem.fileDataList}">
			<c:forEach var="fileDataList" items="${boardItem.fileDataList}" varStatus="tagLoop">
			<c:if test="${tagLoop.index==0}"><input name="oldFileId" type="hidden" value="${fileDataList.fileId}" title="oldFileId" /></c:if>
			</c:forEach>
			</c:if>			
			<span id="viewDiv">
			<c:if test="${!empty boardItem.fileDataList}">
			<c:forEach var="fileDataList" items="${boardItem.fileDataList}" varStatus="tagLoop">
			<img id='viewImageDiv' width='200px' src='<c:url value="/support/fileupload/downloadFile.do"/>?fileId=${fileDataList.fileId}' />
			</c:forEach>
			</c:if>		
			</span>	
