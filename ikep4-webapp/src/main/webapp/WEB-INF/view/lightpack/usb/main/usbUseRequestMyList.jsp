...
//<!--
	...
	function getList() {
		...
		$jq("#searchForm").attr("action", "<c:url value='/lightpack/usb/usbUseRequestMyList.do' />");
						    ...
						    <c:otherwise>
								<c:forEach var="usb" items="${searchResult.entity}" varStatus="status">
									<tr>
										...
										<td class="textCenter" >
											<a href="<c:url value='/lightpack/usb/usbUseRequestMyView.do?usbId=${usb.usbId}'/>">
											${usb.requestReason}</a>
										</td>
										<td class="textCenter">
											<c:choose>
											<c:when test="${usb.approveStatus == 'A' || usb.approveStatus == 'C'}">
