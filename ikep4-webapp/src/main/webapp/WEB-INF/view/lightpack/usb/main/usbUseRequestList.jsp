...
//<!--
	...
	function getList() {
		...
		$jq("#searchForm").attr("action", "<c:url value='/lightpack/usb/usbUseRequestList.do' />");
	...
	function assign(usbId) {
		...
		if(confirm("승인하시겠습니까?")) {
			...
			$jq("#usbId").val(usbId);
			$jq("#searchForm").attr("action", "<c:url value='/lightpack/usb/usbApproveUse.do' />");
			$jq("#searchForm")[0].submit();
		}
	};
						    ...
						    <c:otherwise>
								<c:forEach var="usb" items="${searchResult.entity}" varStatus="status">
									<tr>
										...
										<td class="textCenter" >
											<a href="<c:url value='/lightpack/usb/usbUseRequestView.do?usbId=${usb.usbId}'/>">
											${usb.requestReason}</a>
										</td>
										...
										<td class="textCenter">
											...
											<c:when test="${usb.approveStatus == 'S'}">
												<c:choose>
													<c:when test="${approve.approveUserId == user.userId}">
														<a class="button_ic" id="assignButton" href="#a" onclick="assign('${usb.usbId}'); return false;"><span><ikep4j:message pre='${preDetail}' key='approval' /></span></a>
														<a class="button_ic" id="rejectButton" href="#a" onclick="rejectForm('${usb.usbId}'); return false;"><span><ikep4j:message pre='${preDetail}' key='reject' /></span></a>
													</c:when>
													<c:otherwise>
