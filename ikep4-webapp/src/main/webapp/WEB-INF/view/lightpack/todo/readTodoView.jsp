...
(function($) {
	...
	$jq(document).ready(function() {
		...
		$jq('#tempSaveButton').click( function(event) {
			...
			if ("${task.subworkCode}" == "MYTASK" && contents.length < 1){
...
function goDelete(taskId, searchConditionString){
	...
	if(confirm("작업을 삭제하시겠습니까?")){
		...
		var url = "<c:url value='/lightpack/todo/deleteMyTask.do?taskId='/>"+taskId+"&amp;searchConditionString="+searchConditionString;
...
//]]>
				...
				<div class="blockButton mb15">
					<ul>
						<c:if test="${task.subworkCode != 'TASK'}">
						<li><a class="button" href="<c:url value='/lightpack/todo/updateTodoView.do?taskId=${task.taskId}&amp;searchConditionString=${searchConditionString}'/>"><span><ikep4j:message pre='${buttonPrefix}' key='modify'/></span></a></li>
				...
				<!--blockDetail 2 Start-->
				<div class="blockDetail">
				<form id="todoForm" name="todoForm" action="<c:url value="/lightpack/todo/saveTodo.do"/>" method="post"> 
