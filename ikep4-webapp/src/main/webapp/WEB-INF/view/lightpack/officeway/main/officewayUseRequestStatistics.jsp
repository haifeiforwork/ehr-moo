...
<!--// 
	...
	$jq("#itemForm").attr("action", "<c:url value='/lightpack/officeway/officewayUseRequestStatistics.do' />");
	$jq("#itemForm")[0].submit();
	}
	
	function teamViewPopup(teamId, calMnt) {
		var url = "<c:url value='/lightpack/officeway/officewayUseRequestOtherTeamList.do'/>?teamId="+teamId+"&viewMonth="+calMnt;
    	      		...
    	      		<tr>
    					...
    					<td>
		                    <select name="year" onchange="getList();">                               		
						        <c:forEach var="fyear" begin="2016" end="${nowYear}" step="1">
						        	<option value="${fyear}" <c:if test="${fyear eq year}">selected="selected"</c:if>>${fyear}</option>
						        </c:forEach>
