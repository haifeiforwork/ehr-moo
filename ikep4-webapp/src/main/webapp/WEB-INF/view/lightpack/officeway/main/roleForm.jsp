				...
				(function($) {
					...
					$jq(document).ready(function(){
						...
						$jq("#saveButton").click(function(){
							roleChk1 = 0;
							roleChk2 = 0;
							if("${role.roleName}" == "OFMR"){
								<c:forEach var="item1" items="${roleUserList1}">
									$jq("#roleUserList option").each(function (index) {
										if($jq(this).val() == "${item1.userId}"){
											roleChk1++;
										}
									});
									$jq("#roleGroupList option").each(function (index) {
										if($jq(this).val() == "${item1.groupId}"){
											roleChk1++;
										}
									});
								</c:forEach>
								<c:forEach var="item2" items="${roleGroupList1}">
								$jq("#roleGroupList option").each(function (index) {
									if($jq(this).val() == "${item2.groupId}"){
										roleChk1++;
									}
								});
								</c:forEach>
							...
							if(roleChk1 > 0){
								alert("사무용품 최종 결재 권한을 가진 사용자는 권한을 부여할수 없습니다.");
								return;
							}
							
							if("${role.roleName}" == "OFML"){
								<c:forEach var="item3" items="${roleUserList2}">
									$jq("#roleUserList option").each(function (index) {
										if($jq(this).val() == "${item3.userId}"){
											roleChk2++;
										}
									});
									$jq("#roleGroupList option").each(function (index) {
										if($jq(this).val() == "${item3.groupId}"){
											roleChk2++;
										}
									});
								</c:forEach>
								<c:forEach var="item4" items="${roleGroupList2}">
								$jq("#roleGroupList option").each(function (index) {
									if($jq(this).val() == "${item4.groupId}"){
										roleChk2++;
									}
								});
								</c:forEach>
							...
							if(roleChk2 > 0){
								alert("사무용품 검토 권한을 가진 사용자는 권한을 부여할수 없습니다.");
								return;
							}
						...
						var validOptions = {
								...
								submitHandler : function(role) {
							    	...
							    	$jq.ajax({
										url : '<c:url value="/lightpack/officeway/roleOnSubmit.do" />',
										data : $jq("#roleForm").serialize(),
										type : "post",
										success : function(result) {
