...
//<!--
	...
	function getList() {
		...
		$jq("#searchForm").attr("action", "<c:url value='/lightpack/officeway/officewayUseRequestMyList.do' />");
	...
	$jq(document).ready(function() {
		...
		$jq("#saveButton").click(function() {
			...
			$jq("#searchForm input[name=checkboxItem]:checked").each(function(index) { 
				...
				if($jq("#officeway_"+$jq(this).val()).val() == "T"){
					if(tempItemId == ""){
			...
			if(confirm("선택한 사무용품을 신청하시겠습니까?")) {
				$jq("#regForm").attr("action", "<c:url value='/lightpack/officeway/officewayCheckBoxUseRequest.do' />");
				$jq("#regForm")[0].submit();
			}  
		});
		...
		$jq("#deleteButton").click(function() {  
			...
			if(confirm("선택한 신청건을 삭제하시겠습니까?")) {
				...
				$jq.ajax({
					url : "<c:url value='/lightpack/officeway/officewayRequestDelete.do' />",
					data : $jq("#searchForm").serialize(),
					type : "post",
					dataType : "html",
					success : function(result) {
			...
			<form id="searchForm" name="searchForm" method="post" onsubmit="getList(); return false;" action="">
				...
				<div class="listInfo"> 
						...
						<div class="totalNum">
							...
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color="blue" style="font-weight: bold;font-size: 1.2em;">&nbsp;&nbsp;&nbsp;(나의 기간별 사용실적:<fmt:formatNumber value="${price1}"/>원 / 팀 기간별 사용실적:<fmt:formatNumber value="${price2}"/>원 / ${periodInfo.month}월 신청기간:${periodInfo.startDate}~${periodInfo.endDate})</font>
						</div>
					</div>
				...
				<table summary="<ikep4j:message pre='${preDetail}' key='myReserveList' />">
					...
					<tbody>
						<c:choose>
						    ...
						    <c:otherwise>
								<c:forEach var="officeway" items="${searchResult.entity}" varStatus="status">
									<input type="hidden" id="officeway_${officeway.officewayId}" value="${officeway.status1}"/>
									<tr>
										<td class="textCenter">
											<c:if test="${empty officeway.status2 && (officeway.status1 == 'T' || officeway.status1 == 'S')}">
										...
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<c:choose>
												...
												<c:when test="${officeway.status1 == 'T' && empty officeway.status2}">
													임시저장
												</c:when>
											</c:choose>
										</td>
				...
				<div class="nblockButton"> 
				<a id='createBoardItemButton1' class='button_img01' href="<c:url value='/lightpack/officeway/officewayUseRequestForm.do'/>"><span>작성</span></a>
					<c:if test="${tmpStatus == '1'}">
						<c:if test="${periodCheck}">
							<a id="saveButton" class="button_img01" href="#a"><span>신청</span></a>
						</c:if>
						<a id='updateButton' class='button_img01' href='<c:url value='/lightpack/officeway/officewayUseRequestMyListUpdateForm.do'/>'><span>수정</span></a>
					</c:if>
					<a id="deleteButton" class="button_img01" href="#a"><span><ikep4j:message pre='${preButton}' key='delete'/></span></a>
