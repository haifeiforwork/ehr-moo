...
<c:set var="companyCodeListPrefix" value="message.portal.admin.code.companyCode.companyCodeList"/>
        ...
        //<!--
            ...
            function getList() {
                $jq("#searchForm").attr("action", "<c:url value='/lightpack/officeway/exceptOfficewayList.do' />");
            ...
            function checkCode() {
                if($jq("#productNo").val()=='') {
                    ...
                    $jq.ajax({
                        url : '<c:url value="/lightpack/officeway/checkProductno.do" />',     
            ...
            function deleteForm() {
                ...
                if(confirm("<ikep4j:message pre="${companyCodeListPrefix}" key="confirm.wannaDelete" />")) {
                    $jq.ajax({
                        url : '<c:url value="/lightpack/officeway/deleteOfficewayExcept.do" />',
              ...
              //  $jq("#companycodeOfLeft").click();
                ...
                $jq("#submitButton").click(function() {
	                if (($jq("#codeCheck").val() == 'success') || ($jq("#codeCheck").val() == 'modify')) {
	                    $jq.ajax({
	                        url : '<c:url value="/lightpack/officeway/createOfficewayExcept.do" />',
	                        data : $jq("#company").serialize(),
	                        type : "post",
	                        success : function(result) {
                ...
                var validOptions = {
                        ...
                        /* submitHandler : function(company) {alert();
                            ...
                            if (($jq("#codeCheck").val() == 'success') || 
                                    ($jq("#codeCheck").val() == 'modify')) {
                                $jq.ajax({
                                    url : '<c:url value="/createOfficewayExcept.do" />',
                                    data : $jq("#company").serialize(),
                                    type : "post",
                                    success : function(result) {
                                        alert("<ikep4j:message pre="${companyCodeListPrefix}" key="alert.saveCompleted" />");
                            ...
                            <tr>
                                ...
                                <td>
                                    <div>
                                        ...
                                        &nbsp;<a class="button_img05" id="checkCodeButton" href="#a"><span><ikep4j:message pre="${companyCodeUiPrefix}" key="checkDuplicated" /></span></a>
                                    </div>
                                </td>
                            </tr>
                            <tr>
        ...
        <div class="nblockButton">
          <a class="button_img01" id="submitButton" href="#a"><span><ikep4j:message pre="${companyCodeUiPrefix}" key="button.save" /></span></a>
               <a class="button_img01" id="deleteButton" href="#a"><span><ikep4j:message pre="${companyCodeUiPrefix}" key="button.delete" /></span></a>
