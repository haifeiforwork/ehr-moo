...
<c:forEach var="officeway" items="${searchResult.entity}" varStatus="status">
	<c:if test="${officeway.status1 == 'S'}">
		<c:set var="tmpStatus1"  value="1" />
	</c:if>
	<c:if test="${officeway.status1 == 'C'}">
...
//<!--
	...
	function getList() {
		...
		$jq("#searchForm").attr("action", "<c:url value='/lightpack/officeway/officewayUseRequestTeamListPayment.do' />");
	...
	$jq(document).ready(function() {
		...
		$jq("#confirmButton1,#confirmButton2").click(function() {
			...
			if(confirm("선택한 사무용품을 상신하시겠습니까?")) {
				$jq("#regForm").attr("action", "<c:url value='/lightpack/officeway/officewayTeamCheckBoxUseRequest.do' />");
				$jq("#regForm")[0].submit();
			}  
		});
		...
		$jq("#excelButton").click(function() { 
			...
			$jq("#searchForm").attr("action","<c:url value='/lightpack/officeway/downloadExcelOfficewayTeam.do'/>");
			$jq("#searchForm")[0].submit();
			return false; 
		});
	});
			...
			<form id="searchForm" name="searchForm" method="post" onsubmit="getList(); return false;" action="">
				...
				<div class="nblockButton"> 
					<c:if test="${periodCheck}">
						...
						<a id='updateButton1' class='button_img01' href='<c:url value='/lightpack/officeway/officewayUseRequestTeamListUpdateForm.do'/>'><span>수정</span></a>
						</c:if>
						<c:if test="${oftlRole && tmpStatus2 == '1'}">
						<a id="approveButton1" class="button_img01" href="#a"><span>승인</span></a>
						<a id="rejectButton1" class="button_img01" href="#a"><span>반려</span></a>
						</c:if>
					</c:if>
			</div> 
						    ...
						    <c:otherwise>
								<c:forEach var="officeway" items="${searchResult.entity}" varStatus="status">
									<tr>
										<td class="textCenter">
										<c:if test="${periodCheck}">
										<c:if test="${oftlRole && officeway.status1 == 'C' && empty officeway.status2}">
											<input name="checkboxItem" class="checkbox" title="checkbox" type="checkbox" value="${officeway.officewayId}" /> 
										</c:if>
										<c:if test="${oftrRole && officeway.status1 == 'S' && empty officeway.status2}">
											<input name="checkboxItem" class="checkbox" title="checkbox" type="checkbox" value="${officeway.officewayId}" /> 
										</c:if>
										</c:if>
										</td>
								...
								<tr style="background-color:#FAED7D;color:black;font-weight:bold;">
									...
									<td colspan="2" style="border-left:1px solid #e0e0e0;"></td>
								</tr>
						    </c:otherwise>
						</c:choose>
					</tbody>
				...
				<div class="nblockButton"> 
					<c:if test="${periodCheck}">
						...
						<a id='updateButton2' class='button_img01' href='<c:url value='/lightpack/officeway/officewayUseRequestTeamListUpdateForm.do'/>'><span>수정</span></a>
						</c:if>
						<c:if test="${oftlRole && tmpStatus2 == '1'}">
						<a id="approveButton2" class="button_img01" href="#a"><span>승인</span></a>
						<a id="rejectButton2" class="button_img01" href="#a"><span>반려</span></a>
						</c:if>
					</c:if>
			</div> 
			</form>
