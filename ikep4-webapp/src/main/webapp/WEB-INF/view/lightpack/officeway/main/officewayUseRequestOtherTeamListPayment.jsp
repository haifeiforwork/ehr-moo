...
//<!--
	...
	function getList() {
		...
		$jq("#searchForm").attr("action", "<c:url value='/lightpack/officeway/officewayUseRequestOtherTeamListPayment.do' />");
			...
			<form id="searchForm" name="searchForm" method="post" onsubmit="getList(); return false;" action="">
				...
				<div class="nblockButton"> 
					<%-- <c:if test="${periodCheck}"> --%>
						...
						<a id='updateButton1' class='button_img01' href='<c:url value='/lightpack/officeway/officewayUseRequestOtherTeamListUpdateForm.do?teamId=${teamId}'/>'><span>수정</span></a>
						</c:if>
						</c:if>
						...
						<c:choose>
						    ...
						    <c:otherwise>
								<c:forEach var="officeway" items="${searchResult.entity}" varStatus="status">
									<tr>
										<td class="textCenter">
										<c:if test="${ofmlRole && officeway.status1 == 'S' && officeway.status2 == 'C'}">
										<input name="checkboxItem" class="checkbox" title="checkbox" type="checkbox" value="${officeway.officewayId}" /> 
										</c:if>
										<c:if test="${ofmrRole && officeway.status1 == 'S' && empty officeway.status2}">
										<input name="checkboxItem" class="checkbox" title="checkbox" type="checkbox" value="${officeway.officewayId}" /> 
										</c:if>
										</td>
				...
				<div class="nblockButton"> 
					<%-- <c:if test="${periodCheck}"> --%>
						...
						<c:if test="${!empty teamId}">
						<a id='updateButton2' class='button_img01' href='<c:url value='/lightpack/officeway/officewayUseRequestOtherTeamListUpdateForm.do?teamId=${teamId}'/>'><span>수정</span></a>
						</c:if>
						</c:if>
						<c:if test="${ofmlRole && tmpStatus2 == '1'}">
					...
					<%-- </c:if> --%>
					<li><a class="button_img01 btn_list" href="#a"><span>목록</span></a>
			</div> 
			</form>
		</div>
	</div>
