				...
				for(i=0;i<realtyCnt;i++){
					if($("#productNo"+i).val() == "${exceptList.productNo}"){
						if(tempExceptOfficeSupplies == ""){
							tempExceptOfficeSupplies = $("#productNo"+i).val()+"("+$("#productName"+i).val()+")";
						}else{
							tempExceptOfficeSupplies = tempExceptOfficeSupplies+","+$("#productNo"+i).val()+"("+$("#productName"+i).val()+")";
						}
					}
				}
			</c:forEach>
...
<form id="requestForm" name="requestForm" method="post" action="<c:url value='/lightpack/officesupplies/officesuppliesUseRequest.do'/>" >
