...
<c:set var="officesuppliesCnt" value="0"/>
<c:forEach var="officesupplies" items="${searchResult.entity}" varStatus="status">
<c:set var="officesuppliesCnt" value="${officesuppliesCnt+1}"></c:set>
</c:forEach>
...
//<!--
	...
	$jq(document).ready(function() {
		...
		$jq("a.btn_save").click(function(){
			...
			var price1 = "";    
			var price2 = ""; 
			var tempofficesuppliesId = "";
			var officesuppliesId = "";
			
			var tempExceptOfficeSupplies = "";
			
			<c:forEach var="exceptList" items="${exceptOfficesuppliesList}">
				for(i=0;i<realtyCnt;i++){
					if($jq("#productNo"+i).val() == "${exceptList.productNo}"){
						if(tempExceptOfficeSupplies == ""){
							tempExceptOfficeSupplies = $jq("#productNo"+i).val()+"("+$jq("#productName"+i).val()+")";
						}else{
							tempExceptOfficeSupplies = tempExceptOfficeSupplies+","+$jq("#productNo"+i).val()+"("+$jq("#productName"+i).val()+")";
						}
					}
				}
			</c:forEach>
			if(tempExceptOfficeSupplies != ""){
				alert(tempExceptOfficeSupplies+"는 신청 불가능한 상품입니다.");
				return;
			}
			...
			for(i=0;i<realtyCnt;i++){
				...
				//tempcategory1 = $jq("#category1"+i).val();
				tempcategory2 = $jq("#category2"+i).val();
				tempremark = $jq("#remark"+i).val();
				if(tempremark == ""){
					tempremark = "N/A";
				}
				tempofficesuppliesId = $jq("#officesuppliesId"+i).val();
				if(tempofficesuppliesId == ""){
					tempofficesuppliesId = "newcode";
				}
				if(i == 0){
					...
					price1 = tempprice1;
					price2 = tempprice2;
					officesuppliesId = tempofficesuppliesId;
				}else{
					...
					officesuppliesId = officesuppliesId+"|"+tempofficesuppliesId;
				}
			}
			...
			$jq('input[name=officesuppliesId]').val(officesuppliesId);
			$jq('input[name=status1]').val("S");
			$jq("#requestForm").trigger("submit"); 
			return false;
		});
		
		$jq("a.btn_temp_save").click(function(){
			var tempproductNo = ""; 
			//var tempcategory1 = ""; 
			var tempcategory2 = "";
			var tempremark = "";
		...
		$jq("a.realtyPlus").click(function(){	
			$jq("#realtyBody").append(
				...
				"</tr><input name=\"officesuppliesId"+realtyCnt+"\" id=\"officesuppliesId"+realtyCnt+"\"  type=\"hidden\"  />"
			);
...
<div id="reserveDiv">
	...
	<div class="blockListTable">
		<div id="listDiv">
			<form id="listForm" name="listForm" method="post" action="<c:url value='/lightpack/officesupplies/officesuppliesUseRequestMyList.do'/>" ></form>
			<form id="requestForm" name="requestForm" method="post" action="<c:url value='/lightpack/officesupplies/officesuppliesUseRequestUpdate.do'/>" >
				...
				<table summary="<ikep4j:message pre='${preDetail}' key='myReserveList' />">
					...
					<tbody id="realtyBody">
						<c:forEach var="officesupplies" items="${searchResult.entity}" varStatus="status">
							<tr>
								<td style="text-align:center;"><fmt:formatDate value="${officesupplies.registDate}" pattern="yyyy.MM"/></td>
								<%-- <td style="border-left:1px solid #e0e0e0;">
									<select id="category1${status.index}" name="category1${status.index}">
										<c:forEach var="code" items="${categoryList1}">
											<option value="${code.categoryId}" <c:if test="${officesupplies.category1 eq code.categoryId}">selected="selected"</c:if>>${code.categoryName}</option>
										</c:forEach>
									</select>
								</td> --%>
								<td style="border-left:1px solid #e0e0e0;"><input name="productName${status.index}" id="productName${status.index}" title="" class="inputbox w100" type="text" value="${officesupplies.productName}" /></td>
								<td style="border-left:1px solid #e0e0e0;"><input name="productNo${status.index}" id="productNo${status.index}" title="" class="inputbox w90" type="text" value="${officesupplies.productNo}" /></td>
								<td style="border-left:1px solid #e0e0e0;">
									<select id="category2${status.index}" name="category2${status.index}">
										<c:forEach var="code" items="${categoryList2}">
											<option value="${code.categoryId}" <c:if test="${officesupplies.category2 eq code.categoryId}">selected="selected"</c:if>>${code.categoryName}</option>
										</c:forEach>
									</select>
								</td>
								<td style="border-left:1px solid #e0e0e0;"><input name="amount1${status.index}" id="amount1${status.index}" title="" class="inputbox w90" type="text" value="${officesupplies.amount1}" onblur="calculation1(${status.index});" onkeyup="calculation1(${status.index});" /></td>
								<td style="border-left:1px solid #e0e0e0;text-align:center;"><input name="unit${status.index}" id="unit${status.index}" title="" class="inputbox w90" type="hidden" value="${officesupplies.unit}" />EA</td>
								<td style="border-left:1px solid #e0e0e0;"><input name="price1${status.index}" id="price1${status.index}" title="" class="inputbox w90" type="text" value="${officesupplies.price1}" onblur="calculation2(${status.index});" onkeyup="calculation2(${status.index});" /></td>
								<td style="border-left:1px solid #e0e0e0;"><input name="price2${status.index}" id="price2${status.index}" title="" class="inputbox w90" type="text" value="${officesupplies.price2}" readonly="readonly" /></td>
								<td style="border-left:1px solid #e0e0e0;"><input name="remark${status.index}" id="remark${status.index}" title="" class="inputbox w90" type="text" value="${officesupplies.remark}" /></td>
								<td class="textCenter" style="border-left:1px solid #e0e0e0;">
									${officesupplies.registerName}
									<input name="officesuppliesId${status.index}" id="officesuppliesId${status.index}" type="hidden" value="${officesupplies.officesuppliesId}" />
								</td>
							</tr>
						</c:forEach>
					</tbody>
				...
				<table><tr><td style="text-align:right;border:0 0 0 0;">
					...
					<img style="width:20px;" src="<c:url value="/base/images/icon/ic_btn_plus.gif"/>" />
					</a>
				</td></tr></table>
					<div class="blockButton"> 
						<ul>
							<c:if test="${periodCheck}">
							<li><a class="button btn_save" href="#a"><span>신청</span></a></li>
							</c:if>
							<li><a class="button btn_temp_save" href="#a"><span>임시저장</span></a></li>
