...
<!--// 
	...
	$jq("#itemForm").attr("action", "<c:url value='/lightpack/officesupplies/officesuppliesUseRequestStatistics.do' />");
	$jq("#itemForm")[0].submit();
	}
	
	function teamViewPopup(teamId, calMnt) {
		var url = "<c:url value='/lightpack/officesupplies/officesuppliesUseRequestOtherTeamList.do'/>?teamId="+teamId+"&viewMonth="+calMnt;
    					...
    					<td>
					        ...
					        <select  name="teamId" onchange="getList();">
								...
								<c:forEach var="officesuppliesTeam" items="${officesuppliesUseTeamList}" varStatus="status">
									<option value="${officesuppliesTeam.teamId}" <c:if test="${teamId eq officesuppliesTeam.teamId}">selected="selected"</c:if>>${officesuppliesTeam.teamName}</option>
								</c:forEach>
							</select>		                       	
						</td>
    			    </tr>
