...
//<!--
	...
	function getList() {
		...
		$jq("#searchForm").attr("action", "<c:url value='/lightpack/officesupplies/officesuppliesUseRequestTeamsList.do' />");
	...
	$jq(document).ready(function() {
		...
		$jq("#confirmButton1,#confirmButton2").click(function() {
			...
			if(confirm("선택한 부서의 요청사항을 검토완료하시겠습니까?")) {
				$jq("#regForm").attr("action", "<c:url value='/lightpack/officesupplies/officesuppliesCheckBoxGroupRequest.do' />");
				$jq("#regForm")[0].submit();
			}  
		});
		...
		$jq("#excelButton").click(function() { 
			...
			//if(excelBtnCnt == 0){
				$jq("#searchForm").attr("action","<c:url value='/lightpack/officesupplies/downloadExcelOfficesupplies.do'/>");
				$jq("#searchForm")[0].submit();
			//	excelBtnCnt++;
			//}else{
	...
	<div class="blockListTable">
		<div id="listDiv">
			...
			<form id="searchForm" name="searchForm" method="post" onsubmit="getList(); return false;" action="">
				...
				<ul>
					<c:if test="${ofmrRole}">
					<li><a id="confirmButton1" class="button" href="#a"><span>상신</span></a></li>
					</c:if>
					<c:if test="${ofmlRole}">
					<li><a id="approveButton1" class="button" href="#a"><span>확정</span></a></li>
						...
						<c:choose>
						    ...
						    <c:otherwise>
								<c:forEach var="officesupplies" items="${searchResult.entity}" varStatus="status">
									<tr>
										<td class="textCenter">
											<c:if test="${ofmlRole && searchCondition.searchStatus == 'MC' && officesupplies.price2 != '0'}">
												<input name="checkboxItem" class="checkbox" title="checkbox" type="checkbox" value="${officesupplies.teamId}" /> 
											</c:if>
											<c:if test="${ofmrRole && searchCondition.searchStatus == 'TA' && officesupplies.price2 != '0'}">
												<input name="checkboxItem" class="checkbox" title="checkbox" type="checkbox" value="${officesupplies.teamId}" /> 
											</c:if>
										</td>
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<a href="<c:url value='/lightpack/officesupplies/officesuppliesUseRequestOtherTeamListPayment.do?teamId=${officesupplies.teamId}'/>" >${nowYear}.${nowMonth}</a>
										</td>
										<td class="textCenter" style="border-left:1px solid #e0e0e0;">
											<a href="<c:url value='/lightpack/officesupplies/officesuppliesUseRequestOtherTeamListPayment.do?teamId=${officesupplies.teamId}'/>" > ${officesupplies.teamName}</a>
										</td>
										<td class="textCenter" title="${officesupplies.productName}" style="border-left:1px solid #e0e0e0;">
											<a href="<c:url value='/lightpack/officesupplies/officesuppliesUseRequestOtherTeamListPayment.do?teamId=${officesupplies.teamId}'/>"  ><u>${officesupplies.productCnt}</u></a>
										</td>
										<td class="textRight" style="border-left:1px solid #e0e0e0;">
											<a href="<c:url value='/lightpack/officesupplies/officesuppliesUseRequestOtherTeamListPayment.do?teamId=${officesupplies.teamId}'/>" ><fmt:formatNumber value="${officesupplies.price2}" type="number" /></a>
										</td>
									</tr>
								</c:forEach>
				...
				<ul>
					<c:if test="${ofmrRole}">
					<li><a id="confirmButton2" class="button" href="#a"><span>상신</span></a></li>
					</c:if>
					<c:if test="${ofmlRole}">
					<li><a id="approveButton2" class="button" href="#a"><span>확정</span></a></li>
					<!-- <li><a id="rejectButton" class="button" href="#a"><span>반려</span></a></li> -->
					</c:if>
				</ul>
			</div> 
