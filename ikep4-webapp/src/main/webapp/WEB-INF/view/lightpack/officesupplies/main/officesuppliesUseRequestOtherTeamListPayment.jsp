...
<c:forEach var="officesupplies" items="${searchResult.entity}" varStatus="status">
	<c:if test="${officesupplies.status1 == 'A' && empty officesupplies.status2}">
		<c:set var="tmpStatus1"  value="1" />
	</c:if>
	<c:if test="${officesupplies.status2 == 'C'}">
...
//<!--
	...
	function getList() {
		...
		$jq("#searchForm").attr("action", "<c:url value='/lightpack/officesupplies/officesuppliesUseRequestOtherTeamListPayment.do' />");
	...
	$jq(document).ready(function() {
		...
		$jq("#confirmButton1,#confirmButton2").click(function() {
			...
			if(confirm("선택한 사무용품을 검토 완료하시겠습니까?")) {
				$jq("#regForm").attr("action", "<c:url value='/lightpack/officesupplies/officesuppliesManageCheckBoxUseRequest.do' />");
				$jq("#regForm")[0].submit();
			}  
		});
		...
		$jq("#rejectButton1,#rejectButton2").click(function() {
		...
		$jq("#excelButton").click(function() { 
			...
			$jq("#searchForm").attr("action","<c:url value='/lightpack/officesupplies/downloadExcelOfficesuppliesOtherTeam.do'/>");
			$jq("#searchForm")[0].submit();
		return false; 
		});
...
<div id="reserveDiv">
	...
	<div class="blockListTable">
		<div id="listDiv">
			<form id="listForm" name="listForm" method="post" action="<c:url value='/lightpack/officesupplies/officesuppliesUseRequestTeamsList.do'/>" ></form>
			...
			<form id="searchForm" name="searchForm" method="post" onsubmit="getList(); return false;" action="">
				...
				<ul>
					<c:if test="${periodCheck}">
						...
						<li><a id="rejectButton1" class="button" href="#a"><span>반려</span></a></li>
						<c:if test="${!empty teamId}">
						<li><a id='updateButton1' class='button' href='<c:url value='/lightpack/officesupplies/officesuppliesUseRequestOtherTeamListUpdateForm.do?teamId=${teamId}'/>'><span>수정</span></a></li>
						</c:if>
						</c:if>
					...
					<tbody>
						<c:choose>
							<c:when test="${searchResult.emptyRecord}">
								<tr>
									<td colspan="13" class="emptyRecord">
						    ...
						    <c:otherwise>
								<c:forEach var="officesupplies" items="${searchResult.entity}" varStatus="status">
									<tr>
										<td class="textCenter">
										<c:if test="${ofmlRole && officesupplies.status1 == 'A' && officesupplies.status2 == 'C'}">
										<input name="checkboxItem" class="checkbox" title="checkbox" type="checkbox" value="${officesupplies.officesuppliesId}" /> 
										</c:if>
										<c:if test="${ofmrRole && officesupplies.status1 == 'A' && empty officesupplies.status2}">
										<input name="checkboxItem" class="checkbox" title="checkbox" type="checkbox" value="${officesupplies.officesuppliesId}" /> 
										</c:if>
										</td>
				...
				<ul>
					<c:if test="${periodCheck}">
						...
						<li><a id="rejectButton2" class="button" href="#a"><span>반려</span></a></li>
						<c:if test="${!empty teamId}">
						<li><a id='updateButton2' class='button' href='<c:url value='/lightpack/officesupplies/officesuppliesUseRequestOtherTeamListUpdateForm.do?teamId=${teamId}'/>'><span>수정</span></a></li>
						</c:if>
						</c:if>
						<c:if test="${ofmlRole && tmpStatus2 == '1'}">
