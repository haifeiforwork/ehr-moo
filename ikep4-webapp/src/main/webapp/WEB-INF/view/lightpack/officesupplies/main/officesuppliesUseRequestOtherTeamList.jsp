...
//<!--
	...
	function getList() {
		...
		$jq("#searchForm").attr("action", "<c:url value='/lightpack/officesupplies/officesuppliesUseRequestOtherTeamList.do' />");
			...
			<form id="searchForm" name="searchForm" method="post" onsubmit="getList(); return false;" action="">
				...
				<ul>
					<c:if test="${periodCheck}">
						...
						<li><a id='updateButton' class='button' href='<c:url value='/lightpack/officesupplies/officesuppliesUseRequestOtherTeamListUpdateForm.do?teamId=${teamId}'/>'><span>수정</span></a></li>
						</c:if>
						</c:if>
