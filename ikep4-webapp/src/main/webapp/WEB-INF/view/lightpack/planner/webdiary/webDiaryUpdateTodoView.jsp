...
(function($) {
	...
	writeSubmit = function(targetForm){
		var data = $jq("#orderForm").serialize();
		$jq.ajax({
			//url : '<c:url value="/lightpack/todo/webDiaryUpdateMyTaskAjax.do"/>?'+$jq("#orderForm").serialize(),
			url : '<c:url value="/lightpack/todo/webDiaryUpdateMyTaskAjax.do"/>',
			type : "post",
			data : data,
			success : function(result) {
				window.parent.getMyTaskList("","");
				window.parent.getTaskDetail(result.taskId);
			} /*,
...
*/
				...
				<div class="blockDetail">
					<form id="orderForm" name="orderForm" action="<c:url value="/lightpack/todo/webDiaryUpdateMyTask.do"/>" method="post">
					...
					<input type="hidden" name="${status.expression}" id="etcName" value="${user.userId}"/>
					</spring:bind>	
					...
					<spring:bind path="task.subworkCode">
					<input type="hidden" name="${status.expression}" id="etcName" value="${task.subworkCode}"/>
					</spring:bind>	
