...
(function($) {
	...
	getList = function(isScrollSet) {
		...
		$jq.ajax({     
			url : '<c:url value="/lightpack/cafe/cafe/getSubListCafeImage.do" />',     
		...
		<div id="blockMain">
				...
				<form id="searchForm" method="post" action="" onsubmit="return false">  
					...
					<spring:bind path="searchCondition.cafeId">  
							<input name="${status.expression}" id="${status.expression}" type="hidden" title="cafeId" value="${status.value}" />
					</spring:bind>
