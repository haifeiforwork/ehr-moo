...
(function($) {
	...
	$(document).ready(function() {  
		...
		$("a[name=createBoardButton]").click(function() {   
			...
			<c:if test="${board.boardType eq 0}">
				$("#boardForm").attr("action", "<c:url value='/lightpack/cafe/board/boardAdmin/createBoardTypeBoard.do'/>");
			</c:if>
			<c:if test="${board.boardType eq 1}">
				$("#boardForm").attr("action", "<c:url value='/lightpack/cafe/board/boardAdmin/createCategoryTypeBoard.do'/>");
			</c:if>	 
			<c:if test="${board.boardType eq 2}">
				$("#boardForm").attr("action", "<c:url value='/lightpack/cafe/board/boardAdmin/createLinkTypeBoard.do'/>");
			</c:if>
