...
function iframeReflash()  {
	 location.href = "<c:url value='/lightpack/cafe/board/boardItem/listBoardItemView.do?boardId=${board.boardId}&searchConditionString=${searchConditionString}&popupYn=${popupYn}'/>";  
};
...
(function($){	 
	...
	moveBoardItem = function(result) {
		...
		$jq.ajax({
			url : "<c:url value='/lightpack/cafe/board/boardItem/moveBoardItem.do'/>?orgBoardId="+orgBoardId +"&targetBoardId="+targetBoardId+"&itemIds="+itemIds,
			type : "get",
	...
	$(document).ready(function() {   
		...
		$("#adminMultiDeleteBoardItemButton").click(function() {  
			...
			if(confirm("<ikep4j:message pre="${preMessage}" key="deleteItem" />")) {
				...
				$.post("<c:url value='/lightpack/cafe/board/boardItem/adminMultiDeleteBoardItem.do'/>", {"boardId" : "${board.boardId}", "itemId" : itemIds.toString(), "popupYn" : ${popupYn}}) 
		...
		$("#btnLayoutVertical").click(function(event) { 
			...
			$.get('<c:url value="/lightpack/cafe/board/boardItem/updateUserConfigLayout.do?boardId=${board.boardId}&layoutType=layoutVertical"/>');
		...
		$("#btnLayoutHorizental").click(function(event) {
			...
			$.get('<c:url value="/lightpack/cafe/board/boardItem/updateUserConfigLayout.do?boardId=${board.boardId}&layoutType=layoutHorizental"/>');
		...
		$("#btnLayoutNormal").click(function(event) { 
			...
			$.get('<c:url value="/lightpack/cafe/board/boardItem/updateUserConfigLayout.do?boardId=${board.boardId}&layoutType=layoutNormal"/>');
		...
		$("a.boardItem").click(function() {
			...
			$(this).parents("*.boardItemLine").addClass("bgSelected");
			 ...
			 if(!isLayout) {  
				<c:choose>
					<c:when test="${popupYn}"> 
						iKEP.popupOpen($(this).attr("href")+ "&popupYn=true&searchConditionString=${searchConditionString}&popupYn=${popupYn}", {width:1000, height:600, resizable:false, callback:function(result) {
							location.href = "<c:url value='/lightpack/cafe/board/boardItem/listBoardItemView.do'/>?boardId=" + result[0] + "&searchConditionString=" +  result[1] + "&popupYn=${popupYn}";  
						}}, "param");   
				...
				$("#layoutContent").html("");  
				$("#layoutContent").load($(this).attr("href") + "&layoutType=layoutVertical", function(res, status, xhr) {
					if(status == "error") {
						$(this).empty()
							.html(res);
					}
				...
				$("#layoutContent").html(""); 
				$("#layoutContent").load($(this).attr("href") + "&layoutType=layoutHorizental", function(res, status, xhr) {
					if(status == "error") {
						$(this).empty()
							.html(res);
					}
		...
		//	iKEP.popupOpen("<c:url value='/lightpack/cafe/board/boardItem/readBoardItemView.do?boardId=${board.boardId}&itemId=${itemId}&searchConditionString=searchConditionString&popupYn=${popupYn}'/>", {width:1000, height:600, resizable:false, callback:function(result) {
		//		location.href = "<c:url value='/lightpack/cafe/board/boardItem/listBoardItemView.do'/>?boardId=" + result[0] + "&searchConditionString=" +  result[1] + "&popupYn=${popupYn}";  
		//	}}, "param");			 
		//</c:if>
...
<form id="searchBoardItemForm" method="post" action="<c:url value='/lightpack/cafe/board/boardItem/listBoardItemView.do' />">  
			...
			<div id="listDiv"> 
				...
				<c:choose>
					<c:when test="${searchCondition.viewMode eq '0'}">
						...
						<%@ include file="/WEB-INF/view/lightpack/cafe/board/boardItem/listTypeBoardItemView.jsp"%> 
					</c:when>
					<c:when test="${searchCondition.viewMode eq '1'}">
						<div class="blockListTable summaryView"> 
						<%@ include file="/WEB-INF/view/lightpack/cafe/board/boardItem/simpleTypeBoardItemView.jsp"%>
					</c:when> 				
					<c:when test="${searchCondition.viewMode eq '2'}">
						<div class="blockListTable summaryView"> 
						<%@ include file="/WEB-INF/view/lightpack/cafe/board/boardItem/summaryTypeBoardItemView.jsp"%>
					</c:when>
					<c:when test="${searchCondition.viewMode eq '3'}">
						<div class="blockListTable galleryView"> 
						<%@ include file="/WEB-INF/view/lightpack/cafe/board/boardItem/galleryTypeBoardItemView.jsp"%>
					</c:when>  
				</c:choose>
			...
			<div class="blockButton"> 
				<c:if test="${board.rss eq 1}">
					<div class="buttonicon" id="feedDiv" >
						<input type="hidden" id="feedMetaType" name="feedMetaType" value="CF"/>		
				...
				<ul>
					<c:if test="${searchCondition.isAll=='yes'}"><%--카페 글쓰기 버튼 --%>
						<li><a id='createBoardItemButton' class='button' href='<c:url value='/lightpack/cafe/board/boardItem/createBoardItemViewForMain.do'/>?searchConditionString=isAll^yes~cafeId^${cafe.cafeId}&cafeId=${cafe.cafeId}'><span><ikep4j:message pre='${preButton}' key='create'/></span></a></li>
					</c:if>
					<c:if test="${permission.isWritePermission}"><%--관리자와 쓰기권한자인 경우 등록 버튼을 활성화 시킨다. --%>
						<li><a id='createBoardItemButton' class='button' href='<c:url value='/lightpack/cafe/board/boardItem/createBoardItemView.do?boardId=${searchCondition.boardId}&amp;searchConditionString=${searchConditionString}&amp;popupYn=${popupYn}'/>'><span><ikep4j:message pre='${preButton}' key='create'/></span></a></li>
					</c:if> 
