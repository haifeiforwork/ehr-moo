<%--
=====================================================
* 기능 설명 : Workflow Workplace 업무목록 진행중 업무목록
* 작성자 : 이재경
=====================================================
--%>
...
<script type="text/javascript">
	...
	(function($){
		$(document).ready(function(){
			...
			$("#searchProgressWorkListButton").click(function() {   
				$("#pageIndex").val("1");
				$("#myForm").submit(); 
				return false; 
			});  
			...
			$("#searchkeyword").keypress(function(event){
	            if(event.which == 13) {
	            	$("#searchProgressWorkListButton").click();
	            }
	        });
			
			$jq("select[name=pagePerRecord]").change(function() {
	            //검색용 폼을 서브밋하는 코드 넣으시면 됩니다. 
	            $jq("#searchProgressWorkListButton").click();  
	        });  
		});
	})(jQuery);
...
<!--pageTitle Start-->
<div id="pageTitle">
	<h2><ikep4j:message pre='${prefix}' key='progressworklist'/></h2>
	<div id="pageLocation">
		<ul>
			<li class="liFirst"><ikep4j:message pre='${prefix}' key='workplace'/></li>
			<li><ikep4j:message pre='${prefix}' key='todolist'/></li>
			<li class="liLast"><ikep4j:message pre='${prefix}' key='progressworklist'/></li>
		</ul>
	</div>
</div>
<!--//pageTitle End-->

<form id="myForm" method="post" action="<c:url value='/workflow/workplace/worklist/progressWorkList.do' />">
	<!--blockSearch Start-->
	<div class="blockSearch">
		<div class="corner_RoundBox03">
			...
			<div class="searchBtn">
				<a id="searchProgressWorkListButton" name="searchProgressWorkListButton" href="">
		...
		<table summary="My To-Do 목록">
			...
			<thead>
				<tr>
					...
					<th scope="col" width="8%"><ikep4j:message pre='${prefix}' key='username' /></th>
			...
			<tbody>
				<c:choose>
					<c:when test="${!empty searchResult.entity}">
						<c:forEach var="progressWorkList" items="${searchResult.entity}" varStatus="status">
						<tr onmouseover="mouseOver(this);" onmouseout="mouseOut(this);">
							<td>${(searchResult.recordCount-((searchResult.pageIndex-1)*workplaceSearchCondition.pagePerRecord)-status.count)+1}</td>
							<td>${progressWorkList.partitionName}</td>					
							<td class="textLeft ellipsis">${progressWorkList.processName}</td>
							<td>${progressWorkList.activityName}</td>
							<td class="textLeft ellipsis">${progressWorkList.title}</td>
							<td>
								<c:choose>
									<c:when test="${user.localeCode == portal.defaultLocaleCode}">
		  								${progressWorkList.userName}
	  								</c:when>
	  								<c:otherwise>
	  									${progressWorkList.userEnglishName}
	  								</c:otherwise>
	  							</c:choose>
							</td>
							<td><ikep4j:timezone date="${progressWorkList.procStartDate}" pattern="yyyy.MM.dd HH:mm"/></td>
							<td>
								<a class="button_s" href="javascript:popupState('${progressWorkList.instanceId}', '${progressWorkList.partitionId}', '${progressWorkList.processId}', '${progressWorkList.processVer}');"><span><ikep4j:message pre='${prefixBtn}' key='history' /></span></a>
							</td>
						</tr>
						</c:forEach>
					</c:when>
