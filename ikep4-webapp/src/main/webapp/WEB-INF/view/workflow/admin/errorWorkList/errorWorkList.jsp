<%--
=====================================================
* 기능 설명 : Workflow Workplace 업무목록 에러 업무목록
* 작성자 : 이재경
=====================================================
--%>
...
<!--pageTitle Start-->
<div id="pageTitle">
	<h2><ikep4j:message pre='${prefix}' key='errorworklist'/></h2>
	<div id="pageLocation">
		<ul>
			<li class="liFirst"><ikep4j:message key='ui.workflow.admin.processAdministration.workflow'/></li>
			<li><ikep4j:message key='ui.workflow.admin.processAdministration.current'/></li>
			<li class="liLast"><ikep4j:message pre='${prefix}' key='errorworklist'/></li>
		</ul>
	</div>
</div>
<!--//pageTitle End-->

<form id="myForm" method="post" action="<c:url value='/workflow/admin/workplace/errorWorkList.do' />">
	<!--blockSearch Start-->
		...
		<table summary="My To-Do 목록">
			...
			<thead>
				<tr>
					...
					<th scope="col" width="12%"><ikep4j:message pre='${prefix}' key='errorMsg' /></th>
				</tr>
			</thead>
			<tbody>
				<c:choose>
					<c:when test="${!empty searchResult.entity}">
						<c:forEach var="errorWorkList" items="${searchResult.entity}" varStatus="status">
						<tr onmouseover="mouseOver(this)"; onmouseOut="mouseOut(this)">
							<td>${(searchResult.recordCount-((searchResult.pageIndex-1)*workplaceSearchCondition.pagePerRecord)-status.count)+1}</td>
							<td>${errorWorkList.partitionName}</a></td>					
							<td>${errorWorkList.processName}</td>
							<td>${errorWorkList.activityName}</td>
							<td class="textLeft ellipsis">${errorWorkList.title}</td>
							<td>${errorWorkList.userName}</td>
							<td>${errorWorkList.procStartDate}</td>
							<td class="textLeft ellipsis">${errorWorkList.resultMsg}</td>
						</tr>
						</c:forEach>
					</c:when>
					<c:otherwise>
