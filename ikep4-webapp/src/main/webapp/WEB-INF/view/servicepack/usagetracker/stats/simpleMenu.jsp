...
(function($) {
	$(document).ready(function() {
		...
		$("#excelDownload").click(function() { 
			if("${searchResult.recordCount}" > 5000){
				alert("엑셀다운로드는 5000건 이하만 가능합니다.");
				return;
			}
			
			$jq("#searchForm").attr("action","<c:url value='/servicepack/usagetracker/stats/excel/simpleMenu.do'/>");
			$jq("#searchForm").submit(); 
			return false; 
		});
		...
		var setAction = function(){
			$jq("#searchForm").attr("action","<c:url value='/servicepack/usagetracker/stats/simpleMenu.do'/>");
		};
...
-->
				...
				<!--//pageTitle End-->
				<form id="searchForm" action="<c:url value='/servicepack/usagetracker/stats/simpleMenu.do'/>" method="post">
