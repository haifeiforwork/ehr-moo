...
(function($) {
	$(document).ready(function() {
		...
		$("#excelDownload").click(function() { 
			$jq("#searchForm").attr("action","<c:url value='/servicepack/usagetracker/stats/excel/browser.do'/>");
			$jq("#searchForm").submit(); 
			return false; 
		});
		...
		var setAction = function(){
			$jq("#searchForm").attr("action","<c:url value='/servicepack/usagetracker/stats/browser.do'/>");
		};
	...
	$(window).load(function () { 
		...
		<c:forEach var="info" items="${result.data}"> 
			 var ticks=[];
			 ticks.push("${info.moduleId} : " + ${info.usageCount}); 
		...
		var plot2 = $.jqplot('browserStatsGraph', [s1], {
	    	grid: {
	    		drawBorder: true,
	    		drawGridlines: false,
	    		background: '#ffffff',
...
-->
				...
				<!--pageTitle Start-->
				<div id="pageTitle">
					<h2><ikep4j:message pre='${preStats}' key='pagetitle.browser'/></h2>
				</div>
				<!--//pageTitle End-->
				<form id="searchForm" action="<c:url value='/servicepack/usagetracker/stats/browser.do'/>" method="post">
				...
				<div class="corner_RoundBox02">
					<div class="blockLeft" style="width:48%">
						<ul class="graphText">
							<li><span class="emphasis"><ikep4j:message pre='${preStats}' key='left.browser.${result.searchOption}'/></span>
							</li>
							<li>&nbsp;</li>
							<li>
								 <span class="browser">${result.maxUseAge}</span><ikep4j:message pre='${preStats}' key='left.browser.end'/>
							</li>
						</ul>
						<div class="clear"></div>
					</div>
					<div class="blockRight" style="width:50%">
						<div class="usage_graph" style="text-align:center;">
							<span><strong><ikep4j:message pre='${preStats}' key='top.browser.${result.searchOption}'/> <ikep4j:message pre='${preStats}' key='top.browser.end'/></strong></span>
							<div id="browserStatsGraph" style="margin-top:20px; margin-left:20px; width:97%; height:300px;"></div> 
						</div>
					</div>
					<div class="clear"></div>
					<div class="l_t_corner"></div>
				...
				<div class="blockListTable">  
					...
					<div class="tableTop">  
						<div class="listInfo">  
							...
							<span><ikep4j:message pre='${preStats}' key='grid.browser' /></span> 
							</div>
					...
					<table id="infoTable" summary="">   
						...
						<thead>
							<tr>
								...
								<th scope="col"><ikep4j:message pre='${preStats}' key='grid.browserField' /></th>
								<th scope="col"><ikep4j:message pre='${preStats}' key='grid.accessUser' /></th>
								<th scope="col"><ikep4j:message pre='${preStats}' key='grid.registTime' /></th>
							</tr>
						</thead> 
						<tbody>
							<c:choose>
							    ...
							    <c:otherwise>
									...
									<tr>
										<td> ${rowIndex}</td>
										<td class="textLeft ellipsis">${info.browser}</td>
