...
<c:set var="preUi" value="ui.servicepack.usagetracker.portletusagerank" />
	...
	$(document).ready(function() {  
	    ...
	    var plot3 = $.jqplot('portletusage', [
  		...
  		$("#excelDown").click(function() {  
  			var pageIndex = ${searchCondition.pageIndex};
  		  $jq("input[name='pageIndex']").val(pageIndex);
			$jq.post("<c:url value='/servicepack/usagetracker/excelDownPortletRank.do'/>", $jq("#searchForm").serialize())
		    ...
		    .error(function(event, request, settings) { alert("category create error"); }
				...
				<div class="blockSearch">
					<div class="corner_RoundBox03">
						<form id="searchForm" action="listPortletUsageRank.do">
				...
				<div class="corner_RoundBox02">
					<div class="blockLeft" style="width:48%">
						<ul class="graphText">
							...
							<li><span class="browser">${portletfirst}</span></li>
							<li><span class="emphasis"><ikep4j:message pre='${preUi}' key='message5'/></span><ikep4j:message pre='${preUi}' key='message4'/> <span class="browser2">${portletlast}</span><ikep4j:message pre='${preUi}' key='message6'/></li>
						</ul>
						<div class="clear"></div>
					</div>
					<div class="blockRight" style="width:50%">
						<div class="usage_graph" style="text-align:center;"><span><strong><ikep4j:message pre='${preUi}' key='message1'/> ${returnTerm}<ikep4j:message pre='${preUi}' key='message2'/> <ikep4j:message pre='${preUi}' key='message3'/> <ikep4j:message pre='${preUi}' key='message7'/></strong></span>
							<div id="portletusage" style="margin-top:20px; margin-left:20px; width:97%; height:200px;"></div>
						</div>
				...
				<div class="tableTop">		
					...
					<div class="tableSearch">
						<a id="excelDown" href="#a"><img src="<c:url value="/base/images/icon/ic_xls.gif"/>" alt="<ikep4j:message pre='${common}' key='excel.down'/>" /></a>
					</div>			
					<div class="clear"></div>
				</div>
				<!--//tableTop End-->
					...
					<table summary="detail">
						...
						<thead>
							<tr>
								...
								<th scope="col" width="10%"><ikep4j:message pre='${preUi}' key='tableheader3'/> <a href="#a"><img src="<c:url value="/base/images/icon/ic_tablesort_down.gif"/>" alt="<ikep4j:message pre='${common}' key='order.desc'/>" /></a></th>
								<th scope="col" width="10%"><ikep4j:message pre='${preUi}' key='tableheader4'/></th>
							</tr>
						</thead>
						<tbody>
							<c:forEach var="tmp" items="${portletUsageRankList.entity}" varStatus="inx">
							<tr>
								<td>${tmp.rnum}</td>
								<td>${tmp.portletname}</td>
								<td>${tmp.usagecnt}</td>
