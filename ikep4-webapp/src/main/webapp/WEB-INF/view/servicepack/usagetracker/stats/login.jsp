...
(function($) {
	...
	$(document).ready(function() {
		...
		$("#excelDownload").click(function() { 
			$jq("#searchForm").attr("action","<c:url value='/servicepack/usagetracker/stats/excel/login.do'/>");
			$jq("#searchForm").submit(); 
			return false; 
		});
		...
		var setAction = function(){
			$jq("#searchForm").attr("action","<c:url value='/servicepack/usagetracker/stats/login.do'/>");
		};
	...
	grathWidthSet = function(){
		var rightWidth = $(".corner_RoundBox02").width() - 320;
		$(".blockRight").width(rightWidth);
		$(".usage_graph").width(rightWidth-12);	
		rightWidth = Math.floor(rightWidth*0.97);
		$("#loginStatsGraph").width(rightWidth);
	};	
...
-->
				...
				<!--//pageTitle End-->
				<form id="searchForm" action="<c:url value='/servicepack/usagetracker/stats/login.do'/>" method="post">
				...
				<div class="corner_RoundBox02">
					<div class="blockLeft" style="width:300px;">
						<ul class="graphText">
							<li><span class="emphasis"><ikep4j:message pre='${preStats}' key='left.login.${result.searchOption}'/></span>
							</li>
							<li>&nbsp;</li>
							<li>
								 <span class="browser">${result.minUseAge}</span>&nbsp;<span class="browser">${result.maxUseAge}</span>&nbsp;<ikep4j:message pre='${preStats}' key='left.login.time.${result.viewOption}'/>
							</li>
						</ul>
						<div class="clear"></div>
					</div>
					<div class="blockRight">
						<div class="usage_graph" style="text-align:center;">
							<span><strong><ikep4j:message pre='${preStats}' key='top.login.${result.searchOption}'/> <ikep4j:message pre='${preStats}' key='top.login.time.${result.viewOption}'/></strong></span>
							<div id="loginStatsGraph" style="margin-top:20px; margin-left:20px; height:300px;"></div> 
						</div>
					</div>
					<div class="clear"></div>
					<div class="l_t_corner"></div>
				...
				<div class="blockListTable">  
					...
					<div class="tableTop">  
						<div class="listInfo">  
							...
							<span><ikep4j:message pre='${preStats}' key='grid.login' /></span> 
							</div>
							<div class="tableSearch">
								<a href="#a" id="excelDownload" title="<ikep4j:message  key='ui.servicepack.usagetracker.common.excel.down' />"><img src="<c:url value='/base/images/icon/ic_xls.gif'/>" alt="<ikep4j:message  key='ui.servicepack.usagetracker.common.excel.down' />" /></a>
							</div>
						</div>	  
					...
					<table id="infoTable" summary="">   
						...
						<thead>
							<tr>
								...
								<th scope="col"><ikep4j:message pre='${preStats}' key='grid.loginUser' /></th>
								<th scope="col"><ikep4j:message pre='${preStats}' key='grid.loginTime' /></th>
							</tr>
						</thead> 
						<tbody>
							<c:choose>
							    ...
							    <c:otherwise>
									...
									<tr>
										...
										<td>
										<ikep4j:timezone  date="${info.loginDate}"  pattern="message.servicepack.usagetracker.timezone.dateformat.yyyyMMddhhmmss" keyString="true"/></td>
									</tr>
