...
(function($) {
	...
	$(document).ready(function() {
		new iKEP.Slider("#divPhotoList", {left:"#btnLeft", right:"#btnRight"});
		...
		<c:forEach var="info" items="${proTagList}"  varStatus="loopStatus">
			<c:if test="${loopStatus.count == 1}">
				showExpertTagList('${info.userId}');
			</c:if>
		</c:forEach>
	
	});
	
	showExpertTagList = function(userId) {
		
		$jq.ajax({     
			url : '<c:url value="/servicepack/intelligentsearch/expertTagList.do" />',     
			data :  {'userId':userId},     
			type : "post",     
			success : function(result) {  
				$jq("#expertTagList").html(result);
			}, 
...
(function($) {
	$(document).ready(function() {
		...
		$.template("rowtemplet1", '<li class="tag_No\${index}"><strong><span class="none">No.\${index}</span><a href="#">\${searchKeyword}</a></strong></li>');
	});
})(jQuery);
//-->
 </script> 
				...
				<div class="Intelligent_l">
									...
									<tr>
										<td>
											<div class="Intel_search_con">
												<input name="searchKeyword" id="searchKeyword" class="inputbox" title="search" type="text" value="${searchKeyword}"/>
					...
					<div class="people_Ph">
						...
						<div class="people_photo">
							<div id="btnLeft" class="button-left">
								<a class="people_pre" href="#a"><span>pre</span></a>
							</div>
							<div id="btnRight" class="button-right">
								<a class="people_next" href="#a"><span>next</span></a>
							</div>
							<div id="divPhotoList" class="people_photoList">
								<c:choose>
									<c:when test="${!empty proTagList}">
									<c:forEach var="info" items="${proTagList}"  varStatus="loopStatus">
										<div class="pList">
											<a href="#a" onclick="viewPorfile('${info.userId}');" onmouseover="showExpertTagList('${info.userId}');"><img src="<c:url value='${info.profilePicturePath}'/>" width="50" height="50" alt="image" onerror="this.src='<c:url value='/base/images/common/photo_50x50.gif'/>'" /></a>
											<p><a href="#a" onclick="iKEP.showUserContextMenu(this, '${info.userId}', 'bottom');">
												<c:choose>
						...
						<div class="corporateViewTag" id="expertTagList">
						</div>
						...
						<div class="PowerUser_Title">
							...
							<c:choose>
								...
								<c:forEach var="info" items="${topFiveRankList}"  varStatus="loopStatus">
									...
									<c:choose>
								      <c:when test="${user.localeCode == portal.defaultLocaleCode}">
								       ${info.userName} ${info.jobTitleName} 
								      </c:when>
								      <c:otherwise>
								       ${info.userEnglishName} ${info.jobTitleEnglishName}  
								      </c:otherwise>
								     </c:choose>
									</a></li>
								</c:forEach>	
