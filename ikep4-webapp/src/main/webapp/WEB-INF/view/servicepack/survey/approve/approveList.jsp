...
<ikep4j:message  pre='${preButton}' key='read' var="read"/>
...
$jq(document).ready(function() { 
 ...
 $jq("a[name='approveButton']").click(function() {   
		alert("<ikep4j:message pre='${preButton}' key='auth' arguments='${read}'/>"); 
		return false; 
	});
...
<!--mainContents Start-->
	...
	<form id="searchForm" method="post" action="<c:url value='/servicepack/survey/approve/approveList.do' />">  
	<input type="hidden" name="redirect" value="approveList" title=""/>
				    ...
				    <c:otherwise>
						...
						<tr>
							...
							<td class="textLeft ellipsis">
								<c:choose>
									<c:when test="${user.userId  eq surveyInfo.approverId || isAdmin }">
										<a href="<c:url value='/servicepack/survey/approve/readApprove.do'/>?surveyId=${surveyInfo.surveyId}">${surveyInfo.title}</a>
									</c:when>
									<c:otherwise>
										<a name="approveButton" href="#a" approverId="${surveyInfo.approverId}">${surveyInfo.title}</a>
									</c:otherwise>
								</c:choose>
							</td>
							...
							<td><ikep4j:timezone  date="${surveyInfo.updateDate}"  pattern="message.servicepack.survey.timezone.dateformat.yyyyMMddhhmmss" keyString="true"/>
							</td>
