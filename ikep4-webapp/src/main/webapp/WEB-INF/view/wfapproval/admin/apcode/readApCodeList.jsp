...
<c:set var="preCode"    value="ui.wfapproval.admin.apcode.view" />
...
<!-- 
	...
	(function($) {
				...
				if(!codeTree) {
					codeTree = $treeDiv.jstree({
						...
						json_data 	: {
										...
										ajax : {
												url 	: "<c:url value="/wfapproval/admin/apcode/ajaxTreeCodeChild.do"/>",
        ...
        f_ApCodeDetailForm = function(selectData){
			...
			//코드 상세화면 코드명 설정.
			$("#selectApCodeName").text("("+codeName+")");
			
			//코드 상세화면
			$("#codeDetail").load('<c:url value="/wfapproval/admin/apcode/ajaxReadApCodeView.do?codeId="/>'+codeId)
       		.error(function(event, request, settings) { alert("error"); });
			
			//코드 하위목록화면
			$("#codeChildList").load('<c:url value="/wfapproval/admin/apcode/ajaxListCodeChild.do?parentCodeId="/>'+codeId)
       		.error(function(event, request, settings) { alert("error"); });
        };
		...
		f_ApCodeCreateForm = function(selectData){
			...
			//코드 상세화면
			$("#codeDetail").load('<c:url value="/wfapproval/admin/apcode/ajaxReadApCodeView.do?parentCodeId="/>'+codeId)
       		.error(function(event, request, settings) { alert("error"); });
			
			//코드 하위목록화면
			$("#codeChildList").load('<c:url value="/wfapproval/admin/apcode/ajaxListCodeChild.do"/>')
       		.error(function(event, request, settings) { alert("error"); });
		};
