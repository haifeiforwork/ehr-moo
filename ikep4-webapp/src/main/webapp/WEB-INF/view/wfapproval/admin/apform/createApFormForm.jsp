...
<c:set var="preHeader"  	value="ui.wfapproval.admin.apform.form.header" /> 
<c:set var="preList"    	value="ui.wfapproval.admin.apform.form" />
	...
	(function($) {
		...
		f_ViewApFormProcessSearch = function () {
			...
			viewApFormProcessDialog = new iKEP.Dialog({     
				...
				width 		: 1200, //650,
				height 		: 600,
				params 		: {id:$processId.val(), name:$processName.val(), type:$processType.val(), ver:'' },
		...
		$(document).ready(function() {
			 ...
			 * CK Edit Init
			 */
			$("#apprFormData").ckeditor($.extend(fullCkeditorConfig, {"popupTitle" : "<ikep4j:message key='ui.support.fileupload.header.title' />" }));
			 ...
			 * ApForm 버튼영역 실행
			 */
			$("#createApFormButton a").click(function(){
	            switch (this.id) {
			 ...
			 * ApFormTpl 버튼영역 실행
			 */
			$("#createApFormTplButton a").click(function(){
	            switch (this.id) {
	...
	<div id="apFormTab" class="iKEP_tab">
	    ...
	    <div class="tab_con">
	        ...
	        <div id="tabs-1">
					...
					<table summary="<ikep4j:message pre="${preViewForm}" key="subtitle" />">
						...
						<tbody>
							<tr>
								...
								<th width="18%" scope="row"><ikep4j:message pre='${preViewForm}' key='${status.expression}' /></th>
								<td width="82%">
									<select name="${status.expression}" title="<ikep4j:message pre='${preViewForm}' key='${status.expression}' />">
										...
										<c:forEach var="apCode" items="${listFormClassCd}">
											<option value="${apCode.codeId}">${apCode.codeName}</option>
										</c:forEach>
									</select>
								</td>
								</spring:bind>
							...
							<tr>
								...
								<th scope="row"><ikep4j:message pre='${preViewForm}' key='${status.expression}' /></th>
								<td>
									<input name="${status.expression}" value="${status.value}" type="text" class="inputbox w50" title='<ikep4j:message pre='${preViewForm}' key='${status.expression}' />'  size="50" />
								</td>
								</spring:bind>
							</tr>
							<tr>
								...
								<td>
									<label>
										<input 	type="radio" class="radio" title="<ikep4j:message pre='${preCommon}' key='use'/>" 
												name="${status.expression}" value="Y" checked /><ikep4j:message pre='${preCommon}' key='use'/>
									</label>&nbsp;&nbsp;
									<label>
										<input 	type="radio" class="radio" title="<ikep4j:message pre='${preCommon}' key='unuse'/>" 
												name="${status.expression}" value="N"/><ikep4j:message pre='${preCommon}' key='unuse'/>
									</label>
								</td>
								</spring:bind>
							</tr>
				...
				<!--blockButton Start-->
				<div id="createApFormButton" class="blockButton"> 
					<ul>
						<li><a id="applyApFormButton"   class="button" href="#a"><span><ikep4j:message pre='${preButton}' key='save'/></span></a></li>
			...
			<form id="apFormTpl" name="apFormTpl" modelAttribute="apFormTpl" method="post">
						...
						<tbody>
							<tr>
								...
								<td>
									<select name="${status.expression}" title="<ikep4j:message pre='${preViewFormTpl}' key='${status.expression}' />">
										<option value=""><ikep4j:message pre="${preSearch}" key="select"/></option>
										<c:forEach var="apCode" items="${listMailInfoCd}">
											<option value="${apCode.codeId}">${apCode.codeName}</option>
										</c:forEach>
									</select>
								</td>
							...
							<tr>
								...
								<td>
									<select name="${status.expression}" title="<ikep4j:message pre='${preViewFormTpl}' key='${status.expression}' />">
										<option value=""><ikep4j:message pre="${preSearch}" key="select"/></option>
										<c:forEach var="apCode" items="${listMailRetCd}">
											<option value="${apCode.codeId}">${apCode.codeName}</option>
										</c:forEach>
									</select>
								</td>
				...
				<!--blockButton Start-->
				<div id="createApFormTplButton" class="blockButton"> 
					<ul>
						<li><a id="applyApFormTplButton"   class="button" href="#a"><span><ikep4j:message pre='${preButton}' key='save'/></span></a></li>
						<li><a id="listApFormTplButton" 	class="button" href="#a"><span><ikep4j:message pre='${preButton}' key='list'/></span></a></li>
					</ul>
				</div>
				<!--//blockButton End-->
	        </div>
