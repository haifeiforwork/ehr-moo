...
<c:choose>
	...
	<c:when test="${listType eq 'completeList'}">
		<c:set var="formActUrl"     value="/wfapproval/work/aplist/listApComplete.do" />
	</c:when>
...
<!--pageTitle Start-->
<div id="pageTitle">
	<h2><ikep4j:message pre='${preApCommList}.pageTitle' key='${listType}' /></h2>
	<div id="pageLocation">
		<ul>
			...
			<c:choose>
				<c:when test="${listType eq 'myRequestList'}">
				<li><ikep4j:message pre='${preApCommList}.subtitle' key='request' /></li>
				</c:when>
				<c:otherwise>
				<li><ikep4j:message pre='${preApCommList}.subtitle' key='approval' /></li>
				</c:otherwise>
			</c:choose>			
			<li class="liLast"><ikep4j:message pre='${preApCommList}.pageTitle' key='${listType}' /></li>
		</ul>
	</div>
</div>
<!--//pageTitle End-->
			    ...
			    <c:otherwise>
					<c:forEach var="aplist" items="${apList.entity}" varStatus="i">
						...
						<tr class="${className}">
							...
							<td class="textLeft"><a href="<c:url value="/wfapproval/work/apdoc/readApDoc.do?linkType=mywork&apprId="/>${aplist.appKey01}&instanceId=${aplist.instanceId}&insLogId=${aplist.insLogId}&activityId=${aplist.activityId}">${aplist.title}</a></td>
					    <c:choose>
					 			<c:when test="${listType ne 'myRequestList'}">
							    <c:choose>
								    <c:when test="${listType eq 'todoList'}">
							...
							<td>
								<c:choose>
									<c:when test="${aplist.state == 'RUNNING' || aplist.state == 'ASSIGN'}">
...
<div class="blockButton"> 
	<ul> 
		...
		<c:if test="${listType eq 'myRequestList'}">
		<li><a id="createApButton" class="button" href="#a"><span><ikep4j:message pre='${preButton}' key='create'/></span></a></li>
		</c:if>
	</ul>
</div>
