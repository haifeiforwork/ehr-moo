			    ...
			    <c:otherwise>
					<c:forEach var="aplist" items="${apList.entity}" varStatus="i">
						...
						<tr class="${className}">
							...
							<td class="textLeft"><a href="<c:url value="/wfapproval/work/apdoc/readApDoc.do?apprId="/>${aplist.appKey01}&instanceId=${aplist.instanceId}&insLogId=${aplist.insLogId}&activityId=${aplist.activityId}">${aplist.title}</a></td>
					    <c:choose>
					 			<c:when test="${listType ne 'myRequestList'}">
							    <c:choose>
								    <c:when test="${listType eq 'todoList'}">
