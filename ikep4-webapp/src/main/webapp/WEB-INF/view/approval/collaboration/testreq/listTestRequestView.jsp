...
<c:set var="preSearch2"  	value="ui.approval.testreq.searchCondition" />
...
(function($){	
	...
	goView = function( trMgntNo) {
		var moveFrm = $("#moveForm");
		moveFrm.find("input[name='viewMode']").val("modify");
		moveFrm.find("input[name='trMgntNo']").val( trMgntNo);
		moveFrm.submit();
	},
...
// 		}; 
		...
		$("#searchButton").click(function() {
			...
			for( var i=0; i < 3; i++) {
				...
				var addChar = "0";
				var isChecked = $("#searchCompanyChkVal"+i).is(":checked");
				if( isChecked) {
					addChar = "1";
				}
				companyChkVal = companyChkVal + "" + addChar;
			}
			$("#searchForm").find("input[name='searchCompanyChkVal']").val(companyChkVal);
		...
		//[검색 -달력] 
		$("input[name=searchStartWriteDate]").datepicker({dateFormat: 'yy.mm.dd',yearRange:'c-20:c+10'}).next().eq(0).click(function() { $(this).prev().eq(0).datepicker("show"); });
		$("input[name=searchEndWriteDate]").datepicker({dateFormat: 'yy.mm.dd',yearRange:'c-20:c+10'}).next().eq(0).click(function() { $(this).prev().eq(0).datepicker("show"); });
		...
		var searchCompanyChkVal = $("input[name='searchCompanyChkVal']").val();
			...
			if( searchCompanyChkVal.length == 3) {
				...
				for( var i=0; i < 3; i++) {
					...
					if( val == "1") {
						...
						$("#searchCompanyChkVal"+i).attr("checked", true);
					}
				}
			}
		}
...
<!-- moverForm -->
<form id="moveForm" action="<c:url value='/approval/collaboration/testreq/editTestRequestView.do' />"  method="post">
	<input type="hidden" name="viewMode" value="" />
	<input type="hidden" name="trMgntNo" value="" />
...
<!--mainContents Start-->
			...
			<table summary="tableSearch">
				<tbody>
					<tr>
                        ...
                        <td width="40%">
                        <spring:bind path="searchCondition.searchStartWriteDate">
				           	...
				           	<img src="<c:url value="/base/images/icon/ic_cal.gif"/>" align="absmiddle" alt="calendar"/>
				           	 ...
				           	 - 
				        <spring:bind path="searchCondition.searchEndWriteDate">
				           	<input id="${status.expression}" name="${status.expression}" class="inputbox w20 datepicker" type="text" value="${status.value}"  maxlength="10"/>
				           	<img src="<c:url value="/base/images/icon/ic_cal.gif"/>" align="absmiddle" alt="calendar"/>
				        </spring:bind>
                        </td>
                        ...
                        <!-- 진행상태 -->
	                    <th scope="row" width="5%"><ikep4j:message pre="${preSearch2}" key='processStatus' /></th>
	                    <td width="40%;">
							<spring:bind path="searchCondition.searchProcessStatus">
					...
					<tr>
						<!-- 작성자 -->
	                    <th scope="row" width="5%"><ikep4j:message pre="${preSearch2}" key='writeEmpNo' /></th>
						...
						<!-- 수신자 -->
	                    <th scope="row" width="5%"><ikep4j:message pre="${preSearch2}" key='rcvEmpNo' /></th>
	                    <td width="*" style="padding-top: 5px;">
	                    	<spring:bind path="searchCondition.searchRcvEmpNo">
		                    	<input name="${status.expression}" class="searchUserId" type="hidden" value="${status.value}"  />
	                    	</spring:bind>
	                    	<spring:bind path="searchCondition.searchRcvEmpNm">
		                    	<input name="${status.expression}" class="searchUserName inputbox" type="text" value="${status.value}" size="20" />
	                    	</spring:bind>
					...
					<!-- 제목 -->
					<tr>
						<th scope="row" width="5%"><ikep4j:message pre="${preSearch2}" key='testReqTitle' /></th>
						<td>
							<spring:bind path="searchCondition.searchTestReqTitle">
								<input name="${status.expression}" class="searchUserName inputbox" type="text" value="${status.value}" size="50" />
							</spring:bind>
						</td>
						<th scope="row" width="5%"><ikep4j:message pre="${preSearch2}" key='itemKindCd' /></th>
						<td>
							<spring:bind path="searchCondition.searchItemKindCd">
								<select name="${status.expression}" >
									<option value=""><ikep4j:message pre='${preDefault}' key='option.all'/></option>
					...
					<!-- 품목/회사 -->
					<tr>
						<th scope="row" width="5%"><ikep4j:message pre="${preSearch2}" key='companyChkVal' /></th>
						<td>
							<input type="checkbox" id="searchCompanyChkVal0" class="checkbox"/> <span>MP</span>
							<input type="checkbox" id="searchCompanyChkVal1" class="checkbox"/> <span>P&P</span>
							<input type="checkbox" id="searchCompanyChkVal2"/ class="checkbox"> <span>SP</span>
							<input type="hidden" name="searchCompanyChkVal" value="${searchCondition.searchCompanyChkVal }" />
						</td>
					</tr>
				</tbody>
            </table>
	...
	<div id="container">
		...
		<div class="blockListTable" id="listDiv"> 
			<%@ include file="/WEB-INF/view/approval/collaboration/testreq/listTestRequestIncView.jsp"%>
