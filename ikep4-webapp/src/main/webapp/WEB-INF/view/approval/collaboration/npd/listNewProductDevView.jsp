...
<c:set var="preSearch2"  	value="ui.approval.npd.searchCondition" />
...
(function($){	
	...
	goView = function( mgntNo, rsltFileReadYn) {
		var moveFrm = $("#moveForm");
		moveFrm.find("input[name='viewMode']").val("modify");
		moveFrm.find("input[name='mgntNo']").val( mgntNo);
		moveFrm.find("input[name='rsltFileReadYn']").val( rsltFileReadYn);
		moveFrm.submit();
	},
...
// 		}; 
		...
		// [등록]
		$(".createNewProductDevBtn").click( function(){
			var moveFrm = $("#moveForm");
...
<!-- moverForm -->
<form id="moveForm" action="<c:url value='/approval/collaboration/npd/editNewProductDevView.do' />"  method="post">
	<input type="hidden" name="viewMode" value="" />
	<input type="hidden" name="mgntNo" value="" />
	<input type="hidden" name="rsltFileReadYn" value="" />
...
<form id="searchForm" method="post" action="<c:url value='/approval/collaboration/npd/listNewProductDevView.do' />">  
...
<!--mainContents Start-->
		...
		<div class="corner_RoundBox03">
			<table summary="tableSearch">
				<tbody>
					...
					<tr>
						<!-- 의뢰자 -->
	                    <th scope="row" width="5%"><ikep4j:message pre="${preSearch2}" key='reqUser' /></th>
						...
						<!-- 담당자(기안자) -->
	                    <th scope="row" width="5%"><ikep4j:message pre="${preSearch2}" key='tcsUser' /></th>
	                    <td width="*" style="padding-top: 5px;">
	                    	<spring:bind path="searchCondition.searchTcsDraftEmpNo">
		                    	<input name="${status.expression}" class="searchUserId" type="hidden" value="${status.value}"  />
	                    	</spring:bind>
	                    	<spring:bind path="searchCondition.searchTcsDraftEmpNm">
		                    	<input name="${status.expression}" class="searchUserName inputbox" type="text" value="${status.value}" size="20" />
	                    	</spring:bind>
					...
					<tr>
						<th scope="row" width="5%"><ikep4j:message pre="${preSearch2}" key='productName' /></th>
						<td colspan="3">
	...
	<div class="blockButton">
		<ul>
			<li>
				<a class="button createNewProductDevBtn" href="#a">
	...
	<div id="container">
		...
		<div class="blockListTable" id="listDiv"> 
			<%@ include file="/WEB-INF/view/approval/collaboration/npd/listNewProductDevIncView.jsp"%>
