		...
		$(document).ready(function(){
			...
			var validOptions = {
				rules			: {
			  	...
			  	messages		: {
			  		rejectReason : {
			  			required: "<ikep4j:message pre='${preTitle}' key='rejectReason'/>" + " " + "<ikep4j:message pre='${preMessage2}' key='required'/>"
				    }
			  	},
			    submitHandler : function(form) {
					...
					if (!confirm("<ikep4j:message pre='${preMessage}' key='save'/>")) {
			 		...
			 		$jq.ajax({
						url : iKEP.getContextRoot() + "/approval/collaboration/npd/ajaxRejectNewProductDev.do",
						type : "post",
						data : $("#editForm").serialize(),
						loadingElement : "#saveButton",
						success : function(data, textStatus, jqXHR) {
							alert("<ikep4j:message pre='${preMessage2}' key='rejectSuceess' />");
			...
			<form id="editForm" name="editForm" method="post" action="">
				...
				<table summary="<ikep4j:message pre='${preView}' key='summary'/>">
					...
					<tbody>
						<tr>
							<!-- 업무구분 -->
							<th scope="row" width="20%" style="text-align:center;"><ikep4j:message pre='${preTitle}' key='rejectReason' /></th>
		                    <td width="*">
								<spring:bind path="newProductDev.rejectReason">
									<textarea name="${status.expression}" <c:if test="${npdPermission.rejectBtnActive eq false}">readonly="readonly"</c:if> style="line-height:16px; overflow:auto; width:98%; height:100px; border:none; background:none;">${status.value}</textarea>
								</spring:bind>
		                    </td>
						</tr>
					</tbody>
		...
		<div class="blockButton" style="margin-top: 10px;"> 
			<ul>
				<c:if test="${npdPermission.rejectBtnActive eq true}">
					<li><a id="saveButton" class="button" href="#a"><span><ikep4j:message pre='${preButton2}' key='deny'/></span></a></li>
					<li><a class="button cancelButton" href="#a"><span><ikep4j:message pre='${preButton}' key='cancel'/></span></a></li>
				</c:if>
				<c:if test="${npdPermission.rejectBtnActive eq false}">
