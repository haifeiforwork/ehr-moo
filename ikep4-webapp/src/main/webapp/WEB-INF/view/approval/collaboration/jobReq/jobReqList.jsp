...
<form id="searchBoardItemForm" method="post" action="<c:url value='/approval/collaboration/jobReq/jobReqList.do'/>">
	...
	<div class="tableTop">
		...
		<div class="listInfo">
			...
			<div class="totalNum">
				...
				<span> ${jobReqResultList.recordCount}</span>
			</div>
		</div>
     ...
     <!--blockButton Start-->
       <div class="blockButton" style="position:absolute; right:0; bottom:0px; padding-top:10px;"> 
           <ul>
               ...
               <li><a class="button" href="<c:url value='/approval/collaboration/jobReq/jobReqCreate.do'/>"><span>신규</span></a></li>
           </ul>
       </div>
...
<%-- 	               	   <select name="${status.expression}" title='<ikep4j:message pre='${preSearch}' key='${status.expression}' />'> --%>
	...
	<div class="blockListTable">
		<table summary="업무 의뢰 요청서">
			<caption></caption>
			<thead>
				<tr>
					<th scope="col" width="5%">NO</th>
					<th scope="col" width="7%">
						<a onclick="sort('APPR_REQ_DATE', '<c:if test="${searchCondition.sortColumn eq 'REQ_DRAFT_DATE'}">${searchCondition.sortType}</c:if>');"  href="#a">
						의뢰일
						</a>
					<c:choose>
					...
					<th scope="col" width="20%">
						<a onclick="sort('JOB_REQ_TITLE', '<c:if test="${searchCondition.sortColumn eq 'JOB_REQ_TITLE'}">${searchCondition.sortType}</c:if>');"  href="#a">
							제목
						</a>
					<c:choose>
						<c:when test="${searchCondition.sortColumn eq 'JOB_REQ_TITLE' and searchCondition.sortType eq 'DESC'}"><img alt="<ikep4j:message pre="${preSearch}" key="sortType" post="desc" />" src="<c:url value='/base/images/icon/ic_tablesort_down.gif'/>"></c:when>
						<c:when test="${searchCondition.sortColumn eq 'JOB_REQ_TITLE' and searchCondition.sortType eq 'ASC'}"><img alt="<ikep4j:message pre="${preSearch}" key="sortType" post="asc" />" src="<c:url value='/base/images/icon/ic_tablesort_up.gif'/>"></c:when>
					</c:choose>    
					</th>
			...
			<tbody>
				<c:choose>
					<c:when test="${jobReqResultList.emptyRecord}">
						<tr>
					...
					<c:otherwise>
						<c:forEach var="resultList" items="${jobReqResultList.entity}"
							varStatus="status">
							<tr onmouseover="mouseOver(this);" onmouseout="mouseOut(this);" >
								...
								<td class="textLeft" >
								<a href="<c:url value='/approval/collaboration/jobReq/jobReqDetail.do?id=${resultList.jobReqMgntNo}&processStatus=${resultList.processStatus}&reqApprEmpNo=${resultList.reqApprEmpNo}&rcvApprEmpNo=${resultList.rcvApprEmpNo}'/>">${resultList.jobReqTitle}
								</td>
