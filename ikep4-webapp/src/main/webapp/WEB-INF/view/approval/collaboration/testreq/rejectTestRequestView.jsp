			...
			var validOptions = {
			    ...
			    submitHandler : function(form) {
					...
					if (!confirm("<ikep4j:message pre='${preMessage}' key='save'/>")) {
			 		...
			 		$jq.ajax({
						url : iKEP.getContextRoot() + "/approval/collaboration/testreq/ajaxRejectTestRequest.do",
						type : "post",
	...
	<div class="blockDetail" style="width: 98%;">
				...
				<table summary="<ikep4j:message pre='${preView}' key='summary'/>">
					...
					<tbody>
						<tr>
		                    ...
		                    <td width="*">
								<spring:bind path="testRequest.rejectReason">
									<textarea name="${status.expression}" <c:if test="${permission.rejectBtnActive eq false}">readonly="readonly"</c:if> style="line-height:16px; overflow:auto; width:98%; height:100px; border:none; background:none;">${status.value}</textarea>
								</spring:bind>
		                    </td>
						</tr>
					</tbody>
		...
		<div class="blockButton" style="margin-top: 10px;"> 
			<ul>
				...
				<c:if test="${permission.rejectBtnActive eq false}">
