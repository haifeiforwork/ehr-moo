...
<!--// 
	...
	(function($) {
        ...
        $(document).ready(function() {   
    	    ...
    	    var validOptions = {
    	        ...
    	        submitHandler : function(form) {
                    ...
                    item.sort           = $("#itemForm input[name=sort]:checked").val();
                    
                    if(item.dataType=="1"){
                        item.options        = [];
                        $("#itemForm select[name=options] option").each(function() {
                            item.options.push({"text" : $(this).text(), "value" : $(this).val(), "checked" : $(this).hasClass("selectedOption")?"1":"0"});
                        });     
                    }else{
                        item.lastSeq = 0;
            ...
            if(parent.mode=="edit" && parent.json.type==item.type){
                ...
                $("#itemForm input[name=sort]:input[value=" + parent.json.sort + "]").attr("checked", true);
                ...
                if(parent.json.dataType=="1"){
                    ...
                    $.each(parent.json.options, function(){
                        selected = this.checked=="1" ? " class=\"selectedOption\"":"";
            ...
            $("#addOptionButton").click(function() {
                ...
                item.lastSeq++;
                var optionValue = "C" + item.lastSeq;
                var checked     = $("#itemForm input[name=checked]").attr("checked")?" class=\"selectedOption\"":"";   
        ...
        setOptionForm = function(txt, val, b){
            ...
            $("#itemForm input[name=checked]").attr("checked", b);
        ...
        selectTitle = function(){
            if($("#itemForm input[name=title]").val()=="")
                $("#itemForm input[name=title]").focus();
            else
                $("#itemForm input[name=title]").select();
        }
	})(jQuery);  
    	...
    	<form id="itemForm" name="itemForm" method="post">
    		<table summary="<ikep4j:message pre='${preView}' key='basicInfo'/>">
    			...
    			<tbody>
    	            ...
    	            <tr>
    					<th width="10%" scope="row"><ikep4j:message pre='${preIfm}' key='sort'/></th>
    					<td width="90%">
    						<input type="radio" class="radio" name="sort" value="H" checked="checked" title="<ikep4j:message pre='${preIfm}' key='horizontal'/>"> <ikep4j:message pre='${preIfm}' key='horizontal'/>
                            <input type="radio" class="radio" name="sort" value="V" title="<ikep4j:message pre='${preIfm}' key='vertical'/>"> <ikep4j:message pre='${preIfm}' key='vertical'/>
    					</td>
    	            </tr>
    	            <tr>
    	            ...
    	            <tr id="optionValueTr">
    					...
    					<td style="padding-top:7px;">
                            ...
                            <ikep4j:message pre='${preIfm}' key='defaultValue'/> : <input type="checkbox" class="checkbox" name="defaultValue" />
