	...
	$(document).ready(function() {	
		...
		var validOptions = {
			...
			submitHandler : function() {	
				$("#blockMain").ajaxLoadStart("button");			
				$jq.ajax({
					url : '<c:url value='/approval/admin/apprDefLine/ajaxSaveDefLine.do'/>',
				...
				<div id="tabs-1">
						...
						<div class="blockRight" style="margin-top:10px;width:50%; border:2px solid #eee; padding:10px 10px 0 10px;">				
							<div class="sbox">				
								...
								<div class="blockDetail">
									...
									<tbody>
									<tr>
										<spring:bind path="apprDefLine.defLineType">
										<th scope="row" width="10%"><ikep4j:message pre='${preView}' key='${status.expression}' /></th>
										<td width="65%" id="defLineType" class="textLeft">
										<input type="radio" class="radio" name="${status.expression}" title="<ikep4j:message pre='${preView}' key='${status.expression}' />" value="0" <c:if test="${status.value==0}">checked="checked"</c:if> /> <ikep4j:message pre='${preCommon}' key='innerAppr' />
										<input type="radio" class="radio" name="${status.expression}" title="<ikep4j:message pre='${preView}' key='${status.expression}' />" value="1" <c:if test="${status.value==1}">checked="checked"</c:if> /> <ikep4j:message pre='${preCommon}' key='collAppr' />
										</td>	
										</spring:bind>							
									</tr>	
									<tr>
					...
					<div class="blockButton suttle_btn">
						<ul>
							...
							<li><a id="btnList" class="button"><span><ikep4j:message pre='${preButton}' key='list' /></span></a></li>
						</ul>
					</div>
					</form>
				</div>
