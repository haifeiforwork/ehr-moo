...
(function($){	 
	...
	$(document).ready(function() { 
		$("#updatePageCount").click(function() { 
		    $.post('<c:url value="/approval/work/portlet/exam/updatePageCount.do"/>', $("#configForm").serialize()) 
		    .success(function(data) {
		    	alert('<ikep4j:message pre='${preMsg}' key='updateSuccess' />');
		    	
		    	$("#portletApprovalExamList").parent().parent().parent().trigger("click:reload");
		    })
...
<div id="portletApprovalExamList" class="tableList_1"> 
			    ...
			    <c:otherwise> 
					<c:forEach var="myRequestItem" items="${searchResult.entity}" varStatus="status"> 
						<tr>
							...
							<td class="textCenter">
								<div class="ellipsis">
								<a href="#a" onclick="iKEP.showUserContextMenu(this, '${myRequestItem.examReqId}', 'bottom');" title="${myRequestItem.examReqName}">${myRequestItem.examReqName}</a> 
								</div>
							</td> 
							<td class="textRight">
								<div class="ellipsis">
								<span class="date"><ikep4j:timezone date="${myRequestItem.examReqDate}" pattern="MM.dd"/></span>
								</div>
							</td>
						</tr>	
					</c:forEach>				    
