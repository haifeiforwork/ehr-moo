...
<script type="text/javascript">
	...
	var	NODOC		="<ikep4j:message pre='${preMessage}' key='noDoc'/>";		
		...
		$(document).ready(function(){
			...
			var validOptions = {
				submitHandler	: function(form) {
					...
					if( !existsDocStatus(apprId) ) {
						dialogWindow.callback("NODOC");
						dialogWindow.close();
						return false;
					}
						...
						$jq.ajax({     
							...
							success : function(result) {
								if(result == "nowPasswordError") {
									...
									$jq.ajax({     
										url : '<c:url value="/approval/work/apprLine/updateRecEndApproval.do"/>',     
										data :  $jq("#myForm").serialize(),     
										type : "post",     
										success : function(result) {      
											if(result == "OK") {
