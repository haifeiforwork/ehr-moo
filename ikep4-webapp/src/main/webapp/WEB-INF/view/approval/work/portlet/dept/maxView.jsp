					...
					<tbody>
						<c:choose>
						    <c:when test="${searchResult.emptyRecord}">
							    <tr>
							    	<td colspan="7" class="emptyRecord"><ikep4j:message pre='${preMsg}' key='emptyRecord' /></td> 
							    </tr>				        
						    </c:when>
						    <c:otherwise> 
								<c:forEach var="aplist" items="${searchResult.entity}" varStatus="status"> 
									<tr>
										...
										<td class="textCenter"><a href="#a" onclick="iKEP.showUserContextMenu(this, '${aplist.registerId}', 'bottom')">${aplist.registerName}</a></td>
										<td class="textCenter">${aplist.apprGroupName}</td>
										<td class="textCenter">
											<span class="date"><ikep4j:timezone pattern="yyyy.MM.dd HH:mm" date="${aplist.apprReqDate}"/></span>
										</td>
										<td>대기</td>
									</tr>	
								</c:forEach>				    
						    ...
						    <c:otherwise> 
								<c:forEach var="aplist" items="${userSearchResult.entity}" varStatus="status"> 
									<tr>
										...
										<td class="textCenter">${aplist.apprGroupName}</td>
										<td class="textCenter">
											<span class="date"><ikep4j:timezone pattern="yyyy.MM.dd HH:mm" date="${aplist.apprEndDate}"/></span>
										</td>
										<td>대기</td>
									</tr>	
								</c:forEach>			    
