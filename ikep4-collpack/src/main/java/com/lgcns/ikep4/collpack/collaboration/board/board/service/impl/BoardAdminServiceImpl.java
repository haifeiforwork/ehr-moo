 ...
 */
@Service("wsBoardAdminServiceImpl")
public class BoardAdminServiceImpl implements BoardAdminService {
	...
	public Board readBoard(String boardId) {
		...
		map.put("workspaceId", board.getWorkspaceId());
	...
	private void createTag(Board board, User user) {
		...
		if (!StringUtil.isEmpty(board.getTag())) {
			...
			tag.setTagItemSubType(board.getWorkspaceId()); // 모듈 서브 타입 - 있을때만 넣기
			tag.setTagItemName(board.getBoardName()); // 게시판 이름
			tag.setTagItemContents(board.getDescription()); // 게시판 소개글
			tag.setTagItemUrl("/collpack/collaboration/board/boardItem/listBoardItemView.do?docPopup=true&workspaceId="
					+ board.getWorkspaceId() + "&boardId=" + board.getBoardId()); // 게시판
																					// 팝업창
																					// url
	...
	public List<Board> listByBoardRootIdForMobile(Map<String, String> map) {
		List<Board> boardList = this.boardDao.listByBoardRootIdForMobile(map);
