 ...
 */
@Service("kmsBoardLinereplyServiceImpl")
// @Transactional
...
//	private ActivityStreamService activityStreamService;
	 ...
	 * 게시물 댓글 목록
	 */
	public SearchResult<BoardLinereply> listBoardLinereplyBySearchCondition(
			BoardLinereplySearchCondition searchCondtion) {
	...
	public String createBoardLinereply(BoardLinereply boardLinereply) {
		...
		//등록자 그룹세팅
		HashMap<String, String> userInfo = (HashMap<String, String>) this.userDao.getKmsUserGroup(boardLinereply.getRegisterId());
		boardLinereply.setGroupId(userInfo.get("GROUP_ID"));
		boardLinereply.setGroupName(userInfo.get("GROUP_NAME"));
...
//				IKepConstant.ACTIVITY_CODE_LINE_REPLY_POST,
//				boardLinereply.getItemId(), boardLinereply.getContents());
	...
	public void updateScore(BoardLinereply boardLinereply){
		if (!this.boardLinereplyDao.scoreExists(boardLinereply)) {
			if (!this.boardLinereplyDao.linereplyExists(boardLinereply)) {
				this.boardLinereplyDao.insertScore(boardLinereply);
			}
		}
	}
	...
	public String createReplyBoardLinereply(BoardLinereply boardLinereply) {
		...
		BoardLinereply parent = this.boardLinereplyDao.get(boardLinereply
				.getLinereplyParentId());
...
//		// Activity Stream 댓글 등록
//		this.activityStreamService.create(IKepConstant.ITEM_TYPE_CODE_BBS,
//				IKepConstant.ACTIVITY_CODE_LINE_REPLY_EDIT,
//				boardLinereply.getItemId(), boardLinereply.getContents());
...
//		// Activity Stream 댓글 등록
//		this.activityStreamService.create(IKepConstant.ITEM_TYPE_CODE_BBS,
//				IKepConstant.ACTIVITY_CODE_LINE_REPLY_DELETE,
//				boardLinereply.getItemId(), boardLinereply.getContents());
		this.boardLinereplyDao.deleteLinereplyScore(boardLinereply);
	...
	public void userDeleteBoardLinereply(BoardLinereply boardLinereply) {
		// 하위 댓글 개수를 구한다.
		Integer count = this.boardLinereplyDao.countChildren(boardLinereply
				.getLinereplyId());
...
//					boardLinereply.getItemId(), boardLinereply.getContents());
			...
			this.boardLinereplyDao
					.physicalDeleteThreadByLinereplyId(boardLinereply
							.getLinereplyId());
		...
		//BoardLinereply boardLinereply1 = this.boardLinereplyDao.get(boardLinereply.getLinereplyId());
