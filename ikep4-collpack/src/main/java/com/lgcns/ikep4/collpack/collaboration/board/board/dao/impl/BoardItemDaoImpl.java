 ...
 */
@Repository("wsBoardItemDaoImpl")
public class BoardItemDaoImpl extends GenericDaoSqlmap<BoardItem, String> implements BoardItemDao {

	/** The Constant NAMESPACE. */
	private static final String NAMESPACE = "collpack.collaboration.board.board.dao.boardItem.";
	...
	//LOCATION:협업 공지사항 + 게시글 service
	/**
	 * 게시물 목록
	 */
	public List<BoardItem> listBySearchConditionMain(BoardItemSearchCondition boardItemSearchCondition) {
		return this.sqlSelectForList(NAMESPACE + "listBySearchConditionMain", boardItemSearchCondition);
	}
	...
	public void consultationRequest(Counsel counsel){
		this.sqlInsert(NAMESPACE + "consultationRequest", counsel);
	}
	
	public Integer selectConsultationRequestListCount(Map<String, String> map){
		return (Integer) sqlSelectForObject(NAMESPACE + "selectConsultationRequestListCount" , map);
	}

	public List<Map<String, Object>> selectConsultationRequestList(Map<String, String> map){
		return getSqlMapClientTemplate().queryForList(NAMESPACE + "selectConsultationRequestList" , map);
	}
