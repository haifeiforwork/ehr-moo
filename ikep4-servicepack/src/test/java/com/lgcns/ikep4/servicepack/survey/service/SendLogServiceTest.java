...
public class SendLogServiceTest extends AbstractTransactionalJUnit4SpringContextTests {
	...
	public  void setUpBeforeClass() {
		...
		for (int i = 0; i <MAX_TARGET_USER; i++) {
			...
			log.setReceiverId(idgenService.getNextId());
			sendLogList.add(log);
		}
	...
	public void create() { 
		...
		for (SendLog sendLog : sendLogList) {
			result = sendLogService.create( sendLog );
		}
	...
	public void testUpdate() { 
		...
		for (SendLog sendLog : sendLogList) {
			...
			result = sendLogService.exists( key );
	...
	public void getSendMailListAllBySurveyId() { 
		...
		List<SendLog> sendLogList = sendLogService.getSendMailListAllBySurveyId(survey.getSurveyId());
	...
	public void getSendMailListAllByUser() { 
		...
		List<SendLog> sendLogList = sendLogService.getSendMailListAllByUser(searchSendLog);
	...
	public void getSendNotMail() { 
		...
		List<SendLog> sendLogList = sendLogService.getSendNotMail(survey.getSurveyId());
	...
	public void getTotalSendLog() { 
		...
		Integer count = sendLogService.getTotalSendLog(survey.getSurveyId());
	...
	public void delete() {
		for (SendLog sendLog : sendLogList) {
			SendLogKey key = sendLogService.create( sendLog );
			sendLogService.delete( key );
		}
